...
import {
  ...
  ChevronDown, ChevronRight, Clock, DollarSign, FileSpreadsheet,
  FileText, Filter, Loader2, Lock, Search, Sliders, Square,
  CheckSquare, Users, X, XCircle, Eye, Calendar,
  Play,
} from 'lucide-react';
import {
  RegistryPageShell,
...
export default function Payroll() {
  ...
  const payrollRegistryStatItems = useMemo(() => {
    const monthName = MONTHS[new Date().getMonth()];
    const year = new Date().getFullYear();
    const staffSet = new Set(filtered.map((r) => r.staffCode || r.staffName));
    const totalPayroll = filtered.reduce((s, r) => s + Number(r.finalPayable || 0), 0);
    const disbursed = filtered.filter((r) => r.status === 'Paid').reduce((s, r) => s + Number(r.amount || 0), 0);
    const pendingPay = filtered.filter((r) => r.status === 'Pending').reduce((s, r) => s + Number(r.amount || 0), 0);
    return [
      {
        label: 'Total staff',
        value: String(staffSet.size),
        trend: `${filtered.length} request rows`,
        icon: Users,
        tone: 'navy',
      },
      {
  ...
  /* ─ Render ─────────────────────────────────────────────────────────── */
  return (
    <div className="min-h-full bg-re-bg pb-20 lg:pb-12">
      <Toast toast={toast} />
      {detailRow && (
        <DetailModal
      ...
      <RegistryPageShell>
        <RegistryPageHeader
          ...
          subtitle={`Approve runs, mark paid, and track disbursements — same card layout as student fees. ${manager?.school?.name ? `School: ${manager.school.name}.` : ''}`}
          secondaryAction={(
            <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap sm:items-center sm:justify-end">
              ...
              >
                ...
                >
                  <FileSpreadsheet size={16} className="text-re-gold shrink-0" /> Export CSV
                </button>
                <button
                  type="button"
              ...
              <div className="relative w-full sm:w-auto" ref={notifRef}>
                <button
                  type="button"
                  onClick={() => setShowNotif((p) => !p)}
                  className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-xs font-bold uppercase tracking-wide text-slate-600 hover:bg-slate-50 sm:w-auto"
                >
                  <Bell size={16} className="text-re-gold" />
                  Alerts
                  {unreadCount > 0 && (
                    <span className="rounded-full bg-re-gold px-1.5 py-0.5 text-[10px] font-black text-[#0b1530]">{unreadCount}</span>
                  )}
                </button>
                {showNotif && (
                  <div className="absolute right-0 top-12 z-50 w-72 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
                    <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
                      <p className="text-xs font-bold uppercase tracking-wide text-slate-800">Notifications</p>
                      <button
                        type="button"
                    ...
                    {notifications.map((n) => (
                      <div key={n.id} className={`border-b border-slate-50 px-4 py-3 ${n.read ? '' : 'bg-amber-50/50'}`}>
              ...
              <button
                ...
                className={`inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl border px-4 text-xs font-bold uppercase tracking-wide transition sm:w-auto ${showAnalytics ? 'border-re-navy bg-re-navy text-white shadow-sm' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'}`}
              >
                <BarChart2 size={16} />
                Payroll analytics
              </button>
          ...
          primaryAction={(
            <button
              ...
              className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-re-gold px-5 py-2.5 text-[13px] font-bold text-[#0b1530] shadow-[0_4px_14px_rgba(254,191,16,0.35)] hover:bg-re-gold-light transition-all disabled:opacity-50 sm:w-auto"
            >
              {loading.fetch ? <Loader2 size={18} className="animate-spin" /> : <Play size={18} strokeWidth={2.5} fill="currentColor" className="opacity-90" />}
              Run payroll
            </button>
          )}
        />

        <RegistryStatGrid items={payrollRegistryStatItems} columns="sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5" />
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-4 py-4 sm:px-6">
            <div className="relative min-w-0 flex-1 max-w-full sm:max-w-md">
              ...
              <input
                ...
                className="h-12 w-full rounded-xl border border-slate-200 bg-slate-50 pl-11 pr-4 text-sm font-medium text-slate-800 outline-none transition focus:border-re-gold/40 focus:bg-white focus:ring-2 focus:ring-re-gold/20"
            ...
            <button
              ...
              className={`inline-flex h-12 shrink-0 items-center justify-center gap-2 rounded-xl border px-4 text-xs font-bold uppercase tracking-wide transition ${showFilters ? 'border-re-navy bg-re-navy text-white' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'}`}
            >
              <Sliders size={14} />
              Filters
        ...
        <RegistryCard>
          <div className="border-b border-slate-100 px-4 py-4 sm:px-6">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Payment tracker</p>
            <h3 className="text-lg font-bold text-slate-900">Disbursement progress by staff</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              <thead className="bg-slate-50 text-[9px] uppercase tracking-[0.15em] text-slate-400 border-b border-slate-200">
