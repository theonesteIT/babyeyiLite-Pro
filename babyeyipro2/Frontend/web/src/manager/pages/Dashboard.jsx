            ...
            <div className="px-4 sm:px-6 lg:px-8 py-5 sm:py-7 max-w-[1480px] mx-auto space-y-6 sm:space-y-8">
                ...
                <section className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div className="min-w-0 space-y-1">
                        <h1 className="text-2xl sm:text-3xl font-bold text-slate-800 tracking-tight">
                            Welcome back, {String(manager?.first_name || 'Manager').toUpperCase()}
                            <span className="ml-1 inline-block" aria-hidden>👋</span>
                        </h1>
                        <p className="text-sm sm:text-[15px] text-slate-500 font-medium max-w-xl">
                            Here is what is happening at {manager?.school?.name ? String(manager.school.name) : 'your school'} today — academics, finance, and attendance at a glance.
                        </p>
                    </div>
                    <div className="flex flex-col sm:flex-row flex-wrap gap-2 sm:gap-3 shrink-0">
                        <button
                            ...
                            className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-[13px] font-semibold text-slate-700 shadow-sm hover:border-slate-300 hover:bg-slate-50 transition-all"
                        >
                        ...
                        <div className="relative">
                            ...
                            >
                                Quick actions
                                <ChevronDown size={16} className={quickMenuOpen ? 'rotate-180 transition-transform' : 'transition-transform'} />
                            </button>
                            {quickMenuOpen && (
                                <>
                                    <button type="button" className="fixed inset-0 z-30 bg-transparent cursor-default" aria-label="Close menu" onClick={() => setQuickMenuOpen(false)} />
                                    <div className="absolute right-0 z-40 mt-2 w-[min(100vw-2rem,16rem)] rounded-2xl border border-slate-200/90 bg-white py-2 shadow-xl">
                                        {[{
                                            label: 'Add / review students',
                                            path: '/students',
                                        }, {
                                            label: 'Collect fees',
                                            ...
                                            <button
                                                key={item.path}
                                                type="button"
                                                className="w-full px-4 py-2.5 text-left text-[13px] font-medium text-slate-700 hover:bg-slate-50"
                                                onClick={() => {
                ...
                <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                    {[{
                        ...
                        trend: feesSparkDelta || `${feeLayout.pct}% of expected`,
                        tint: 'gold',
                        icon: Coins,
                        ...
                        <button
                            ...
                            className="group flex flex-col rounded-2xl border border-slate-200/90 bg-white p-5 text-left shadow-[0_8px_30px_-20px_rgba(15,34,66,0.35)] transition-all hover:-translate-y-0.5 hover:shadow-[0_16px_40px_-24px_rgba(15,34,66,0.45)]"
                        >
                            <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0">
                                    <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">{card.label}</p>
                                    <p className="mt-2 text-xl sm:text-2xl font-bold text-slate-800 tabular-nums truncate">{card.value}</p>
                                    <p className={`mt-1.5 text-xs font-semibold tabular-nums ${String(card.trend || '').startsWith('-') ? 'text-rose-600' : 'text-emerald-600'}`}>
                                        {card.trend}
                                    </p>
                                </div>
                                <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-full ${card.tint === 'gold' ? 'bg-re-gold text-re-navy' : 'bg-[#1E3A5F] text-white'}`}>
