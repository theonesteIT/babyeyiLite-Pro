 ...
 * 
 * Usage:
 *   node bulk-attach-photos-server.js --school-code 01003 input-nursery1.json
 *   node bulk-attach-photos-server.js --school-code 01003
 *   node bulk-attach-photos-server.js input-nursery1.json     (defaults to school_id=1)
 *   node bulk-attach-photos-server.js                          (defaults to school_id=1 + input-students.json)
 * ================================================================
 */
...
async function resolveSchool(db, config) {
  if (!config.SCHOOL_CODE) {
    ...
    console.log('   Usage: node bulk-attach-photos-server.js --school-code 01003 [input.json]\n');
    return 1;
  }
  ...
  if (!school) {
    ...
    console.log('-'.repeat(60));
    console.log(`\nUsage: node bulk-attach-photos-server.js --school-code <CODE> [input.json]\n`);
    process.exit(1);
  }
...
async function main() {
  ...
  try {
    ...
    for (const jsonStudent of students) {
      try {
        ...
        if (!picId) {
          logger.logSkipped(
            { pic_id: picId, json_name: jsonName },
            `No pic_id assigned`
          );
          continue;
        }
        ...
        const match = matcher.findBest(jsonName, allDbStudents);
        if (!match) {
          logger.logSkipped(
            { pic_id: picId, json_name: jsonName, best_score: 'N/A' },
        ...
        logger.logError(
          { pic_id: jsonStudent.pic_id, json_name: jsonStudent.student_name },
          err
        );
      }
    }
