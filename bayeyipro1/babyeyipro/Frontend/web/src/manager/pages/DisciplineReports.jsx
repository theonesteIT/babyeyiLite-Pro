      ...
      <div className="relative w-full min-h-[280px] overflow-hidden bg-[#c87800]">
        <div className="relative z-20 max-w-[1600px] mx-auto px-6 md:px-12 pt-16 pb-24">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-white/80">Student Conduct</p>
          </div>
        ...
        <div className="bg-white rounded-t-[32px] border border-black/5 overflow-hidden flex flex-col">
          <div className="grid grid-cols-1 lg:grid-cols-4 border-b border-black/5">
            <div className="lg:col-span-3 grid grid-cols-2 md:grid-cols-4 divide-x divide-y md:divide-y-0 divide-black/5">
              {[
                ...
                <div key={stat.label} className="p-4 sm:p-8 flex flex-col items-center justify-center text-center group hover:bg-re-bg/20 transition-all">
                  ...
                  <span className="text-sm sm:text-2xl font-black text-re-text tracking-tighter">{stat.value}</span>
            ...
            <div className="hidden lg:flex flex-col border-l border-black/5 bg-re-bg/30 p-6 justify-center gap-3">
              <button
                ...
                className="w-full h-11 flex items-center justify-center gap-2 bg-[#c87800] border border-[#c87800] text-white font-black text-[9px] uppercase tracking-widest rounded-xl hover:opacity-90 transition-all"
              >
                <Activity size={14} />
          ...
          <div className="lg:hidden flex flex-col bg-white border-b border-black/5 divide-y divide-black/5">
            <div className="flex items-center justify-between px-6 py-3">
              ...
              <button onClick={() => setShowMobileClassModal(true)} className="text-[9px] font-black text-[#c87800] uppercase tracking-widest hover:underline">Change Class</button>
            </div>
            <div className="flex items-center justify-between px-6 py-3">
              <span className="text-[10px] font-black text-[#000435] uppercase tracking-widest">{activeAcademicYear} · {activeTerm}</span>
              <span className="text-[9px] font-black text-[#000435]/60 uppercase tracking-widest">From manager settings</span>
            </div>
          </div>
          ...
          <div className="flex p-6 md:px-8 border-b border-black/5 flex-col lg:flex-row items-center gap-3 bg-white/50">
            <div className="relative w-full lg:w-[11rem]">
              ...
              <select
                ...
                className="w-full h-10 bg-white border border-black/5 rounded-xl pl-9 pr-3 text-[10px] font-black uppercase tracking-widest outline-none"
              >
            ...
            <div className="h-10 px-3 inline-flex items-center bg-re-bg rounded-xl border border-black/5 text-[10px] font-black uppercase tracking-widest text-[#000435]">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-re-bg/20 border-b border-black/5">
                  <th onClick={() => toggleSort('name')} className="px-4 sm:px-8 py-3 sm:py-4 text-[7px] sm:text-[8px] font-black text-re-text-muted uppercase tracking-[0.2em] opacity-40 border-r border-black/5 cursor-pointer">Student {sortBadge('name')}</th>
                  <th onClick={() => toggleSort('class')} className="hidden md:table-cell px-8 py-4 text-[8px] font-black text-re-text-muted uppercase tracking-[0.2em] opacity-40 border-r border-black/5 cursor-pointer">Class {sortBadge('class')}</th>
                  <th onClick={() => toggleSort('standing')} className="hidden md:table-cell px-8 py-4 text-[8px] font-black text-re-text-muted uppercase tracking-[0.2em] opacity-40 border-r border-black/5 cursor-pointer">Conduct Standing {sortBadge('standing')}</th>
                  <th onClick={() => toggleSort('score')} className="md:hidden px-4 py-3 text-[7px] font-black text-re-text-muted uppercase tracking-[0.2em] opacity-40 border-r border-black/5 text-center cursor-pointer">Score {sortBadge('score')}</th>
                  ...
                  <tr>
                    <td colSpan="5" className="p-12 text-center">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-[10px] font-black text-re-text-muted uppercase tracking-widest">Loading Discipline Reports...</p>
                      </div>
                    </td>
                  </tr>
                    ...
                    return (
                      <tr key={s.id} className="hover:bg-re-bg/60 even:bg-re-bg/20 transition-colors group cursor-default">
                        ...
                        <td className="hidden md:table-cell px-8 py-5 border-r border-black/5">
                          <p className="text-[10px] font-black text-[#1E3A5F]">{s.class_name || 'Unassigned'}</p>
                        </td>

                        <td className="hidden md:table-cell px-8 py-5 border-r border-black/5">
                          <div className="flex flex-col gap-1">
                            <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg ring-1 ring-inset text-[8px] font-black uppercase tracking-widest w-fit ${
                        ...
                        <td className="md:hidden px-4 py-3 text-center border-r border-black/5">
                          <div className="text-sm font-black text-[#1E3A5F]">{remaining.toFixed(0)}</div>
                        ...
                        <td className="px-4 sm:px-8 py-3 sm:py-5 text-right">
                          <button
                            ...
                            className="h-8 px-4 rounded-xl flex items-center justify-center gap-2 bg-white border border-black/5 text-re-text font-black text-[9px] uppercase tracking-widest hover:bg-re-bg hover:text-[#1E3A5F] transition-all ml-auto"
                          >
                            <Activity size={12} />
          ...
          <div className="flex px-4 sm:px-8 py-5 bg-re-bg/20 border-t border-black/5 flex-row items-center justify-between gap-4">
            <p className="text-[7px] sm:text-[8px] font-black text-re-text-muted uppercase tracking-[0.2em] opacity-40 italic whitespace-nowrap">
        ...
        <div className="fixed lg:hidden inset-0 z-[200] bg-black/50 flex flex-col justify-end backdrop-blur-sm">
          <div className="bg-white w-full rounded-t-3xl max-h-[80vh] flex flex-col">
            <div className="p-4 border-b border-black/5 flex items-center justify-between">
              ...
              <button onClick={() => setShowMobileClassModal(false)} className="w-8 h-8 flex items-center justify-center bg-re-bg border border-black/5 rounded-full text-[#000435] font-black text-xs">X</button>
            </div>
            <div className="overflow-y-auto p-4 flex flex-col gap-2">
              {uniqueClasses.map((cls) => (
                <button
                  ...
                  className={`p-4 rounded-xl text-[10px] font-black tracking-widest text-left border ${selectedClass === cls ? 'bg-[#1E3A5F] text-white border-[#1E3A5F]' : 'bg-white border-black/5 text-[#000435]'}`}
                >
                  {cls}
                </button>
              ))}
