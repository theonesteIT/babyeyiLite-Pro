...
const Field = ({ label, children }) => (
    <div className="space-y-1.5">
        <label className="text-[9px] font-black text-[#1E3A5F] uppercase tracking-widest ml-1">{label}</label>
        {children}
    </div>
);
    ...
    return (
        <div className="space-y-5">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-xl font-black text-[#1E3A5F] uppercase tracking-tighter">Attendance Events</h2>
                ...
                <button onClick={() => { setForm({ event_name:'', start_time:'07:00', end_time:'08:00', late_threshold:'07:15', target_group:'STUDENTS', residency_filter:'ALL' }); setModal(true); }}
                    className="h-9 px-4 rounded-xl flex items-center gap-1.5 text-white font-black text-[9px] uppercase tracking-widest shadow-md hover:scale-105 active:scale-95 transition-all"
            ...
            {loading ? (
                ...
                <div className="py-14 text-center rounded-2xl border border-dashed border-black/10 bg-slate-50/50">
                    ...
                    <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">No attendance events defined yet</p>
                        ...
                        <div key={ev.id} className="bg-white border border-black/5 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all group relative overflow-hidden">
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <div className="flex-1 min-w-0 pr-6">
                                    <h3 className="text-xs font-black text-[#1E3A5F] uppercase tracking-tight truncate">{ev.event_name}</h3>
                                    ...
                                    <div className="flex flex-wrap gap-1.5 mt-2.5">
                                        <span className="px-2 py-0.5 rounded bg-blue-50 text-blue-600 text-[8px] font-black uppercase tracking-widest">{ev.target_group}</span>
                                        <span className="px-2 py-0.5 rounded bg-slate-50 text-slate-500 text-[8px] font-black uppercase tracking-widest">{ev.residency_filter}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <Modal title="New Attendance Event" subtitle="Define a tracking window for gate scans" icon={Watch} onClose={() => setModal(false)}>
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="flex gap-3 pt-2">
                            <button type="button" onClick={() => setModal(false)} className="flex-1 h-11 rounded-2xl border border-black/5 text-[#1E3A5F] font-black text-[10px] uppercase tracking-widest hover:bg-slate-50 transition-all">Cancel</button>
                            <button type="submit" disabled={saving} className="flex-[1.5] h-11 rounded-2xl bg-[#1E3A5F] text-white font-black text-[10px] uppercase tracking-widest shadow-lg hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-2">
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h2 className="text-xl font-black text-[#1E3A5F] uppercase tracking-tighter">School Calendar</h2>
            ...
            <div className="flex flex-wrap gap-2">
                {EVENT_TYPES.map(t => (
                    <span key={t.value} className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border"
            ...
            <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
                <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
                    ...
                    <div className="text-center">
                        <p className="text-base font-black text-[#1E3A5F] uppercase tracking-wide">{MONTHS[month]} {year}</p>
                    ...
                    {cells.map((day, i) => {
                        ...
                        return (
                            ...
                            >
                                {day && (
                                    <>
                                        <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-[11px] font-black mb-1 ${todayFlag ? 'bg-[#1E3A5F] text-white' : 'text-slate-600'}`}>
                                            {day}
                                        </span>
                                        <div className="space-y-0.5">
                                            {evs.slice(0, 2).map(ev => (
                                                <div key={ev.id}
                                                    className="flex items-center gap-1 px-1 py-0.5 rounded text-[8px] font-black truncate group/ev"
                                            ...
                                            {evs.length > 2 && (
                                                <span className="text-[8px] font-black text-slate-400">+{evs.length - 2} more</span>
                                            )}
                                        </div>
                                    </>
                                )}
            ...
            {events.length > 0 && (
                <div>
                    <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">This Month — {events.length} Event{events.length !== 1 ? 's' : ''}</h3>
...
const Preferences = () => {
    ...
    return (
        <div className="space-y-5">
            <div>
                <h2 className="text-xl font-black text-[#1E3A5F] uppercase tracking-tighter">System Preferences</h2>
            ...
            <div className="bg-white border border-black/5 rounded-2xl p-5 shadow-sm space-y-4">
                <div className="flex items-center gap-2 mb-1">
                    ...
                    <h3 className="text-[10px] font-black uppercase tracking-widest text-[#1E3A5F]">Academic Calendar</h3>
                </div>
                    ...
                    <Field label="Active Terms">
                        <div className="flex gap-3 h-11 items-center pl-3">
                            {['Term 1','Term 2','Term 3'].map(term => (
                                <label key={term} className="inline-flex items-center gap-1.5 text-[11px] font-black cursor-pointer">
                ...
                {activeTerms.length > 0 && (
                    <div className="space-y-2 pt-1">
                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Term Start &amp; End Dates</p>
                        {activeTerms.map(term => {
                            ...
                            return (
                                <div key={term} className="flex flex-col sm:flex-row sm:items-center gap-2 p-3 bg-slate-50 rounded-xl border border-black/5">
                                    <span className="text-[10px] font-black text-[#1E3A5F] uppercase w-16 shrink-0">{term}</span>
                                    ...
                                    {cfg.start && cfg.end && <span className="px-2 py-1 bg-emerald-50 text-emerald-700 text-[8px] font-black rounded-lg border border-emerald-100 shrink-0">✓ Set</span>}
                                </div>
                            );
                        })}
                    </div>
                ...
                <button type="button" disabled={isSaving} onClick={handleSave}
                    className="h-10 px-4 rounded-xl bg-[#1E3A5F] text-white font-black text-[10px] uppercase tracking-widest inline-flex items-center gap-2 hover:scale-[1.02] active:scale-95 transition-all">
                ...
                {[
                    ...
                    <div key={i} className="flex items-center justify-between p-4 bg-white border border-black/5 rounded-2xl shadow-sm">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-[10px] font-black uppercase tracking-widest text-[#1E3A5F]">{p.label}</p>
        ...
        <div className="animate-in fade-in duration-700 bg-slate-50/60 min-h-screen">
            ...
            <div className="relative w-full overflow-hidden bg-[#c87800]">
                ...
                <div className="relative z-20 max-w-[1600px] mx-auto px-6 md:px-12 pt-10 pb-16 flex items-center gap-5">
                    ...
                    <div>
                        <p className="text-[9px] font-black uppercase tracking-[0.2em] mb-1" style={{ color:'#FEBF10' }}>Settings Dashboard</p>
                ...
                <div className="bg-white rounded-t-3xl shadow-sm border border-slate-100 overflow-hidden flex flex-col md:flex-row min-h-[500px]">
                    ...
                    <div className="w-full md:w-56 lg:w-60 bg-slate-50/60 border-r border-black/5 flex flex-col pt-5 shrink-0">
                        <div className="px-5 mb-4">
                            <p className="text-[9px] font-black uppercase tracking-[0.2em] text-[#1E3A5F]/50 flex items-center gap-1.5">
                            ...
                            {TABS.map(t => {
                                ...
                                return (
                                    <button key={t.id} onClick={() => setActiveTab(t.id)}
                                        className={`flex items-center gap-3 px-4 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${active ? 'bg-white shadow-sm text-[#1E3A5F] ring-1 ring-black/5' : 'text-slate-500 hover:text-[#1E3A5F] hover:bg-white/60'}`}>
                        ...
                        <div className="px-5 mt-auto pb-6 hidden md:block">
                            <div className="bg-white border border-black/5 p-3 rounded-2xl text-center">
                                <p className="text-[8px] font-black uppercase tracking-widest text-slate-400">System Status</p>
                                <p className="text-[10px] font-black uppercase tracking-tight text-emerald-500 mt-0.5">Online</p>
                            </div>
                        </div>
                    </div>
