...
export default function StockReports() {
  ...
  return (
    <div style={{ fontFamily:"'Montserrat',sans-serif", background:"#f4f6fb", minHeight:"100vh", color:"#0f172a" }}>
      <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@400;500;600;700;800;900&display=swap" rel="stylesheet" />

      {/* Orange hero header */}
      <div style={{ background:"#c87800", padding:"28px 24px 40px", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ position:"absolute", bottom:0, left:0, right:0, height:1, background:"linear-gradient(to right, transparent, rgba(254,191,16,.4), transparent)" }} />
        <div style={{ maxWidth:1400, margin:"0 auto", display:"flex", alignItems:"center", gap:16 }}>
          <div style={{ width:52, height:52, borderRadius:16, background:"rgba(255,255,255,.12)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
            <Package size={26} color="#fff" />
          </div>
          <div>
            <p style={{ fontSize:9, fontWeight:700, color:"rgba(255,255,255,.6)", textTransform:"uppercase", letterSpacing:"0.25em", marginBottom:2 }}>Inventory Management</p>
            <h1 style={{ fontSize:"clamp(20px,3vw,28px)", fontWeight:600, color:"#fff", letterSpacing:"-0.5px", margin:0 }}>Stock Reports</h1>
            <p style={{ fontSize:10, fontWeight:500, color:"rgba(255,255,255,.6)", textTransform:"uppercase", letterSpacing:"0.2em", margin:0 }}>Real-time inventory tracking &amp; movements</p>
            {storeMeta && (storeMeta.term || storeMeta.academic_year) && (
              <p style={{ fontSize:11, fontWeight:600, color:"rgba(255,255,255,.85)", margin:"8px 0 0", letterSpacing:"0.02em" }}>
                {storeMeta.term ? `${storeMeta.term}` : ''}{storeMeta.term && storeMeta.academic_year ? ' · ' : ''}{storeMeta.academic_year || ''}
                <span style={{ fontWeight:500, color:"rgba(255,255,255,.55)", marginLeft:8 }}>(same data as School Store)</span>
              </p>
            )}
          </div>
          <div style={{ marginLeft:"auto", display:"flex", gap:8, alignItems:"center" }}>
            <button onClick={() => setRefreshKey(k => k+1)} style={{ display:"flex", alignItems:"center", gap:6, padding:"7px 14px", borderRadius:8, border:"1px solid rgba(255,255,255,.25)", background:"rgba(255,255,255,.12)", color:"#fff", fontFamily:"'Montserrat',sans-serif", fontSize:11, fontWeight:700, cursor:"pointer" }}>
      ...
      <div style={{ background:"#fff", borderBottom:"1px solid #e2e8f0", position:"sticky", top:0, zIndex:50 }}>
        <div style={{ maxWidth:1400, margin:"0 auto", padding:"0 24px", display:"flex", gap:2, overflowX:"auto", scrollbarWidth:"none" }}>
          {NAV_TABS.map(({ id, label, Icon: I }) => (
            <button key={id} onClick={() => setTab(id)}
              style={{ display:"flex", alignItems:"center", gap:6, padding:"11px 14px", background:"none", border:"none", borderBottom: tab===id ? `2px solid ${ACCENT}` : "2px solid transparent", color: tab===id ? ACCENT : "#64748b", fontFamily:"'Montserrat',sans-serif", fontSize:11.5, fontWeight:700, cursor:"pointer", whiteSpace:"nowrap", transition:"all .15s" }}>
              <I size={13} /> {label}
            </button>
          ))}
        </div>
