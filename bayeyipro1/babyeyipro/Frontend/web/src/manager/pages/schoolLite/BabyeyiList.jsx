  ...
  return (
    <div className="relative" ref={ref}>
      <button type="button" onClick={() => setOpen(o => !o)}
        className={`flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl text-[11px] sm:text-[12px] font-black transition-all border ${
      ...
      {open && (
        <div className="absolute top-full mt-1.5 left-1/2 -translate-x-1/2 sm:left-auto sm:right-0 sm:translate-x-0 rounded-2xl shadow-2xl z-[90] w-[min(100vw-1rem,340px)] sm:w-[min(100vw-2rem,320px)] flex flex-col bg-[#000435] border border-amber-400/30 max-h-[min(72vh,460px)]">
          ...
          <div className="overflow-y-auto flex-1 min-h-0 py-1">
            ...
            {moreHint && autoFiltered.length > 0 && (
              <p className="px-4 pt-2 pb-1 text-[9px] font-black uppercase tracking-widest text-white/30 border-t border-white/10">{moreHint}</p>
            )}
            {autoFiltered.map((l) => (
              <button type="button" key={l.code} onClick={() => { setLang(l.code); setOpen(false); }}
...
function useMachineDocBody(lang, rec, T) {
  const pmBase = getParentMessageForDisplay(rec, lang, T);
  ...
  useEffect(() => {
    const pm0 = getParentMessageForDisplay(rec, lang, T);
    ...
    (async () => {
      try {
        const parentMsg = pm0.trim() ? await translateLongText(pm0, "en", lang) : "";
        const payments = await Promise.all(
          (rec.payments || []).map(async (p) => ({
            ...p,
            name: p.name ? await translateWithLingvaCached(String(p.name), "en", lang) : p.name,
          }))
        );
        const requirements = await Promise.all(
          (rec.requirements || []).map(async (r) => ({
            ...r,
            item: r.item ? await translateWithLingvaCached(String(r.item), "en", lang) : r.item,
            description: r.description ? await translateWithLingvaCached(String(r.description), "en", lang) : r.description,
          }))
        );
        const otherInfos = await Promise.all(
          (rec.otherInfos || []).map(async (n) => ({
            ...n,
            item: n.item ? await translateWithLingvaCached(String(n.item), "en", lang) : n.item,
            details: n.details ? await translateWithLingvaCached(String(n.details), "en", lang) : n.details,
          }))
        );
        ...
        const leaders = await Promise.all(
          (rec.leaders || []).map(async (l) => ({
            ...l,
            name: l.name ? await translateWithLingvaCached(String(l.name), "en", lang) : l.name,
            role: l.role ? await translateWithLingvaCached(String(l.role), "en", lang) : l.role,
          }))
        );
          ...
          br.map(async (bk) => ({
            ...
            bankName:
              bk.bankName && String(bk.bankName).trim() && bk.bankName !== "—"
                ? await translateWithLingvaCached(String(bk.bankName), "en", lang)
                : bk.bankName,
            accountName:
              bk.accountName && String(bk.accountName).trim() && bk.accountName !== "—"
                ? await translateWithLingvaCached(String(bk.accountName), "en", lang)
                : bk.accountName,
          }))
        );
        const merged = {
      ...
      <div className="w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl overflow-hidden shadow-2xl bg-[#000435] border-2 border-amber-400/30 flex flex-col max-h-[92vh]" style={{ fontFamily: FONT }}>
        <div className="px-5 py-4 border-b border-white/10 flex items-center justify-between shrink-0 bg-[#000435]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-black text-white text-[14px]">{T.shareDoc || "Share Document"}</p>
...
function KinyarwandaSectionEditModal({ target, recId, docId, onClose, onSaved, T }) {
  ...
  return (
    <div
      className="fixed inset-0 z-[70] flex items-center justify-center overflow-y-auto overscroll-contain p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-[max(0.75rem,env(safe-area-inset-top))] bg-black/70 backdrop-blur-sm min-h-[100dvh]"
      onClick={(e) => e.target === e.currentTarget && !saving && onClose()}
      role="dialog"
      aria-modal="true"
    >
      <div
        className="w-full max-w-lg mx-auto my-auto rounded-2xl border border-amber-400/30 bg-[#000435] shadow-2xl p-4 sm:p-5 max-h-[min(90dvh,720px)] overflow-y-auto shrink-0"
        style={{ fontFamily: FONT }}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-start justify-between gap-2 mb-3">
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-amber-400/80 mb-0.5">Kinyarwanda</p>
            <p className="text-white font-black text-[15px] leading-tight">{title}</p>
          </div>
          <button
            type="button"
        ...
        <div className="space-y-3">
          ...
          <div>
            <p className="text-[9px] font-black uppercase tracking-wider text-white/35 mb-1">{T.livePreviewLabel || "Live preview"}</p>
        ...
        <div className="flex gap-2 mt-4">
          ...
          <button
            ...
            onClick={handleSave}
            disabled={saving}
            className="flex-1 py-2.5 rounded-xl bg-amber-400 text-[#000435] text-[12px] font-black disabled:opacity-50"
          >
            {saving ? "…" : T.save || "Save"}
          </button>
        </div>
...
function OfficialDoc({
  ...
  const docBody = useMachineDocBody(lang, rec, T);
      ...
      <div className="w-full max-w-3xl my-4">
        ...
        <div className="bg-[#000435] border-2 border-amber-400/30 rounded-t-2xl px-3 sm:px-4 py-3 flex items-center gap-2 flex-wrap">
          ...
          <button onClick={handleRegen} disabled={regenerating}
            ...
            {regenerating ? <span className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : "↻"} {T.regen || "Regen"}
            </button>
          {!blocked ? (
          ...
          {!blocked ? (
            <button onClick={handlePDF} disabled={downloading}
              ...
              {downloading ? <span className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : "📄"} {T.pdfBtn || "PDF"} {lang !== "en" ? langMeta(lang).flag : ""}
            </button>
          ) : (
        ...
        <div className="relative bg-white shadow-2xl rounded-b-2xl overflow-hidden" style={{ fontFamily: "Georgia,'Times New Roman',serif" }}>
          {docBody.busy && !isCoreBabyeyiLang(lang) && (
            <div className="absolute inset-0 z-10 bg-white/75 backdrop-blur-[2px] flex items-center justify-center p-4">
              <div className="flex flex-col items-center gap-2 text-[#000435]">
                ...
                <p className="text-[11px] font-black text-center">{T.translating || "Translating document…"}</p>
              </div>
            </div>
          )}
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-2 gap-2 mb-3">
          <div className={`rounded-xl px-3 py-2.5 border ${rec.exceedsLimit ? "bg-red-500/10 border-red-500/25" : "bg-emerald-500/8 border-emerald-500/20"}`}>
            <p className="text-[9px] font-black uppercase tracking-wider text-white/35 mb-0.5">{T.cardTotalFee || "Total fee"}</p>
          ...
          <div className="rounded-xl px-3 py-2.5 border bg-white/4 border-white/10">
            <p className="text-[9px] font-black uppercase tracking-wider text-white/35 mb-0.5">{T.bankShort || "Bank"}</p>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="flex gap-2 shrink-0">
            <button onClick={() => setShowFilters(!showFilters)}
              ...
              ⚙ {T.filters || "Filters"} {activeFilters > 0 && <span className={`w-5 h-5 rounded-full text-[10px] font-black flex items-center justify-center ${showFilters || activeFilters>0?"bg-[#000435] text-amber-400":"bg-amber-400 text-[#000435]"}`}>{activeFilters}</span>}
            </button>
        ...
        {showFilters && (
          <div className="rounded-2xl bg-[#000435] border border-amber-400/20 p-4" style={{ fontFamily: FONT }}>
            <div className="flex items-center justify-between mb-3">
              <p className="text-[11px] font-black uppercase tracking-widest text-white/40">{T.filters || "Filters"}</p>
              {activeFilters > 0 && <button type="button" onClick={() => setFilters({ status:"", level:"", term:"", year:"" })} className="text-[11px] text-red-400 font-bold flex items-center gap-1">✕ {T.clearAll || "Clear all"}</button>}
            </div>
