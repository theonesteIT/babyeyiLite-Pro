...
const DashboardLayout = ({ children, title }) => {
  ...
  return (
    <div className="flex h-screen bg-re-bg font-sans overflow-hidden">
      {/* Sidebar — Desktop */}
      {!isChatFocused && <div className="hidden lg:block w-[304px] flex-shrink-0">
        <Sidebar />
      </div>}
      ...
      {!isChatFocused && <div
        className={`fixed inset-y-0 left-0 z-50 w-[304px] max-w-[88vw] transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] lg:hidden ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
