...
export default function GateScanner() {
  ...
  return (
    <div className="min-h-screen bg-[#F1F4FB]" style={{ fontFamily:"'Montserrat',sans-serif" }}>
      ...
      <div className="bg-white border-b border-slate-200">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl flex items-center justify-center shrink-0" style={{ background:NAVY }}>
              <ScanLine size={18} style={{ color:AMBER }} />
            </div>
            <div>
              <h1 className="text-base font-black tracking-tight" style={{ color:NAVY }}>Gate Scanner</h1>
              <p className="text-[11px] text-slate-400 font-medium uppercase tracking-wider">Student Exit &amp; Return Control</p>
            </div>
          </div>
          ...
          <div className="flex items-center bg-slate-100 rounded-2xl p-1 gap-1 self-start sm:self-auto">
            <button
              ...
              style={scanMode==="EXIT" ? { background:NAVY, color:AMBER, boxShadow:"0 2px 8px rgba(0,4,53,.25)" } : { color:"#64748B" }}>
              <LogOut size={13} /> Exit Mode
            </button>
            <button
              onClick={()=>{ setScanMode("RETURN"); if(cameraActive) startCamera("RETURN"); }}
              className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all"
              style={scanMode==="RETURN" ? { background:NAVY, color:AMBER, boxShadow:"0 2px 8px rgba(0,4,53,.25)" } : { color:"#64748B" }}>
      ...
      {/* ── 3-column grid ── */}
      <div className="max-w-[1400px] mx-auto p-4 sm:p-6 grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-4">
