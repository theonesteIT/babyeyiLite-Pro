...
const stepBadge = (n, active, done) => (
  <div
    className={`w-8 h-8 rounded-xl flex items-center justify-center text-[11px] font-black shrink-0 transition-all ${
      active ? 'bg-[#1E3A5F] text-white shadow-lg' : done ? 'bg-emerald-500 text-white' : 'bg-slate-200 text-slate-500'
    }`}
  >
...
const Borrowing = () => {
  ...
  const inputCls = (key) =>
    `w-full bg-re-bg border rounded-xl px-3 py-2.5 text-[11px] font-bold outline-none text-[#1E3A5F] shadow-[inset_0_2px_8px_rgba(15,23,42,0.06)] ${
      fieldErr(key) ? 'border-red-500 ring-1 ring-red-500/30' : 'border-black/5 focus:border-[#1E3A5F]/20'
    }`;
  ...
  return (
    <div className="animate-in fade-in duration-700 bg-re-bg min-h-screen" style={{ fontFamily: "'Montserrat', sans-serif" }}>
      <div className="relative w-full min-h-[220px] sm:min-h-[280px] overflow-hidden">
        ...
        <div className="relative z-20 max-w-[1600px] mx-auto px-4 sm:px-6 md:px-12 pt-12 sm:pt-16 pb-16 sm:pb-24 flex items-center gap-6">
          <div className="hidden sm:flex shrink-0 w-20 h-20 rounded-[32px] border border-white/10 bg-white/5 items-center justify-center backdrop-blur-xl shadow-2xl">
            <BookMarked size={40} style={{ color: '#FEBF10' }} />
          </div>
          <div className="min-w-0 space-y-1">
            <div className="flex items-center gap-2 mb-2">
              <span className="w-6 h-1 rounded-full animate-pulse shrink-0" style={{ background: '#FEBF10' }} />
              <p className="text-[10px] font-black uppercase tracking-[0.3em] truncate" style={{ color: '#FEBF10' }}>Circulation</p>
            </div>
            ...
            <p className="text-[9px] sm:text-xs font-bold text-white/40 uppercase tracking-widest italic opacity-60">
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 md:px-12 -mt-14 sm:-mt-24 relative z-20 pb-16 space-y-4">
        {/* Issue wizard card */}
          ...
          <div className="px-4 sm:px-8 py-4 sm:py-6 border-b border-black/5 flex flex-col sm:flex-row sm:items-center gap-4 justify-between bg-gradient-to-r from-[#1E3A5F]/5 to-transparent">
            <div className="flex items-center gap-3">
              ...
              <div className="ml-1 min-w-0">
                <p className="text-[9px] font-black uppercase tracking-[0.25em] text-slate-400">New loan</p>
                <p className="text-sm font-black text-[#1E3A5F] truncate">
            ...
            {step === 2 && (
              <button
                ...
                className="h-10 px-4 rounded-xl border border-black/5 text-[9px] font-black uppercase tracking-widest text-[#1E3A5F] hover:bg-re-bg flex items-center gap-2 self-start sm:self-auto"
              >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <button
                  ...
                  className={`flex items-center gap-4 p-4 rounded-2xl border-2 transition-all text-left ${
                    userKind === 'student' ? 'border-[#1E3A5F] bg-[#1E3A5F]/5 shadow-md' : 'border-black/5 hover:border-[#1E3A5F]/30'
                  }`}
                >
                  ...
                  <div>
                    ...
                    <p className="font-black text-[#1E3A5F]">Select Student</p>
                ...
                <button
                  ...
                  className={`flex items-center gap-4 p-4 rounded-2xl border-2 transition-all text-left ${
                    userKind === 'teacher' ? 'border-[#1E3A5F] bg-[#1E3A5F]/5 shadow-md' : 'border-black/5 hover:border-[#1E3A5F]/30'
                  }`}
                >
                  ...
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-widest text-amber-500">Step 1</p>
                    <p className="font-black text-[#1E3A5F]">Select Teacher / Staff</p>
              ...
              {userKind && (
                <div className="space-y-3">
                  ...
                  <div className="relative">
                    ...
                    />
                    {searching && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-black text-slate-400">…</span>}
                  </div>
                    ...
                    {searchResults.map((row) => (
                      ...
                      >
                        <div className="min-w-0">
                          <p className="font-black text-[#1E3A5F] text-[13px] truncate">{userKind === 'student' ? row.name : row.name}</p>
          ...
          {step === 2 && selected && (
            <div className="p-4 sm:p-8 space-y-5">
              <div className="flex items-start gap-3 p-4 rounded-2xl bg-re-bg/80 border border-black/5">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Borrower</p>
                  <p className="font-black text-[#1E3A5F] text-[15px] leading-tight">{borrowerLabel()}</p>
                  <p className="text-[11px] font-bold text-slate-500 mt-0.5">{borrowerSub()}</p>
                </div>
              </div>

              <div className="relative" ref={bookDdRef}>
                <label className="text-[9px] font-black uppercase tracking-widest text-slate-400 block mb-1">Book *</label>
                <button
                  type="button"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-[9px] font-black uppercase tracking-widest text-slate-400 block mb-1">Borrow date *</label>
                ...
                <div>
                  <label className="text-[9px] font-black uppercase tracking-widest text-slate-400 block mb-1">Return date *</label>
              ...
              <div>
                <label className="text-[9px] font-black uppercase tracking-widest text-slate-400 block mb-1">Notes</label>
                <textarea
                  rows={2}
                  value={form.notes}
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-2">
                <button
                  ...
                  disabled={issueBusy}
                  className="flex-1 h-11 rounded-xl text-white font-black text-[10px] uppercase tracking-widest shadow-lg disabled:opacity-50 flex items-center justify-center gap-2"
                  style={{ background: 'linear-gradient(135deg,#1E3A5F 0%,#0D2644 100%)' }}
                >
                  <Save size={14} /> {issueBusy ? 'Issuing…' : 'Issue book'}
                </button>
                <button type="button" onClick={resetFlow} className="h-11 px-6 rounded-xl border border-black/5 font-black text-[10px] uppercase text-slate-500 hover:bg-re-bg">
                  Cancel
                </button>
              </div>
            </div>
        ...
        <div className="bg-white rounded-[24px] sm:rounded-[32px] shadow-2xl border border-black/5 overflow-hidden flex flex-col min-h-[400px]">
          <div className="grid grid-cols-1 lg:grid-cols-4 border-b border-black/5">
            <div className="lg:col-span-3 grid grid-cols-2 md:grid-cols-4 divide-x divide-y md:divide-y-0 divide-black/5">
              {[
                ...
                <div key={i} className="p-4 sm:p-8 flex flex-col items-center justify-center text-center">
                  ...
                  <span className="text-lg sm:text-2xl font-black text-re-text">{s.value}</span>
                  <p className="text-[6px] sm:text-[8px] font-black text-re-text-muted uppercase tracking-[0.2em] mt-1 opacity-60">{s.label}</p>
                </div>
              ))}
            </div>
            <div className="flex flex-col border-t lg:border-t-0 lg:border-l border-black/5 bg-re-bg/30 p-4 sm:p-6 justify-center gap-2 sm:gap-3">
              <button type="button" onClick={exportPDF} className="w-full h-10 sm:h-11 flex items-center justify-center gap-2 text-white rounded-xl font-black text-[8px] sm:text-[9px] uppercase tracking-widest shadow-xl hover:scale-[1.02]" style={{ background: 'linear-gradient(135deg,#1E3A5F 0%,#0D2644 100%)' }}>
                <Printer size={14} /><span>Print loans</span>
              </button>
              <button type="button" onClick={() => { resetFlow(); setStep(1); }} className="w-full h-10 sm:h-11 flex items-center justify-center gap-2 bg-white border border-black/5 rounded-xl font-black text-[8px] sm:text-[9px] uppercase tracking-widest hover:bg-re-bg">
                <Plus size={14} className="text-amber-500" /><span>Issue book</span>
              </button>
              <button type="button" onClick={loadLoans} className="w-full h-9 flex items-center justify-center gap-2 text-[9px] font-black uppercase text-slate-500 border border-black/5 rounded-xl hover:bg-white">
          ...
          <div className="flex flex-col lg:flex-row flex-wrap px-3 py-2 border-b border-black/5 gap-2 bg-re-bg/20">
            <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)}
              className="h-9 w-full lg:w-[8rem] bg-white/80 rounded-lg border border-black/5 text-[9px] font-black uppercase tracking-widest px-2">
              {['All', 'Student', 'Staff'].map((t) => <option key={t}>{t}</option>)}
            </select>
            <button type="button" onClick={() => setOverdueOnly((p) => !p)}
              className={`h-9 px-3 rounded-lg text-[9px] font-black uppercase border w-full lg:w-auto ${overdueOnly ? 'bg-red-100 text-red-600 border-red-200' : 'bg-white border-black/5'}`}>
              Overdue only
            </button>
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input value={listSearch} onChange={(e) => setListSearch(e.target.value)} placeholder="Search borrower or book…"
                className="w-full h-9 pl-8 pr-3 rounded-lg border border-black/5 bg-white/80 text-[9px] font-black uppercase" />
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse min-w-[820px]">
              <thead>
                <tr className="bg-re-bg/20 border-b border-black/5">
                  <th onClick={() => toggleSort('borrower_name')} className="px-4 py-3 text-[8px] font-black text-re-text-muted uppercase tracking-[0.2em] opacity-40 border-r cursor-pointer">Borrower {sortBadge('borrower_name')}</th>
                  <th className="hidden lg:table-cell px-4 py-3 text-[8px] font-black text-re-text-muted uppercase opacity-40 border-r text-right">Load</th>
                  <th className="hidden md:table-cell px-4 py-3 text-[8px] font-black text-re-text-muted uppercase opacity-40 border-r">Book</th>
                  <th className="hidden sm:table-cell px-4 py-3 text-[8px] font-black text-re-text-muted uppercase opacity-40 border-r text-right">Qty</th>
                  <th className="hidden md:table-cell px-4 py-3 text-[8px] font-black text-re-text-muted uppercase opacity-40 border-r text-right">Issued</th>
                  <th onClick={() => toggleSort('due_date')} className="px-4 py-3 text-[8px] font-black text-re-text-muted uppercase opacity-40 border-r cursor-pointer text-right">Due {sortBadge('due_date')}</th>
                  <th className="px-4 py-3 text-[8px] font-black text-re-text-muted uppercase opacity-40 text-right">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-black/5">
                  ...
                  <tr key={l.id} className={`hover:bg-re-bg/60 ${l.overdue ? 'border-l-4 border-red-400' : ''}`}>
                    <td className="px-4 py-3 border-r">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="min-w-0">
                          <p className="text-[12px] font-black text-[#1E3A5F] truncate">{l.borrower_name}</p>
                          <p className="text-[8px] font-bold text-slate-400 uppercase truncate">{l.borrower_detail} · {l.user_type}</p>
                          <p className="lg:hidden text-[8px] font-black mt-0.5 uppercase">
                    ...
                    <td className="hidden lg:table-cell px-4 py-3 border-r text-right">
                      <span className={`text-[11px] font-black ${loadWarn ? 'text-red-500' : 'text-slate-600'}`}>{loadText}</span>
                      {loadWarn && <p className="text-[7px] font-black text-red-400 uppercase">At limit</p>}
                    </td>
                    ...
                    <td className="hidden sm:table-cell px-4 py-3 border-r text-right font-black text-[11px]">{l.quantity}</td>
                    <td className="hidden md:table-cell px-4 py-3 border-r text-right text-[11px] font-black">{l.borrow_date}</td>
                    <td className="px-4 py-3 border-r text-right">
                      <p className={`text-[12px] font-black ${l.overdue ? 'text-red-500' : 'text-slate-600'}`}>{l.return_date}</p>
                      {l.overdue && <p className="text-[8px] font-black text-red-400 uppercase">Overdue</p>}
                    </td>
                    <td className="px-4 py-3 text-right">
                      <button type="button" onClick={() => handleReturn(l.id)}
                        className="h-8 px-3 rounded-xl inline-flex items-center gap-1.5 bg-white border border-black/5 font-black text-[8px] uppercase tracking-widest hover:bg-re-bg">
                ...
                {!loansLoading && filtered.length === 0 && (
                  <tr><td colSpan={7} className="px-6 py-10 text-center text-[10px] font-black text-slate-400 uppercase">No active loans</td></tr>
                )}
              </tbody>
            </table>
          </div>
