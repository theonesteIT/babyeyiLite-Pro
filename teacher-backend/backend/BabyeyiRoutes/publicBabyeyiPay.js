...
function buildIntentFilters(q = {}) {
  ...
  if (channel === 'guest_finder') {
    whereParts.push(`JSON_CONTAINS(COALESCE(i.payload_json, '{}'), CAST('true' AS JSON), '$.public_pay_no_login') = 1`);
  } else if (channel === 'other') {
    whereParts.push(`NOT (JSON_CONTAINS(COALESCE(i.payload_json, '{}'), CAST('true' AS JSON), '$.public_pay_no_login') = 1)`);
  }
  if (district) {
...
router.post('/intent', async (req, res) => {
  try {
    ...
    if (shouldInitMtnMomo) {
      ...
      const payerMsg = `Babyeyi doc ${babyeyiId}`;
      try {
        const mtn = await mtnRequestToPay({
          amount,
          currency: 'RWF',
          externalId,
          msisdn250: msisdn,
          payerMessage: payerMsg,
          payeeNote: payerName ? `Payer: ${String(payerName).slice(0, 80)}` : 'Babyeyi payment',
        });
        provider = 'mtn_momo';
      ...
      const msisdn = toMsisdn250(payerPhone);
      const cnumber = normalizeRwandaPhone(payerPhone);
  ...
  try {
    ...
    const [rows] = await db.promisePool.execute(
      `SELECT i.id, i.school_id, i.babyeyi_id, i.total_rwf, i.status, i.created_at,
              ...
              CASE WHEN JSON_CONTAINS(COALESCE(i.payload_json, '{}'), CAST('true' AS JSON), '$.public_pay_no_login') = 1
...
router.get('/invoices', requireRole('SUPER_ADMIN', 'FULL_SYSTEM_CONTROLLER', 'SCHOOL_ADMIN', 'SCHOOL_MANAGER', 'ACCOUNTANT'), async (req, res) => {
  try {
    ...
    const [rows] = await db.promisePool.execute(
      `SELECT i.id, i.invoice_no, i.invoice_status, i.invoice_paid_at, i.invoice_sent_at, i.invoice_due_at, i.created_at,
              i.status, i.total_rwf, i.payer_name, i.payer_phone, i.payer_email,
              i.school_id, i.babyeyi_id, i.provider, i.provider_status, i.payload_json,
...
// POST /api/public/babyeyi-pay/invoices — manual school invoice (stored as babyeyi_payment_intents)
router.post('/invoices', requireRole('SUPER_ADMIN', 'FULL_SYSTEM_CONTROLLER', 'SCHOOL_ADMIN', 'SCHOOL_MANAGER', 'ACCOUNTANT'), async (req, res) => {
  try {
    ...
    const userSchoolId = Number(req.user?.school_id || 0);
    const schoolScoped = ['SCHOOL_ADMIN', 'SCHOOL_MANAGER', 'ACCOUNTANT'].includes(role);
    const schoolId = schoolScoped ? userSchoolId : Number(req.body?.school_id || 0);
    if (!schoolId) {
      return res.status(400).json({ success: false, message: 'School context is required to create an invoice' });
    }

    let babyeyiId = Number(req.body?.babyeyi_id || 0);
    if (!babyeyiId) {
      const [bRows] = await db.promisePool.execute(
        `SELECT id FROM school_babyeyi WHERE school_id=? AND is_active=1 AND status='approved' ORDER BY id DESC LIMIT 1`,
        [schoolId]
      );
      babyeyiId = Number(bRows?.[0]?.id || 0);
    }
    if (!babyeyiId) {
      return res.status(400).json({
        success: false,
        message: 'No approved Babyeyi fee card found for this school. Create or approve one before issuing invoices.',
      });
    }
    const [[b]] = await db.promisePool.execute(
      `SELECT id FROM school_babyeyi WHERE id=? AND school_id=? AND is_active=1 LIMIT 1`,
      [babyeyiId, schoolId]
    );
    if (!b) {
      return res.status(404).json({ success: false, message: 'Babyeyi card not found for this school' });
    }

    const items = Array.isArray(req.body?.items) ? req.body.items : [];
    const taxRate = Math.max(0, Math.min(1, Number(req.body?.tax_rate ?? req.body?.taxRate ?? 0) || 0));
    ...
    const total_rwf = Math.round((subTotal + tax) * 100) / 100;
    if (!Number.isFinite(total_rwf) || total_rwf < 0) {
      return res.status(400).json({ success: false, message: 'Invalid line items or total' });
    }

    const billTo = req.body?.bill_to || req.body?.billTo || {};
    const studentName = String(billTo?.name || req.body?.student_name || 'Student').trim().slice(0, 180) || 'Student';
    const studentUid = String(billTo?.uid || billTo?.student_uid || req.body?.student_uid || '').trim().slice(0, 80);
    const className = String(billTo?.class || billTo?.class_name || req.body?.class_name || '').trim().slice(0, 120);
    const payerName = String(req.body?.payer_name || studentName).trim().slice(0, 180) || studentName;
    const payerPhone = String(req.body?.payer_phone || '').trim().slice(0, 40) || null;
    const payerEmail = String(req.body?.payer_email || '').trim().slice(0, 180) || null;
    const notes = String(req.body?.notes || '').trim().slice(0, 2000);

    const dueRaw = String(req.body?.due_date || req.body?.dueDate || '').trim();
    let invoiceDueAt = null;
    if (dueRaw) {
      const d = new Date(`${dueRaw}T23:59:59`);
      invoiceDueAt = Number.isNaN(d.getTime()) ? null : d;
    }
    if (!invoiceDueAt) {
      const dueDays = Math.max(1, Number(process.env.INVOICE_DUE_DAYS || 14));
      invoiceDueAt = new Date(Date.now() + dueDays * 24 * 60 * 60 * 1000);
    }
    ...
    const payload = {
      ...
      selected_students: [],
      payer: req.body?.payer || { name: payerName, phone: payerPhone, email: payerEmail },
    };
    ...
    const invoiceNo = makeInvoiceNo(insertId);
    await db.promisePool.execute(`UPDATE babyeyi_payment_intents SET invoice_no = ? WHERE id = ?`, [invoiceNo, insertId]);
    notifyInvoiceStatusByIntentId(insertId, 'accountant_portal_create').catch((e) => {
      console.warn('[invoice/email] accountant portal create notify failed:', e.message);
    });

    return res.status(201).json({
      success: true,
      message: 'Invoice created',
      data: { id: insertId, invoice_no: invoiceNo, invoice_status: invoiceStatus, total_rwf },
    });
  } catch (err) {
...
// PATCH /api/public/babyeyi-pay/invoices/:id — mark sent / mark paid (school-scoped)
router.patch('/invoices/:id', requireRole('SUPER_ADMIN', 'FULL_SYSTEM_CONTROLLER', 'SCHOOL_ADMIN', 'SCHOOL_MANAGER', 'ACCOUNTANT'), async (req, res) => {
  try {
    ...
    if (!['mark_sent', 'mark_paid'].includes(action)) {
      return res.status(400).json({ success: false, message: 'Use action mark_sent or mark_paid' });
    }
    ...
    const [rows] = await db.promisePool.execute(
      `SELECT id, school_id, invoice_status, status FROM babyeyi_payment_intents WHERE id = ? LIMIT 1`,
      [id]
    );
    const row = rows?.[0];
    ...
    if (action === 'mark_sent') {
      await db.promisePool.execute(
        `UPDATE babyeyi_payment_intents SET invoice_sent_at = NOW() WHERE id = ?`,
        [id]
      );
      notifyInvoiceStatusByIntentId(id, 'accountant_mark_sent').catch((e) => {
        console.warn('[invoice/email] mark sent notify failed:', e.message);
      });
    ...
    notifyInvoiceStatusByIntentId(id, 'accountant_mark_paid').catch((e) => {
      console.warn('[invoice/email] mark paid notify failed:', e.message);
    });
    return res.json({ success: true, message: 'Invoice marked as paid' });
  } catch (err) {
    console.error('[public/babyeyi-pay/invoices PATCH]', err);
