  ...
  try {
    ...
    if (isSchoolManager) {
      try {
        ...
        const isSchoolApproved = activeByStatus;
...
router.get('/verify', (req, res) => {
  // IMPORTANT:
  // Frontends often poll this endpoint on boot. Returning 401 spams the console and
  // can be misread as "route not found". Instead, we return 200 with success=false.
  if (!req.session?.userId) {
    return res.json({ success: false, authenticated: false, data: null, message: 'Not authenticated' });
  }
  return res.json({
    success: true,
    authenticated: true,
...
// Alias commonly used by frontends
router.get('/me', (req, res) => {
  ...
  return res.json({
    success: true,
    authenticated: true,
    data: req.session.user || { id: req.session.userId, role: req.session.roleCode },
  });
});
