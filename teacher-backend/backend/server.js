    ...
    if (rows.length > 0 && rows[0].is_active) {
      ...
      if (isSchoolManager && !req.user.school_id) {
        ...
        if (schoolRows && schoolRows.length > 0 && schoolRows[0].id) {
          ...
          req.user.school = { id: s.id, name: s.school_name, code: s.school_code, email: s.email || null, phone: s.phone || null, district: s.district || null, province: s.province || null };
...
app.get('/api/session/me', async (req, res) => {
  try {
    if (req.session?.user) {
      ...
      // Role may be { code, name }, a legacy string, or only on req.session.roleCode — normalize for clients
      let rawRole = '';
      if (typeof data.role === 'string' && data.role.trim()) rawRole = data.role.trim();
      else if (data.role && typeof data.role === 'object' && data.role.code != null) rawRole = String(data.role.code).trim();
      else if (data.role_code) rawRole = String(data.role_code).trim();
      else if (req.session?.roleCode) rawRole = String(req.session.roleCode).trim();
      const roleCode = rawRole.toUpperCase();
      if (roleCode) data.role_code = roleCode;
      const isSchoolManager = roleCode === 'SCHOOL_ADMIN' || roleCode === 'SCHOOL_MANAGER';
...
const storekeeperRoutes = require('./BabyeyiRoutes/storekeeper');
console.log('  ✅  storekeeper.js');
const shuleAvanceMockRoutes = require('./BabyeyiRoutes/shuleAvanceMock');
console.log('  ✅  shuleAvanceMock.js');
...
app.use('/api', storekeeperRoutes);
console.log('  ✅  /api/store/*');
app.use('/api', shuleAvanceMockRoutes);
console.log('  ✅  /api/services/shule-avance/* (demo mock)');
...
const startServer = async () => {
  try {
    ...
    console.log('✅  Database connected (Raw Pool)');
...
const { initAttendanceScheduler } = require('./utils/attendanceScheduler');
