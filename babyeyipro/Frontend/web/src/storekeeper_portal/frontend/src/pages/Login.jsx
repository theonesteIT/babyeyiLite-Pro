...
import { Mail, Lock, Eye, EyeOff, AlertCircle, RefreshCw, Shield, ArrowLeft } from 'lucide-react';
...
const Login = () => {
  ...
  if (authLoading) return (
    <div className="min-h-screen bg-re-bg flex items-center justify-center">
      <RefreshCw className="animate-spin text-re-navy w-8 h-8 opacity-50" />
    </div>
  );
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2.5 bg-re-bg border border-black/5 rounded-xl text-[11px] font-bold outline-none focus:ring-2 focus:ring-re-navy/20 transition-all"
                />
              </div>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-10 py-2.5 bg-re-bg border border-black/5 rounded-xl text-[11px] font-bold outline-none focus:ring-2 focus:ring-re-navy/20 transition-all"
                />
                <button type="button" onClick={() => setShowPassword(s => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-re-text-muted/40 hover:text-re-navy transition-colors">
