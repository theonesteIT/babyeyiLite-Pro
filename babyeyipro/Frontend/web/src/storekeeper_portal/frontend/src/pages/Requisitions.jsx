...
const RequisitionDrawer = ({ req, onClose, onIssue, onReject }) => {
  ...
  return createPortal(
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-[220] w-full md:w-[440px] bg-white shadow-md border-l border-black/10 flex flex-col animate-in slide-in-from-right duration-300">
        <div className="flex items-center justify-between px-7 py-5 border-b border-black/5 shrink-0">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-slate-50 border border-black/5 flex items-center justify-center font-semibold text-lg text-re-navy">
              {req.dept?.charAt(0) || 'R'}
            </div>
            <div>
              <h3 className="font-semibold text-re-navy text-sm uppercase tracking-tight">{req.dept}</h3>
        ...
        <div className="flex-1 overflow-y-auto px-7 py-6 space-y-6">
          {[
            ...
            <div key={f.label} className="flex items-start justify-between gap-4 group">
              <span className="text-[9px] font-semibold uppercase tracking-widest text-slate-400 mt-0.5 shrink-0">{f.label}</span>
              <div className="flex-1 border-b border-dashed border-black/10" />
              <span className="text-[11px] font-semibold text-re-navy text-right max-w-[200px]">{f.value}</span>
            </div>
          ))}

          <div className={`inline-flex items-center px-3 py-1.5 rounded-xl text-[10px] font-semibold uppercase tracking-wider border ${STATUS_COLORS[req.status] || 'bg-slate-100 text-slate-500 border-slate-200'}`}>
            {req.status}
          </div>
        </div>
        ...
        {req.status === 'pending' && (
          <div className="px-7 py-5 border-t border-black/5 flex gap-3 shrink-0">
            <button
              ...
              className="flex-1 py-2.5 rounded-xl border border-red-200 text-red-500 font-semibold text-[10px] uppercase tracking-widest hover:bg-red-50 transition-all flex items-center justify-center gap-2"
            >
              <XCircle size={13} /> Reject
            </button>
            <button
              onClick={() => { onIssue(req); onClose(); }}
              className="flex-1 py-2.5 rounded-xl text-white font-semibold text-[10px] uppercase tracking-widest transition-all flex items-center justify-center gap-2 border border-black/15 shadow-sm"
        ...
        {req.status === 'approved' && (
          <div className="px-7 py-5 border-t border-black/5 shrink-0">
            <button
              onClick={() => { onIssue(req); onClose(); }}
              className="w-full py-2.5 rounded-xl text-white font-semibold text-[10px] uppercase tracking-widest flex items-center justify-center gap-2"
              style={{ background: 'linear-gradient(135deg,#1E3A5F,#0D2644)' }}
            >
        ...
        toolbar={(
          <>
            <div className="relative w-full sm:w-[12rem] shrink-0">
              <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[8px] font-semibold uppercase text-slate-400 tracking-[0.2em] z-[1] pointer-events-none">Status</span>
              <select
                value={status}
      ...
      >
        {loading ? (
          <div className="flex items-center justify-center py-20 gap-3">
            ...
            <p className="text-[11px] font-semibold uppercase tracking-widest text-slate-400">Loading requisitions…</p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 gap-2">
            <ClipboardList size={36} className="text-slate-200" />
            <p className="text-[11px] font-semibold uppercase tracking-widest text-slate-400">No requisitions match your filters</p>
          </div>
        ) : (
              ...
              {filtered.map((req) => (
                ...
                >
                  <td className="px-4 sm:px-6 py-3 border-r border-black/5">
                    <p className="font-semibold text-[#1E3A5F] text-[11px]">{req.id}</p>
                  ...
                  <td className="px-4 sm:px-6 py-3 border-r border-black/5 text-[10px] font-semibold text-[#1E3A5F]">
                    <span className="inline-flex items-center gap-2">
                      <span className="w-8 h-8 rounded-lg bg-re-bg border border-black/5 flex items-center justify-center font-semibold text-[11px] shrink-0 group-hover:bg-[#1E3A5F] group-hover:text-white transition-colors">
                        {req.dept?.charAt(0) || 'R'}
                      </span>
