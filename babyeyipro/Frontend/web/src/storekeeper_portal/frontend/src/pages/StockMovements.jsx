    ...
    <div className="fixed inset-0 z-[220] flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm p-0 sm:p-4">
      <div className="bg-white w-full sm:max-w-md rounded-t-[32px] sm:rounded-[28px] shadow-sm flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between px-6 py-5 border-b border-black/5 shrink-0">
          <div>
            <p className="text-[9px] font-semibold uppercase tracking-[0.3em] text-re-navy">Record movement</p>
            <h3 className="font-semibold text-re-navy text-base mt-0.5">New stock in / stock out</h3>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-slate-50 rounded-xl text-slate-400"><X size={18} /></button>
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-3">
          <div>
            <label className="text-[9px] font-semibold uppercase tracking-widest text-slate-400 block mb-1">Item *</label>
          ...
          <div>
            <label className="text-[9px] font-semibold uppercase tracking-widest text-slate-400 block mb-1">Type *</label>
            <div className="grid grid-cols-2 gap-2">
              {MOVEMENT_TYPES.map(t => (
                <button key={t} type="button" onClick={() => set('type', t)}
                  className={`py-2 rounded-xl text-[10px] font-semibold uppercase tracking-widest transition-all border ${form.type === t ? 'border-re-navy/40 bg-re-navy/5 text-re-navy' : 'border-black/5 bg-re-bg text-slate-400 hover:border-re-navy/20'}`}>
...
export default function StockMovements() {
  ...
  const typeChip = (type) => {
    ...
    return <span className={`text-[8px] font-semibold px-2 py-0.5 rounded-full uppercase ${cfg[type] || 'bg-slate-100 text-slate-500'}`}>{MOVEMENT_LABELS[type] || type}</span>;
  };
        ...
        toolbar={(
          <>
            ...
            <input
              ...
              className="h-9 md:h-8 w-full sm:w-[10.5rem] bg-white/80 rounded-lg outline-none border border-black/5 focus:border-[#1E3A5F]/20 focus:bg-white transition-all text-[#1E3A5F] text-[9px] font-semibold uppercase tracking-tight shadow-[inset_0_2px_8px_rgba(15,23,42,0.06)] px-3"
            />
      ...
      >
        {loading ? (
          <div className="flex items-center justify-center py-20 gap-3">
            ...
            <p className="text-[11px] font-semibold uppercase tracking-widest text-slate-400">Loading movements…</p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 gap-2">
            <ArrowDown size={36} className="text-slate-200" />
            <p className="text-[11px] font-semibold uppercase tracking-widest text-slate-400">No movements match your filters</p>
          </div>
        ) : (
            ...
            <tbody className="divide-y divide-black/5">
              {filtered.map((m, i) => (
                <tr key={m.id || i} className="hover:bg-re-bg/60 even:bg-re-bg/20 transition-colors">
                  ...
                  <td className="px-4 sm:px-6 py-3 border-r border-black/5 font-semibold text-slate-700 text-[11px]">{m.stock_after}</td>
