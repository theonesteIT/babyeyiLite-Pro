...
const ExpandableNavItem = ({ icon: Icon, name, subItems, onClose }) => {
  ...
  const pathMatches = p => location.pathname === p;
          ...
          {subItems.map(sub => (
            <NavLink key={sub.path} to={sub.path} onClick={onClose}
              className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-bold transition-all
                ${pathMatches(sub.path) ? 'text-re-navy bg-re-navy/10' : 'text-re-text-muted hover:text-re-navy hover:bg-re-navy/5'}`}>
              <sub.icon size={11} className={pathMatches(sub.path) ? 'text-re-navy' : 'text-re-text-muted/40'} />
              {sub.name}
            </NavLink>
          ))}
        </div>
        ...
        <div className="rounded-2xl bg-re-bg shadow-inner p-3 space-y-1">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[9px] text-re-navy font-black uppercase tracking-[0.2em] opacity-60 mt-0.5">Staff Portal</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="rounded-2xl border border-black/5 bg-re-bg shadow-inner p-2 space-y-2">
          ...
          <div className="flex items-center gap-2.5 px-2 py-1.5 bg-white rounded-xl border border-black/5 shadow-sm">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-black truncate text-re-navy uppercase tracking-tight">{staff?.first_name || 'Librarian'}</p>
              <p className="text-[9px] text-re-text-muted truncate font-bold uppercase tracking-wider opacity-50 mt-0.5">{staff?.role_name || 'Librarian'}</p>
            </div>
