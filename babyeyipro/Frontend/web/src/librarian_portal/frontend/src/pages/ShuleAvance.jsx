...
const ShuleAvance = () => {
   ...
   if (loading) return (
      <div className="flex h-screen items-center justify-center">
         <div className="w-10 h-10 border-4 border-[#FEBF10]/20 border-t-[#FEBF10] rounded-full animate-spin"></div>
      </div>
   );
            ...
            <div className="relative z-10 max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-12 pt-10 sm:pt-12 pb-20 sm:pb-24 flex items-center justify-between">
               <div className="space-y-1 max-w-3xl">
                  <div className="flex items-center gap-2 mb-1">
                     ...
                     <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#FEBF10]">Staff services</p>
                  </div>
                  ...
                  <p className="text-[10px] sm:text-[11px] font-medium text-white/60 tracking-wider">
                     Advance salary and staff services
                  </p>
               </div>
            </div>
         </div>
                           ...
                           <form onSubmit={handleApply} className="grid grid-cols-1 md:grid-cols-2 gap-5">
                              ...
                              <div className="md:col-span-2 flex justify-between items-center bg-re-bg/50 p-4 rounded-lg border border-dashed border-gray-200">
                                 ...
                                 <button
                                    type="submit"
                                    disabled={submitting}
                                    className="text-white px-6 py-2.5 rounded-lg font-semibold hover:scale-[1.02] active:scale-95 transition-all text-[10px] uppercase tracking-widest disabled:opacity-50"
                              ...
                              <div className="flex flex-col md:flex-row items-center justify-between gap-8 py-2">
                                 <div className="space-y-4 max-w-sm">
                                    ...
                                    <button
                                       ...
                                       className="text-white px-8 py-3.5 rounded-xl font-semibold hover:scale-[1.02] active:scale-95 transition-all text-[10px] uppercase tracking-widest inline-flex items-center gap-3"
                                       style={{ background: "linear-gradient(135deg, #1E3A5F 0%, #0D2644 100%)", boxShadow: "0 12px 30px rgba(30,58,95,0.22)" }}
                                    >
                           ...
                           {loanStatus?.history?.length > 0 ? loanStatus.history.map((loan) => (
                              <div key={loan.id} className="px-5 py-3.5 flex items-center justify-between group hover:bg-re-bg/30 transition-all cursor-default">
                                 ...
                                 <div className="text-right">
                                    ...
                                    <span className={`text-[7px] font-semibold uppercase tracking-widest px-2 py-0.5 rounded-full inline-block mt-0.5 ${loan.status === 'completed' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-amber-50 text-amber-600 border border-amber-100'}`}>
                                       {loan.status}
                                    </span>
                                 </div>
                              </div>
