      ...
      <div className="relative w-full min-h-[200px] sm:min-h-[220px] overflow-hidden bg-[#c87800]">
        ...
        <div className="relative z-10 max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-12 pt-10 sm:pt-12 pb-20 sm:pb-24 flex items-center justify-between">
          <div className="space-y-1 max-w-3xl">
            <div className="flex items-center gap-2 mb-1 text-center md:text-left">
              <span className="w-5 h-1 rounded-full bg-[#FEBF10]" aria-hidden />
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#FEBF10]">Library Portal</p>
            </div>
            <h1 className="text-xl md:text-2xl font-semibold text-white tracking-tight leading-none mb-1 mt-1 uppercase text-center md:text-left">
              {feature} System
            </h1>
            <p className="text-[10px] sm:text-[11px] font-medium text-white/60 tracking-wider text-center md:text-left">
              {PORTAL.brandLine} — module in progress
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-[24px] shadow-2xl border border-black/5 overflow-hidden flex flex-col items-center p-8 md:p-12 text-center">
            ...
            <div className="space-y-2 mb-10 max-w-xl">
              <h2 className="text-xl md:text-2xl font-semibold text-re-text tracking-tight uppercase leading-tight">Coming soon</h2>
              <p className="text-[11px] md:text-xs font-bold text-re-text-muted opacity-80 tracking-wide leading-relaxed">
                The <span className="text-re-purple font-semibold">{feature}</span> area is not wired up for the conduct portal yet. Core tools live on the dashboard, students, attendance, and conduct overview.
              </p>
            </div>
            ...
            <div className="w-full max-w-md bg-re-bg rounded-2xl p-6 border border-black/5 space-y-4 mb-10">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-[8px] font-semibold text-re-text-muted uppercase tracking-[0.2em]">Deployment Readiness</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 w-full max-w-2xl mb-12">
              {[
                ...
                <div key={i} className="flex items-center gap-3 p-3.5 rounded-xl bg-re-bg/30 border border-black/5 text-left group hover:bg-white hover:border-re-purple/10 transition-all cursor-default">
                  ...
                  <div>
                    <p className="text-[9px] font-semibold text-re-text uppercase tracking-tight">{item.label}</p>
                    <p className="text-[7px] font-semibold text-re-text-muted uppercase opacity-40 tracking-wider leading-none mt-0.5">{item.detail}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-col md:flex-row items-center gap-4 w-full justify-center">
              <Link
                to="/"
                className="flex items-center gap-2 px-10 py-4 bg-re-grad-purple rounded-xl text-white font-semibold text-[10px] uppercase tracking-widest shadow-re-premium-purple hover:scale-[1.02] active:scale-95 transition-all"
              >
                <ArrowLeft size={14} />
                Back to Dashboard
              </Link>
              <button className="flex items-center gap-2 text-[9px] font-semibold text-re-text-muted hover:text-re-purple uppercase tracking-widest transition-colors">
                Read Roadmap
          ...
          <p className="text-center text-[8px] text-re-text-muted mt-8 font-semibold uppercase tracking-[0.3em] opacity-30 italic">Developed & Engineered by Babyeyi Intelligence Systems</p>
        </div>
      </div>
    </div>
  );
