  ...
  if (!draft) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 font-sans" style={{ fontFamily: "'Montserrat', sans-serif" }}>
        <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl border border-slate-100 p-8 text-center space-y-4">
          ...
          <h2 className="text-xl font-black text-[#000435]">No Draft Found</h2>
          ...
          <button
            ...
            className="mt-6 w-full h-12 rounded-xl bg-[#000435] text-[#FEBF10] text-[11px] font-black uppercase tracking-wider flex items-center justify-center gap-2 hover:bg-[#000435]/90 transition-all shadow-lg shadow-[#000435]/20"
          >
    ...
    <div className="min-h-screen bg-slate-50 pb-20" style={{ fontFamily: "'Montserrat', sans-serif" }}>
      ...
      <div className="relative bg-[#000435] overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-8 pt-10 pb-24 sm:pb-32">
          <button
            onClick={() => navigate(-1)}
            className="mb-6 h-9 px-3 rounded-lg bg-white/5 border border-white/10 text-white/70 text-[10px] font-black uppercase tracking-wider inline-flex items-center gap-2 hover:bg-white/10 hover:text-white transition-all"
          >
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            ...
            <div>
              <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.3em] text-[#FEBF10]/80 mb-1">FINANCE · DISBURSEMENT</p>
              ...
              <p className="text-[10px] sm:text-xs text-white/40 mt-2 font-bold uppercase tracking-widest max-w-md leading-relaxed">
      ...
      {/* ── Main Content ── */}
      <div className="relative max-w-4xl mx-auto px-4 sm:px-8 -mt-16 sm:-mt-20 z-10 space-y-6">
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-slate-100 p-6 sm:p-8">
          <div className="flex items-center gap-2 mb-6 border-b border-slate-100 pb-4">
            ...
            <h2 className="text-[13px] font-black text-[#000435] uppercase tracking-wider">Payroll Summary</h2>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5"><User size={10} /> STAFF</p>
              <p className="text-[13px] font-black text-[#000435] truncate">{draft.staffName}</p>
            </div>
            <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">STAFF CODE</p>
              <p className="text-[13px] font-black text-[#000435] truncate">{draft.staffCode}</p>
            </div>
            <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">PERIOD</p>
              <p className="text-[11px] font-black text-[#000435]">{draft.month} · {draft.term} · {draft.year}</p>
            </div>
            <div className="bg-emerald-50 rounded-2xl p-4 border border-emerald-100/50 relative overflow-hidden">
              ...
              <p className="text-[9px] font-black text-emerald-600/60 uppercase tracking-widest mb-1.5 relative z-10">AMOUNT TO PAY</p>
              <p className="text-[16px] xl:text-[18px] font-black text-emerald-600 tabular-nums relative z-10">{fmt(draft.amount)} <span className="text-[10px]">RWF</span></p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-slate-100 p-6 sm:p-8">
          <div className="flex items-center gap-2 mb-6 border-b border-slate-100 pb-4">
            ...
            <h2 className="text-[13px] font-black text-[#000435] uppercase tracking-wider">Payment Configuration</h2>
          </div>

          {/* Method Selection */}
          <div className="mb-8">
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">SELECT DISBURSEMENT METHOD</label>
          ...
          <div className="max-w-2xl bg-slate-50 rounded-2xl p-5 sm:p-6 border border-slate-100">
            {paymentMethod === 'MTN' && (
              <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                <label className="block text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2">MTN PHONE NUMBER</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="h-12 w-full rounded-xl border border-slate-200 bg-white px-11 text-[13px] font-black text-[#000435] outline-none focus:border-[#000435]/40 focus:bg-white transition-all placeholder:text-slate-300/80 hover:border-slate-300 shadow-sm"
                    placeholder="e.g. 078XXXXXXX"
                    inputMode="tel"
                  />
                </div>
                <p className="mt-2 text-[9px] font-bold text-slate-400 uppercase tracking-widest">Verify the mobile money number before proceeding.</p>
              </div>
            )}
          ...
          {error && (
            <div className="mt-6 rounded-xl border border-red-200 bg-red-50 p-4 animate-in fade-in zoom-in-95 shadow-sm">
              <p className="text-[10px] font-black text-red-600 flex items-center gap-2 uppercase tracking-wider">
          ...
          <div className="mt-8 pt-6 border-t border-slate-100 flex flex-col-reverse sm:flex-row justify-between items-center gap-4">
            <button
              onClick={() => navigate(-1)}
              disabled={loading}
              className="w-full sm:w-auto h-12 px-6 rounded-xl border border-slate-200 bg-white text-slate-600 text-[11px] font-black uppercase tracking-widest hover:bg-slate-50 hover:text-slate-800 transition-all disabled:opacity-50"
            >
              Cancel Payment
            </button>
            <button
              onClick={handlePay}
              disabled={loading}
              className="w-full sm:w-auto h-12 px-10 rounded-xl bg-[#000435] text-[#FEBF10] text-[11px] font-black uppercase tracking-widest inline-flex items-center justify-center gap-2.5 hover:bg-[#000435]/90 active:scale-95 transition-all shadow-xl shadow-[#000435]/30 disabled:opacity-50 disabled:pointer-events-none"
            >
              {loading ? (
                <>
