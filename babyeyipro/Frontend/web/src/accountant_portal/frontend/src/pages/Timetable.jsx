...
export default function Timetable() {
    ...
    return (
        <div className="animate-in fade-in duration-700 bg-re-bg min-h-screen pb-12">
                ...
                <div className="relative z-20 max-w-[1600px] mx-auto px-6 md:px-12 pt-16 pb-24">
                    <div className="space-y-1">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-[10px] font-medium uppercase tracking-[0.2em] text-white/80">School schedule</span>
                        </div>
            ...
            {/* ── Main Content Area ── */}
            <div className="acct-shell-timetable px-4 md:px-12">
                <div className="acct-panel-sheet overflow-hidden flex flex-col">
                    ...
                    <div className="px-6 py-5 border-b border-black/5 flex flex-col xl:flex-row items-center justify-between gap-4 bg-white md:bg-re-bg/20">
                        ...
                        <div className="flex flex-col sm:flex-row items-center gap-4 w-full xl:w-auto">
                            ...
                            <div className="flex bg-white p-1 rounded-xl shadow-sm border border-black/5 w-full sm:w-auto overflow-x-auto custom-scrollbar">
                                <button
                                    ...
                                    className={`flex-1 sm:flex-none flex items-center justify-center gap-2 h-8 sm:h-10 px-3 sm:px-6 rounded-lg text-[8px] sm:text-[10px] font-medium uppercase tracking-widest transition-all ${view === 'grid' ? 'bg-re-bg text-re-orange border border-re-orange/10' : 'text-re-text-muted hover:text-re-text hover:bg-re-bg/50'}`}
                                >
                                    <Calendar size={12} className="sm:w-3.5 sm:h-3.5" /> Weekly Grid
                                </button>
                                <button
                                    onClick={() => setView('list')}
                                    className={`flex-1 sm:flex-none flex items-center justify-center gap-2 h-8 sm:h-10 px-3 sm:px-6 rounded-lg text-[8px] sm:text-[10px] font-medium uppercase tracking-widest transition-all ${view === 'list' ? 'bg-re-bg text-re-orange border border-re-orange/10' : 'text-re-text-muted hover:text-re-text hover:bg-re-bg/50'}`}
                                >
                            ...
                            <div className={`w-full sm:w-auto flex items-center gap-1.5 sm:gap-2 ${view === 'grid' ? 'hidden sm:flex' : 'flex'}`}>
                                ...
                                <div className="flex-1 relative">
                                    <select
                                        ...
                                        className="w-full h-8 sm:h-10 bg-white border border-black/5 rounded-lg sm:rounded-xl text-[8px] sm:text-xs font-medium uppercase tracking-widest text-re-text text-center focus:outline-none focus:border-re-orange/30 appearance-none px-2"
                                    >
                        ...
                        <div className="hidden xl:flex flex-1 min-w-0 max-w-xl items-center">
                            <p className="text-[9px] md:text-[10px] font-medium text-re-text-muted leading-snug">
                    ...
                    <div className="px-4 md:px-6 py-3 border-b border-black/5 bg-re-bg/40 xl:hidden">
                        <p className="text-[9px] font-medium text-re-text-muted leading-relaxed">
                            {PORTAL.copy?.timetableExplainer}
                        </p>
                    </div>
                    ...
                    <div className="bg-white">
                        {view === 'grid' ? (
                                ...
                                <table className="w-full text-left border-collapse table-fixed">
                                    <thead>
                                        <tr>
                                            ...
                                            {days.map(day => (
                                                <th key={day} className="border-b border-r border-black/5 bg-re-bg/50 p-1 sm:p-4 text-center text-[7px] sm:text-[10px] font-medium uppercase tracking-[0.05em] sm:tracking-widest text-re-text-muted">
                                                    {day.substring(0, 3)}
                                                </th>
                                            ))}
                                        </tr>
                                    ...
                                    <tbody>
                                        {['08:00', '10:00', '12:00', '14:00', '16:00'].map(time => (
                                            <tr key={time}>
                                                <td className="border-r border-b border-black/5 bg-re-bg p-1 sm:p-4 text-center text-[8px] sm:text-xs font-medium text-[#000435] w-12 sm:w-24 sticky left-0 z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                                                    {time}
                                                </td>
                                                {days.map(day => {
                                                            ...
                                                            {session && (
                                                                <div className={`h-full p-1 sm:p-4 flex flex-col gap-0.5 sm:gap-2 ring-1 ring-inset ${getColorClasses(session.color)}`}>
                                                                    <div className="flex items-center justify-between">
                                                                        <span className="text-[6px] sm:text-[10px] font-medium uppercase tracking-widest opacity-70 truncate">{session.type}</span>
                                                                    ...
                                                                    <div>
                                                                        <h4 className="text-[8px] sm:text-sm font-medium tracking-tight leading-[1] sm:leading-tight truncate sm:whitespace-normal">{session.subject}</h4>
                                                                        <p className="text-[7px] sm:text-[10px] font-medium opacity-80 mt-0.5 sm:mt-1 truncate">{session.group}</p>
                                                                    </div>
                                                                    <div className="mt-auto flex items-center gap-1 text-[7px] sm:text-[9px] font-medium uppercase tracking-widest opacity-80 truncate">
                            ...
                            <div className="space-y-4 p-3">
                                <h3 className="text-sm font-medium uppercase tracking-widest text-re-text-muted px-2">
                                ...
                                {todaySchedule.length === 0 ? (
                                    <div className="bg-white rounded-2xl sm:rounded-3xl p-6 sm:p-12 text-center border border-black/5 shadow-sm">
                                        ...
                                        <h3 className="text-[10px] sm:text-sm font-medium text-re-text tracking-tight uppercase">No periods in this view</h3>
                                        <p className="text-[8px] sm:text-[10px] font-medium text-re-text-muted uppercase tracking-widest mt-2 block">
                                        ...
                                        {todaySchedule.map(session => (
                                            <div key={session.id} className={`bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-6 border border-black/5 shadow-lg relative overflow-hidden group hover:-translate-y-1 transition-all duration-300`}>
                                                ...
                                                <div className="flex items-center justify-between mb-3 sm:mb-4 relative z-10">
                                                    <div className={`px-2 py-1 sm:px-3 sm:py-1.5 rounded-lg text-[7px] sm:text-[8px] font-medium uppercase tracking-widest ring-1 ring-inset ${getColorClasses(session.color)}`}>
                                                        {session.type}
                                                    </div>
                                                ...
                                                <div className="relative z-10 mb-5 sm:mb-6">
                                                    <h3 className="text-base sm:text-xl font-medium text-re-text tracking-tight">{session.subject}</h3>
                                                    <p className="text-[10px] sm:text-[11px] font-medium text-re-text-muted uppercase tracking-widest mt-1">{session.group}</p>
                                                </div>

                                                <div className="space-y-2 sm:space-y-3 relative z-10 border-t border-black/5 pt-3 sm:pt-4">
                                                    <div className="flex items-center gap-2 sm:gap-3 text-[11px] sm:text-xs font-medium text-[#000435]">
