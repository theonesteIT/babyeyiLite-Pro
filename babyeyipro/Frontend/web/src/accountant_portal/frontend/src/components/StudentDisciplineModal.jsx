            ...
            <div className="fixed inset-y-0 right-0 z-[110] w-full md:w-[420px] bg-white shadow-[-20px_0_60px_-15px_rgba(0,0,0,0.1)] flex flex-col animate-in slide-in-from-right duration-500 ease-out">
                ...
                <div className="flex items-center justify-between px-8 py-6 border-b border-black/5 bg-white shrink-0">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-re-bg border border-black/5 flex items-center justify-center text-re-text font-medium text-lg shadow-inner relative overflow-hidden shrink-0">
                        ...
                        <div className="min-w-0">
                            <h3 className="font-medium text-re-text text-base leading-tight uppercase tracking-tight truncate">{student.first_name} {student.last_name}</h3>
                            <div className="flex flex-col gap-0.5 mt-0.5">
                                <p className="text-[9px] text-re-text-muted font-medium flex items-center gap-1 uppercase tracking-widest opacity-40 truncate">
                ...
                <div className="flex-1 overflow-y-auto px-8 py-8 space-y-8 custom-scrollbar bg-white">
                    ...
                    <div className={`flex items-center gap-3 px-4 py-3 rounded-2xl border ${isCritical ? 'bg-red-50 border-red-100/50' : isWarning ? 'bg-orange-50 border-orange-100/50' : 'bg-emerald-50 border-emerald-100/50'}`}>
                        ...
                        <div>
                            <p className="text-[10px] font-medium text-re-text uppercase tracking-widest">{statusLabel} Conduct Status</p>
                            <p className="text-[9px] text-re-text/40 font-medium uppercase tracking-tight leading-none mt-1">Period: {term} {academicYear ? `· ${academicYear}` : ''}</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center gap-2 mb-2">
                            <span className="text-[9px] font-medium text-re-text-muted uppercase tracking-[0.3em] opacity-40">Profile Info</span>
                        ...
                        {[
                            ...
                            <div key={i} className="flex items-center justify-between group">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] font-medium text-re-text-muted uppercase tracking-widest">{item.label}</span>
                                </div>
                                <div className="flex-1 mx-3 border-b border-dashed border-black/10 group-hover:border-[#FEBF10]/30 transition-colors" />
                                <span className="text-[10px] font-medium text-re-text uppercase tracking-tight">{item.value}</span>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-re-bg rounded-3xl p-5 border border-black/5 shadow-inner relative overflow-hidden group">
                            ...
                            <p className="text-[8px] text-re-text-muted uppercase tracking-[0.2em] font-medium mb-1 relative z-10 opacity-60">Current Score</p>
                            <div className="flex items-baseline gap-1 relative z-10">
                                <span className={`text-2xl font-medium tracking-tighter ${isCritical ? 'text-red-500' : isWarning ? 'text-orange-500' : 'text-emerald-500'}`}>
                                    {Number(student.discipline_remaining).toFixed(0)}
                                </span>
                                <span className="text-[10px] font-medium uppercase tracking-widest" style={{ color: "#FEBF10" }}>/{student.discipline_total}</span>
                            </div>
                        </div>
                        <div className="bg-re-bg rounded-3xl p-5 border border-black/5 shadow-inner relative overflow-hidden group text-right">
                            ...
                            <p className="text-[8px] text-re-text-muted uppercase tracking-[0.2em] font-medium mb-1 relative z-10 opacity-60">Conduct Standing</p>
                            <div className="flex items-baseline gap-1 justify-end relative z-10">
                                <span className="text-xl font-medium text-emerald-500 tracking-tighter">+{totalRewards}</span>
                                <span className="text-xl font-medium text-re-text/20 mx-1">|</span>
                                <span className="text-xl font-medium text-red-500 tracking-tighter">-{totalDeductions}</span>
                                <span className="text-[9px] font-medium uppercase tracking-widest ml-1 opacity-60">Pts</span>
                            </div>
                        </div>
                    </div>
                ...
                <div className="px-8 py-6 flex-1 overflow-y-auto">
                    <div className="flex items-center gap-2 mb-4">
                        <span className="text-[9px] font-medium text-re-text-muted uppercase tracking-[0.3em] opacity-40">Disciplinary & Conduct Log</span>
                        <div className="flex-1 h-px bg-black/5" />
                    </div>
                    ...
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-10 gap-3">
                            ...
                            <p className="text-[8px] font-medium text-re-text-muted uppercase tracking-[0.2em] opacity-40">Loading history...</p>
                        </div>
                    ) : logs.length === 0 ? (
                        <div className="text-center py-8">
                            <p className="text-[9px] font-medium text-re-text-muted uppercase tracking-widest italic opacity-40">No disciplinary records found.</p>
                        </div>
                    ) : (
                                    ...
                                    <div key={log.id} className="flex items-start gap-3 p-4 rounded-2xl bg-re-bg/50 border border-black/[0.02] group hover:bg-white hover:border-black/5 transition-all">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <div className="flex items-center justify-between mb-0.5">
                                                <div className="flex items-center gap-2">
                                                    <span className="text-[8px] font-medium uppercase tracking-widest text-re-text">{log.lesson_subject || 'General'}</span>
                                                    <span className="w-1 h-1 bg-black/10 rounded-full"></span>
                                                    <span className="text-[8px] font-medium text-re-text-muted opacity-40 uppercase">{dateStr}</span>
                                                </div>
                                                <span className={`text-[10px] font-medium uppercase tracking-widest ${isPositive ? 'text-emerald-500' : isDeduction ? 'text-red-500' : 'text-blue-500'}`}>
                                                    {isPositive ? '+' : isDeduction ? '-' : ''}{Math.abs(pts)} Pts
                                                </span>
                                            </div>
                                            <p className="text-[10px] font-medium text-re-text-muted leading-relaxed tracking-tight group-hover:text-re-text transition-colors mt-1">{log.description || 'No specific description provided.'}</p>
                                        </div>
                                    </div>
                                );
                            })}
                ...
                <div className="px-8 py-5 border-t border-black/5 bg-re-bg/20 flex flex-col gap-2">
                    <button
                        ...
                        className="h-10 w-full flex items-center justify-center gap-2 text-white rounded-xl font-medium text-[9px] uppercase tracking-widest shadow-lg active:scale-95 transition-all"
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <a
                            ...
                            className={`h-9 flex items-center justify-center gap-2 bg-white border border-black/5 text-re-text font-medium text-[9px] uppercase tracking-widest rounded-xl transition-all ${
                                parentPhone
                                    ? 'hover:bg-re-bg'
                        ...
                        <button
                            ...
                            className="h-9 flex items-center justify-center gap-2 bg-white border border-black/5 text-re-text font-medium text-[9px] uppercase tracking-widest rounded-xl hover:bg-re-bg transition-all disabled:opacity-50"
                        >
