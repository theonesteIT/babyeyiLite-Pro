...
const TopNav = ({ title, onMenuClick }) => {
    ...
    return (
    <>
        <header className="h-16 flex items-center justify-between px-4 md:px-6 bg-white/90 backdrop-blur-xl border-b border-black/10 sticky top-0 z-20 gap-3 font-sans">

            {/* Left — hamburger + page title */}
            <div className="flex items-center gap-3 shrink-0">
                <button
                    onClick={onMenuClick}
                    className="lg:hidden p-2 text-re-text-muted hover:bg-re-bg hover:text-[#1E3A5F] rounded-xl transition-all"
                >
                    <Menu size={18} />
                </button>
                <h1 className="hidden lg:block text-sm font-semibold text-re-text tracking-tight">
                    {title || 'Dashboard'}
                </h1>
            </div>
            ...
            <div className="flex flex-1 max-w-sm">
                <div className="relative w-full group">
                    <span className="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none text-re-text-muted/40 group-focus-within:text-[#1E3A5F] transition-colors">
                        <Search size={16} />
                    </span>
                    <input
                        type="text"
                        ...
                        className="w-full bg-re-bg border border-black/10 rounded-2xl py-2.5 pl-10 pr-4 text-xs font-medium outline-none shadow-inner focus:ring-2 transition-all text-re-text"
                        style={{ '--tw-ring-color': 'rgba(30,58,95,0.15)' }}
            ...
            <div className="flex items-center gap-2 shrink-0">
                ...
                {/* Notification bell */}
                <button className="relative p-2 text-re-text-muted hover:bg-re-bg hover:text-[#1E3A5F] rounded-xl transition-all group">
                    <Bell size={17} />
                ...
                <div className="relative" ref={userRef}>
                    <button
                        ...
                        className="flex items-center gap-2.5 hover:bg-re-bg rounded-xl px-2 py-1.5 transition-all group"
                    >
                        ...
                        <div className="hidden sm:block text-left">
                            <p className="text-xs font-semibold text-re-text leading-tight group-hover:text-[#1E3A5F] transition-colors tracking-tight">
                                {teacher?.first_name || 'Teacher'}
                            </p>
                            <p className="text-[10px] text-re-text-muted/60 font-medium leading-tight tracking-wide">
                        ...
                        <div className="absolute right-0 mt-1.5 w-52 bg-white rounded-2xl shadow-md border border-black/10 z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-150">
                            ...
                            <div className="px-4 py-3 border-b border-black/5">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-xs font-semibold text-re-text tracking-tight">
                            ...
                            <div className="py-1">
                                <button
                                    ...
                                    className="w-full flex items-center gap-2.5 px-4 py-2.5 text-xs font-medium text-re-text-muted hover:bg-re-bg hover:text-[#1E3A5F] transition-all"
                                >
                                ...
                                <button
                                    type="button"
                                    onClick={() => { navigate(h('/ticha-deals')); setUserOpen(false); }}
                                    className="w-full flex items-center gap-2.5 px-4 py-2.5 text-xs font-medium text-re-text-muted hover:bg-re-bg hover:text-[#1E3A5F] transition-all"
                                >
