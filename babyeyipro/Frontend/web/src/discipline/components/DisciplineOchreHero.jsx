...
export default function DisciplineOchreHero({
  eyebrow,
  titleLine,
  titleAccent,
  subtitle,
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex flex-col sm:flex-row gap-5 sm:gap-6 lg:gap-8 items-start">
            ...
            <div className="space-y-1 max-w-3xl">
              ...
              <h1 className="text-xl sm:text-2xl md:text-3xl font-medium text-white tracking-tight leading-tight capitalize">
                {titleLine}{' '}
                <span className="text-[#FEBF10]">{titleAccent}</span>
              </h1>
              <p className="text-xs sm:text-sm font-normal text-white/82 max-w-xl leading-relaxed">{subtitle}</p>
            </div>
          </div>
          {rightSlot ? <div className="flex flex-wrap items-center gap-3 shrink-0">{rightSlot}</div> : null}
        </div>
      </div>
    </div>
  );
