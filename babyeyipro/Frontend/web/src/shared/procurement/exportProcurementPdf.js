...
function drawSignatureBlock(doc, blocks, startY, margin, W) {
  ...
  blocks.forEach((block, i) => {
    ...
    doc.setTextColor(...SLATE);
    doc.setFontSize(7.5);
    doc.text(block.name || '—', x, y + 26);
    doc.setFontSize(6.5);
    doc.setTextColor(...MUTED);
    doc.text(`Date: ${fmtDate(block.date)}`, x, y + 32);
  });
  return y + 40;
}
...
export async function exportPurchaseRequestPdf({ request, school, autoPrint = false }) {
  ...
  y = sectionTitle(doc, 'Authorization', y + 4, margin);
  drawSignatureBlock(doc, [
    { title: 'Requested By', name: request.requested_by, date: request.submitted_at || request.request_date },
    { title: 'Reviewed By', name: request.reviewed_by || request.reviewer, date: request.reviewed_at },
    { title: 'Approved By', name: request.approved_by || request.approver, date: request.approved_at },
  ], y, margin, W);
