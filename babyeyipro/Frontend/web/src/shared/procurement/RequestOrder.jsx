...
import {
  fetchProcurementStats, fetchPurchaseRequests, createPurchaseRequest,
...
import { formatMoneyRWF, fmtNum } from './amountInWords';
import { T, REQUEST_STATUSES, PRIORITIES, PORTAL_SOURCES } from './constants';
import { formatProcurementDate, paginateList } from './procurementFormat';
import TablePagination from '../components/TablePagination';
import ProcurementDateFilterDrawer, { ProcurementFilterButton } from './ProcurementDateFilterDrawer';
import {
...
import useProcurementAcademicOptions from './useProcurementAcademicOptions';
...
export default function RequestOrder({
  ...
  const showAllSchool = requestScope === 'school';
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const requestParams = showAllSchool ? {} : { portal: portalSource };
      const [s, r, sch] = await Promise.all([
        fetchProcurementStats(),
        fetchPurchaseRequests(requestParams),
        fetchSchoolInfo(),
      ]);
      setStats(s);
      setRequests(r || []);
  ...
  }, [portalSource, showAllSchool]);

  useEffect(() => { load(); }, [load]);

  const filtered = useMemo(() => {
    let list = filterProcurementList(requests, 'request_date', dateFilter, academicOptions);
    ...
    return list;
  }, [requests, search, statusFilter, dateFilter, academicOptions]);

  const pagination = useMemo(() => paginateList(filtered, page, pageSize), [filtered, page, pageSize]);

  useEffect(() => { setPage(1); }, [search, statusFilter, pageSize, dateFilter]);

  const ownStats = useMemo(() => {
    if (showAllSchool && stats?.requests) {
      return {
        pending: stats.requests.pending ?? 0,
        under_review: stats.requests.under_review ?? 0,
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <div className="relative">
                ...
                <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search requests..."
                  className="h-10 pl-9 pr-3 rounded-xl border border-black/10 text-sm w-40 sm:w-48" />
              </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-[#F7F8FC] text-[10px] uppercase tracking-widest text-[#000435]/60">
                  ...
                  {showAllSchool && <th className="px-5 py-3 text-left font-semibold">Department</th>}
                  <th className="px-5 py-3 text-left font-semibold">Requested By</th>
                  {showAllSchool && <th className="px-5 py-3 text-left font-semibold">Portal</th>}
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={showAllSchool ? 8 : 6} className="px-5 py-12 text-center text-black/40">Loading...</td></tr>
                ) : filtered.length === 0 ? (
                  <tr><td colSpan={showAllSchool ? 8 : 6} className="px-5 py-12 text-center text-black/40">
                  ...
                  <tr key={req.db_id || req.id} className="border-t border-black/5 hover:bg-amber-50/30 transition-colors">
                    ...
                    {showAllSchool && <td className="px-5 py-3.5 text-black/60">{req.department || '—'}</td>}
                    <td className="px-5 py-3.5">{req.requested_by}</td>
                    {showAllSchool && (
                      <td className="px-5 py-3.5 text-black/60 text-xs">
                        {PORTAL_SOURCES[req.source_portal] || req.source_portal || '—'}
                      </td>
                    )}
