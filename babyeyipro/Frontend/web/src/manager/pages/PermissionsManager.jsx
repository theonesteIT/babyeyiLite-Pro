    ...
    return (
        <div className="animate-in fade-in duration-700 bg-re-bg min-h-screen pb-12">
            ...
            <div className="max-w-[1600px] mx-auto px-6 -mt-20 relative z-20 pb-20">
                <div className="bg-white rounded-t-[32px] shadow-sm border border-black/10 overflow-hidden flex flex-col min-h-[600px]">
                    ...
                    <div className="px-6 py-4 bg-re-bg/10 border-b border-black/5 flex flex-col lg:flex-row items-center justify-between gap-4">
                            ...
                            <div className="flex bg-white rounded-xl p-1 shadow-sm border border-black/5">
                                {['ALL', 'PENDING', 'APPROVED', 'REJECTED', 'CANCELLED'].map(f => (
                                    <button
                                        ...
                                        className={`px-4 py-2 rounded-lg text-[9px] font-semibold uppercase tracking-widest transition-all ${activeFilter === f ? 'bg-[#1E3A5F] text-white shadow-md' : 'text-re-text-muted hover:bg-re-bg'
                                            }`}
                                    >
                                        {f}
                                    </button>
                        ...
                        <div className="flex items-center gap-3 w-full lg:w-auto">
                            ...
                            <select
                                className="h-11 px-3 bg-white border border-black/5 rounded-xl text-[10px] font-semibold uppercase tracking-widest outline-none"
                                value={classFilter}
                    ...
                    <div className="flex-1 overflow-x-auto">
                        {loading ? (
                            <div className="p-24 text-center">
                                ...
                                <p className="text-[10px] font-semibold text-re-text-muted uppercase tracking-widest">Accessing Vault...</p>
                            </div>
                        ) : (
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-re-bg/30">
                                        <th className="px-6 py-4 text-[9px] font-semibold text-re-text-muted uppercase tracking-widest border-b border-black/5">Student</th>
                                        <th className="px-6 py-4 text-[9px] font-semibold text-re-text-muted uppercase tracking-widest border-b border-black/5">Duration</th>
                                        <th className="px-6 py-4 text-[9px] font-semibold text-re-text-muted uppercase tracking-widest border-b border-black/5">Reason & Type</th>
                                        <th className="px-6 py-4 text-[9px] font-semibold text-re-text-muted uppercase tracking-widest border-b border-black/5">Status</th>
                                        <th className="px-6 py-4 text-[9px] font-semibold text-re-text-muted uppercase tracking-widest border-b border-black/5 text-right">Actions</th>
                                    </tr>
                                </thead>
                                            ...
                                            <tr key={p.id} className="hover:bg-re-bg/20 transition-colors group">
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            <div className="text-xs font-semibold text-re-text uppercase tracking-tight">{p.first_name} {p.last_name}</div>
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="space-y-1">
                                                        <div className="inline-flex px-2 py-0.5 rounded bg-re-bg border border-black/5 text-[8px] font-semibold text-re-text-muted uppercase tracking-widest">
                                                            {p.permission_type}
                                                        </div>
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-2">
                                                        <span className={`px-2 py-1 rounded-lg text-[8px] font-semibold uppercase tracking-widest ring-1 ${p.status === 'PENDING' ? 'bg-orange-50 text-orange-600 ring-orange-500/20' :
                                                        ...
                                                        {active && (
                                                            <span className="flex items-center gap-1 text-[8px] font-semibold text-emerald-500 uppercase tracking-widest bg-emerald-100/50 px-2 py-1 rounded-lg animate-pulse">
                        ...
                        {!loading && filteredPermissions.length === 0 && (
                            <div className="p-24 text-center">
                                ...
                                <p className="text-[10px] font-semibold text-re-text-muted uppercase tracking-widest">No permission records found</p>
                            </div>
                        )}
                    </div>
                </div>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#0a192f]/60 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="bg-white rounded-[40px] shadow-sm border border-black/10 w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-300">
                        <div className="p-8 bg-gradient-to-br from-[#1E3A5F] to-[#0D2644] text-white">
                            <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-3">
                                    ...
                                    <h3 className="text-lg font-semibold uppercase tracking-widest">Issue Out-Pass</h3>
                                </div>
                        ...
                        <form onSubmit={handleCreate} className="p-8 space-y-6">
                            <div className="space-y-4">
                                <div className="space-y-1.5">
                                    <label className="text-[9px] font-semibold text-[#1E3A5F] uppercase tracking-widest ml-1">Select Student</label>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1.5">
                                        <label className="text-[9px] font-semibold text-[#1E3A5F] uppercase tracking-widest ml-1">From</label>
                                        <input
                                    ...
                                    <div className="space-y-1.5">
                                        <label className="text-[9px] font-semibold text-[#1E3A5F] uppercase tracking-widest ml-1">To</label>
                                        <input
                                            required type="datetime-local"
                                            className="w-full h-12 px-4 bg-re-bg rounded-2xl border border-black/5 outline-none text-[10px] font-bold uppercase"
                                            value={form.ends_at}
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-[9px] font-semibold text-[#1E3A5F] uppercase tracking-widest ml-1">Leave Type</label>
                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                                        {['MEDICAL', 'FAMILY', 'OFFICIAL', 'OTHER'].map(type => (
                                            <button
                                                key={type} type="button"
                                                onClick={() => setForm({ ...form, permission_type: type })}
                                                className={`py-2 rounded-xl text-[8px] font-semibold uppercase tracking-widest transition-all border ${form.permission_type === type ? 'bg-[#1E3A5F] text-white border-[#1E3A5F]' : 'bg-re-bg border-black/5 text-re-text-muted hover:border-[#1E3A5F]/30'
                                                    }`}
                                            >
                                                {type}
                                            </button>
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-[9px] font-semibold text-[#1E3A5F] uppercase tracking-widest ml-1">Reason / Remarks</label>
                                    <textarea
                            ...
                            <div className="flex items-center gap-3 pt-6">
                                <button
                                    ...
                                    className="flex-1 h-13 rounded-2xl border border-black/5 text-re-text font-medium text-[10px] uppercase tracking-[0.2em] hover:bg-re-bg transition-all"
                                >
                                    Cancel
                                </button>
                                <button
                                    type="submit"
                                    disabled={isSaving}
                                    className="flex-[2] h-13 rounded-2xl text-white font-medium text-[10px] uppercase tracking-[0.2em] shadow-sm active:scale-95 transition-all flex items-center justify-center gap-2"
                                    style={{ background: "linear-gradient(135deg, #1E3A5F 0%, #0D2644 100%)" }}
                                >
            ...
            {selectedPermission && (
                <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-[#0a192f]/60 backdrop-blur-sm">
                    <div className="bg-white rounded-[28px] shadow-sm border border-black/10 w-full max-w-3xl overflow-hidden">
                        <div className="px-6 py-5 bg-[#000435] text-white flex items-center justify-between">
                            <div>
                                <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-white/70">Permission Details</p>
                                <h3 className="text-2xl font-semibold tracking-tight">PERM-{String(selectedPermission.id).padStart(6, '0')}</h3>
                            </div>
                            <button
                            ...
                            <div className="grid grid-cols-1 gap-3">
                                {[
                                    ...
                                    <div key={label} className="flex items-start justify-between gap-6 py-2 border-b border-black/5">
                                        <p className="text-sm leading-none font-semibold text-[#1e2a3b]">{label}</p>
