            ...
            <div className="fixed inset-y-0 right-0 z-[110] w-full md:w-[420px] bg-white shadow-[-20px_0_60px_-15px_rgba(0,0,0,0.1)] flex flex-col animate-in slide-in-from-right duration-500 ease-out">
                ...
                <div className="flex items-center justify-between px-8 py-6 border-b border-black/5 bg-white shrink-0">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-re-bg border border-black/5 flex items-center justify-center text-re-text font-semibold text-lg shadow-inner relative overflow-hidden">
                        ...
                        <div>
                            <h3 className="font-semibold text-re-text text-base leading-tight uppercase tracking-tight">{student.name}</h3>
                ...
                <div className="flex-1 overflow-y-auto px-8 py-8 space-y-8 custom-scrollbar">
                    ...
                    <div className={`flex items-center gap-3 px-4 py-3 rounded-2xl border ${student.status === 'Exceptional' ? 'bg-emerald-50 border-emerald-100/50' : student.status === 'Review Required' ? 'bg-red-50 border-red-100/50' : 'bg-re-navy/5 border-re-navy/10'}`}>
                        ...
                        <div>
                            <p className={`text-[10px] font-semibold uppercase tracking-widest ${student.status === 'Review Required' ? 'text-red-600' : 'text-re-text'}`}>{student.status} Performing Student</p>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-re-bg rounded-3xl p-5 border border-black/5 shadow-inner relative overflow-hidden group">
                            ...
                            <div className="flex items-baseline gap-1 relative z-10">
                                <span className="text-2xl font-semibold text-re-text tracking-tighter">{student.gpa}</span>
                        ...
                        <div className="bg-re-bg rounded-3xl p-5 border border-black/5 shadow-inner relative overflow-hidden group text-right">
                            ...
                            <p className="text-[8px] text-re-text-muted uppercase tracking-[0.2em] font-semibold mb-1 relative z-10 opacity-60">Presence Status</p>
                            <div className="flex items-baseline gap-1 justify-end relative z-10">
                                <span className="text-[12px] font-semibold text-re-text tracking-tighter uppercase whitespace-nowrap">{student.attendance}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4 pb-4">
                        <div className="flex items-center gap-2 mb-2">
                            <span className="text-[9px] font-semibold text-re-text-muted uppercase tracking-[0.3em] opacity-40">Performance Trajectory</span>
                            <div className="flex-1 h-px bg-black/5" />
                        </div>
                ...
                <div className="px-8 py-6 border-t border-black/5 bg-re-bg/20 grid grid-cols-2 gap-3">
                    ...
                    <button className="h-12 w-full flex items-center justify-center gap-2 bg-white border border-black/5 text-re-text font-medium text-[10px] uppercase tracking-widest rounded-2xl hover:bg-re-bg transition-all">
            ...
            <div className="relative w-full min-h-[280px] overflow-hidden bg-[#c87800]">
                ...
                <div className="relative z-20 max-w-[1600px] mx-auto px-6 md:px-12 pt-20 pb-24 flex items-center gap-8">
                    ...
                    <div className="hidden md:flex shrink-0 w-24 h-24 rounded-[32px] border border-white/10 bg-white/5 items-center justify-center backdrop-blur-xl shadow-sm relative overflow-hidden group">
                        ...
                        <BookOpen size={40} style={{ color: "#FEBF10" }} className="" />
                    </div>

                    <div className="space-y-1">
                        <div className="flex items-center gap-2 mb-2">
                            <span className="w-6 h-1 rounded-full animate-pulse" style={{ background: "#FEBF10" }}></span>
                            <p className="text-[10px] font-semibold uppercase tracking-[0.3em]" style={{ color: "#FEBF10" }}>Class Intel Overview</p>
                        </div>
                ...
                <div className="bg-white rounded-t-[32px] shadow-sm border border-black/10 overflow-hidden flex flex-col">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-4 border-b border-black/5">
                        <div className="lg:col-span-3 grid grid-cols-2 md:grid-cols-4 divide-x divide-y md:divide-y-0 divide-black/5">
                            <div className="p-4 sm:p-8 flex flex-col items-center justify-center text-center group hover:bg-re-bg/20 transition-all cursor-default relative overflow-hidden">
                                ...
                                <div className="flex items-baseline gap-1 relative z-10">
                                    <span className="text-sm sm:text-2xl font-semibold text-re-text tracking-tighter group-hover:text-[#1E3A5F] transition-colors">{MOCK_STATS.avgGpa}</span>
                                    <span className="text-[10px] font-semibold uppercase tracking-widest text-emerald-500">{MOCK_STATS.gpaTrend}</span>
                                </div>
                                <p className="text-[6px] sm:text-[8px] font-semibold text-re-text-muted uppercase tracking-[0.2em] mt-0.5 sm:mt-1 opacity-60">Class GPA</p>
                            </div>
                            ...
                            {[
                                ...
                                <div key={i} className="p-4 sm:p-8 flex flex-col items-center justify-center text-center group hover:bg-re-bg/20 transition-all cursor-default">
                                    ...
                                    <span className="text-sm sm:text-2xl font-semibold text-re-text tracking-tighter group-hover:text-[#1E3A5F] transition-colors">{stat.value}</span>
                        ...
                        <div className="hidden lg:flex flex-col border-l border-black/5 bg-re-bg/30 p-6 justify-center gap-3 relative">
                            <button
                                className="w-full h-11 px-4 flex items-center justify-center gap-2 text-white rounded-xl font-medium text-[9px] uppercase tracking-widest shadow-sm active:scale-95 transition-all whitespace-nowrap"
                                style={{ background: "linear-gradient(135deg, #1E3A5F 0%, #0D2644 100%)" }}
                            >
                            ...
                            <button
                                className="w-full h-11 px-4 flex items-center justify-center gap-2 bg-white border border-black/5 text-re-text font-medium text-[9px] uppercase tracking-widest rounded-xl hover:bg-re-bg transition-all hover:shadow-sm whitespace-nowrap"
                            >
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-3 border-b border-black/5 divide-y xl:divide-y-0 xl:divide-x divide-black/5 bg-white">
                        ...
                        <div className="xl:col-span-2 p-6 md:p-8 bg-re-bg/20">
                            <h3 className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[#1E3A5F] mb-4">Subject Mastery Matrix</h3>
                                ...
                                {MOCK_SUBJECTS.map((sub, i) => (
                                    <div key={i} className="bg-white p-4 rounded-xl border border-black/5 shadow-sm relative overflow-hidden group">
                                        ...
                                        <div className="flex items-baseline gap-1 mt-1">
                                            <p className="text-xl font-semibold text-re-text tracking-tighter">{sub.avg}%</p>
                                        </div>
                        ...
                        <div className="p-6 md:p-8 bg-red-50/50">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-[10px] font-semibold uppercase tracking-[0.2em] text-red-600 flex items-center gap-1.5"><AlertTriangle size={12} /> Intervention Hotlist</h3>
                                <span className="bg-red-100 text-red-600 text-[8px] font-semibold px-2 py-0.5 rounded-md uppercase tracking-widest">{atRiskStudents.length} Flagged</span>
                            </div>
                            <div className="space-y-2">
                                {atRiskStudents.map(stu => (
                                    ...
                                    >
                                        <div>
                                            <p className="text-xs font-semibold text-re-text leading-tight group-hover:text-red-600 transition-colors">{stu.name}</p>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-re-bg/20 border-b border-black/5">
                                    <th className="px-4 sm:px-8 py-3 sm:py-4 text-[7px] sm:text-[8px] font-semibold text-re-text-muted uppercase tracking-[0.2em] opacity-40 border-r border-black/5 last:border-r-0">Student Profile</th>
                                    <th className="hidden md:table-cell px-6 py-4 text-[8px] font-semibold text-re-text-muted uppercase tracking-[0.2em] opacity-40 border-r border-black/5">Cum. GPA</th>
                                    <th className="hidden lg:table-cell px-6 py-4 text-[8px] font-semibold text-re-text-muted uppercase tracking-[0.2em] opacity-40 border-r border-black/5">Core Mastery (M, S, E)</th>
                                    <th className="hidden xl:table-cell px-6 py-4 text-[8px] font-semibold text-re-text-muted uppercase tracking-[0.2em] opacity-40 border-r border-black/5">Alt. Risk Factor</th>
                                    ...
                                    >
                                        <td className="px-4 sm:px-8 py-3 sm:py-5 border-r border-black/5 last:border-r-0">
                                            <div className="flex items-center gap-3 sm:gap-4">
                                                ...
                                                <div>
                                                    <p className="text-xs sm:text-sm font-semibold text-re-text tracking-tight uppercase leading-none mb-1 group-hover:text-[#1E3A5F] transition-colors">{stu.name}</p>
                                        ...
                                        <td className="hidden md:table-cell px-6 py-5">
                                            <div className="flex flex-col gap-0.5">
                                                <div className="flex items-center gap-2">
                                                    <p className="text-[10px] font-semibold text-re-text uppercase tracking-tight">GPA: {stu.gpa}</p>
                                        ...
                                        <td className="px-4 sm:px-8 py-3 sm:py-5 text-right">
                                            <button
                                                ...
                                                className="h-8 px-4 rounded-xl flex items-center justify-center gap-2 bg-white border border-black/5 text-re-text font-medium text-[9px] uppercase tracking-widest shadow-sm hover:bg-re-bg hover:text-[#1E3A5F] transition-all ml-auto"
                                            >
                                                <Eye size={12} />
