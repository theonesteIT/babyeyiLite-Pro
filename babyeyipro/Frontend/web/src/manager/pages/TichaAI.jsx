    ...
    <div className="relative w-full bg-re-bg min-h-screen">
        ...
        <div className="relative z-20 max-w-[1600px] mx-auto px-6 md:px-12 pt-8 md:pt-12 pb-10 md:pb-16 flex items-center gap-6">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-[9px] font-black uppercase tracking-[0.3em]" style={{ color: "#FEBF10" }}>Cognitive Core</p>
            </div>
            <h1 className="text-3xl md:text-5xl font-black text-white tracking-tighter leading-none mb-1 mt-1 uppercase">Ticha<span style={{ color: "#FEBF10" }}>AI</span></h1>
      ...
      <div className="max-w-[1600px] mx-auto px-4 md:px-12 -mt-10 md:-mt-12 relative z-20 pb-20">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
          ...
          <div className="lg:col-span-2 flex flex-col h-[calc(100vh-220px)] md:h-[calc(100vh-280px)] min-h-[450px] bg-white rounded-[32px] shadow-2xl border border-black/5 overflow-hidden relative">
            <header className="px-6 py-4 md:py-5 border-b border-black/5 bg-white/80 backdrop-blur-md flex items-center justify-between sticky top-0 z-30">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl flex items-center justify-center text-white shadow-xl" style={{ background: "linear-gradient(135deg, #1E3A5F 0%, #0D2644 100%)" }}>
                  <Bot className="size-5 md:size-6" style={{ color: "#FEBF10" }} />
                </div>
                <div>
                  <h2 className="text-sm md:text-base font-black text-[#1E3A5F] tracking-tight uppercase">Assistant Engine</h2>
            ...
            {showHistoryMob && (
              <div className="lg:hidden absolute top-[70px] md:top-[85px] inset-x-0 z-40 bg-white/95 backdrop-blur-xl border-b border-black/10 shadow-2xl max-h-[350px] overflow-y-auto animate-in slide-in-from-top duration-300">
                <div className="p-4 space-y-2">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <span className="w-0.5 h-2 rounded-full" style={{ background: "#FEBF10" }}></span>
                      <h3 className="text-[8px] font-black text-[#1E3A5F] uppercase tracking-widest">Previous Inquiries</h3>
                    </div>
                    <button onClick={() => setShowHistoryMob(false)} className="text-[8px] font-black uppercase tracking-widest text-red-500 hover:opacity-80">Close</button>
                  </div>
                  {history.length > 0 ? history.map((item) => (
                    ...
                    >
                      <p className="text-[10px] font-black text-[#1E3A5F] truncate uppercase tracking-tight">{item.prompt}</p>
            ...
            <div className="flex-1 overflow-y-auto px-4 md:px-8 py-6 md:py-10 space-y-8 custom-scrollbar bg-re-bg/20">
              {messages.length === 0 ? (
                <div className="max-w-2xl mx-auto pt-8 md:pt-12 text-center space-y-6">
                  <div className="w-16 h-16 md:w-20 md:h-20 bg-white shadow-xl rounded-[24px] flex items-center justify-center mx-auto border border-black/5 relative relative transition-transform hover:scale-105 duration-300">
                  ...
                  <div className="space-y-1">
                    <h3 className="text-lg md:text-2xl font-black text-[#1E3A5F] tracking-tighter uppercase leading-tight">Muraho! How can I help?</h3>
                    <p className="text-[9px] md:text-[10px] text-re-text-muted font-black uppercase tracking-[0.3em] opacity-60 pt-2">Guidance • Curriculum • Planning</p>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-6 px-4 max-w-lg mx-auto">
                    {[
                      ...
                      <button
                        ...
                        className="p-4 rounded-2xl bg-white hover:bg-re-navy/5 text-[9px] font-black text-[#1E3A5F] border border-black/5 text-left transition-all uppercase tracking-widest shadow-sm hover:shadow-md hover:-translate-y-0.5 group"
                      >
                ...
                <div className="max-w-4xl mx-auto space-y-6 md:space-y-8">
                  {messages.map((msg, i) => (
                    <div key={i} className={`flex items-start gap-3 md:gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                      ...
                      >
                        {msg.content}
                        <div className={`text-[8px] font-black uppercase tracking-widest mt-2 pt-2 border-t ${msg.role === 'user' ? 'border-[#1E3A5F]/10 text-[#1E3A5F] text-right opacity-60' : 'border-black/5 text-[#1E3A5F] opacity-40'}`}>
              ...
              <form className="max-w-4xl mx-auto relative group flex gap-2 md:gap-3" onSubmit={handleSend}>
                <div className="relative flex-1">
                  <input
                    ...
                    className="w-full h-12 md:h-14 bg-re-bg rounded-2xl pl-12 pr-12 font-black outline-none border border-black/5 focus:border-[#1E3A5F]/20 shadow-inner focus:ring-4 focus:ring-[#1E3A5F]/5 transition-all text-[#1E3A5F] text-xs md:text-sm placeholder:text-[#1E3A5F]/30"
                    value={input}
                ...
                <button
                  ...
                  className="w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center text-white shadow-xl hover:scale-[1.03] active:scale-95 transition-all shrink-0 disabled:opacity-50 disabled:hover:scale-100"
                  style={{ background: 'linear-gradient(135deg, #1E3A5F 0%, #0D2644 100%)' }}
                >
                  <Send size={18} />
                </button>
              </form>
              <p className="text-center text-[8px] text-[#1E3A5F] mt-3 font-black uppercase tracking-[0.2em] opacity-30 italic">Intelligent Support by Babyeyi AI Engine</p>
            </div>
          </div>
          ...
          <div className="hidden lg:flex flex-col space-y-6 lg:sticky lg:top-8 h-fit">
            <div className="bg-white rounded-[32px] shadow-sm border border-black/5 p-6 md:p-8 flex flex-col max-h-[600px]">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-[10px] font-black text-[#1E3A5F] uppercase tracking-[0.2em]">Previous Inquiries</h3>
              </div>
              <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                {history.map((item) => (
                  ...
                  >
                    <p className="text-[10px] md:text-xs font-black text-[#1E3A5F] truncate group-hover:text-[#FEBF10] uppercase tracking-tight transition-colors">{item.prompt}</p>
                    <div className="flex items-center gap-2 mt-2">
                      <span className="text-[8px] text-[#1E3A5F] font-black uppercase tracking-widest opacity-40">
                      ...
                      <span className="text-[8px] text-[#FEBF10] font-black uppercase tracking-widest opacity-80">Verified</span>
                    </div>
                  </button>
                ))}
                {history.length === 0 && (
            ...
            <div className="relative rounded-[32px] p-8 text-white shadow-xl overflow-hidden group cursor-pointer" style={{ background: 'linear-gradient(135deg, #1E3A5F 0%, #0a192f 100%)' }}>
              <div className="relative z-10 flex flex-col gap-4">
                ...
                <div>
                  <h4 className="font-black text-xs tracking-widest uppercase opacity-90" style={{ color: "#FEBF10" }}>Auto-Lesson Plan</h4>
