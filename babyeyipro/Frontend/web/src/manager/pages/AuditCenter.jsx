    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-3">
                ...
                <button onClick={() => fetch(page)} className="flex items-center gap-1.5 px-3 py-2.5 bg-[#1E3A5F] text-white rounded-xl text-[11px] font-semibold uppercase tracking-wide hover:bg-[#0D2644] transition-colors">
                            ...
                            return (
                                <tr key={r.id} className="border-b border-slate-50 hover:bg-slate-50/60 transition-colors">
                                    <td className="px-4 py-3">
                                        <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-lg text-[10px] font-semibold border ${s.bg} ${s.text} ${s.border}`}>
                                    ...
                                    <td className="px-4 py-3">
                                        <span className={`inline-flex px-2 py-1 rounded-lg text-[9px] font-semibold ${roleColor(r.role_code)}`}>
                                            {r.role_code || '—'}
                                        </span>
                                    </td>
                                    <td className="px-4 py-3">
                    ...
                    return (
                        <div key={r.id} className="bg-white border border-slate-100 rounded-2xl p-4 space-y-2">
                            ...
                            <div className="flex items-center gap-2">
                                <span className={`inline-flex px-2 py-0.5 rounded-lg text-[9px] font-semibold ${roleColor(r.role_code)}`}>{r.role_code || '—'}</span>
...
const StaffLogins = () => {
    ...
    return (
        <div className="space-y-4">
            ...
            {/* Mini stats */}
            <div className="flex gap-3 flex-wrap text-[10px] font-semibold text-slate-400 uppercase tracking-widest">
                        ...
                        return (
                            <div key={r.id} className={`bg-white border rounded-2xl p-4 space-y-3 transition-all hover:shadow-md ${isToday ? 'border-emerald-200' : 'border-slate-100'}`}>
                                ...
                                <div className="flex items-center gap-3">
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-[13px] font-semibold shrink-0 ${isToday ? 'bg-emerald-100 text-emerald-700' : 'bg-[#1E3A5F]/10 text-[#1E3A5F]'}`}>
                                        {initials}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className="text-[12px] font-semibold text-slate-800 truncate uppercase tracking-tight">
                                            {r.first_name} {r.last_name}
                                        </p>
                                        <p className="text-[9px] text-slate-400 font-bold truncate">{r.email || '—'}</p>
                                    </div>
                                    {isToday && (
                                        <span className="flex items-center gap-1 px-2 py-0.5 bg-emerald-50 border border-emerald-200 rounded-full text-[8px] font-semibold text-emerald-600">
                        ...
                        {TABS.map(t => {
                            ...
                            return (
                                <button
                                    key={t.id}
                                    onClick={() => setTab(t.id)}
                                    className={`flex items-center gap-2 px-5 py-3.5 text-[11px] font-semibold uppercase tracking-widest transition-all border-b-2 ${active ? 'border-[#1E3A5F] text-[#1E3A5F] bg-white' : 'border-transparent text-slate-400 hover:text-slate-600 hover:bg-white/60'}`}
                                >
                                    <T size={13} />
