...
const DashboardLayout = ({ children, title }) => {
  ...
  return (
    <div className="flex h-screen bg-re-bg font-sans overflow-hidden">
      ...
      {/* Main content — avoid overflow-hidden here or the TopNav user menu gets clipped and clicks fall through to the page below */}
      <div className="flex-1 flex flex-col min-w-0 min-h-0 relative">
        {!isChatFocused && <TopNav title={title} onMenuClick={() => setIsSidebarOpen(true)} />}

        <main className={`flex-1 min-h-0 overflow-y-auto overflow-x-hidden relative ${isChatFocused ? 'pb-0' : 'pb-[5.75rem] lg:pb-0'}`}>
          {/* Background glows */}
