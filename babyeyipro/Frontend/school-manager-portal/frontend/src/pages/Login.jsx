...
import { Mail, Lock, Eye, EyeOff, AlertCircle, RefreshCw, ChevronRight, GraduationCap, ArrowLeft, School } from 'lucide-react';
...
const Login = () => {
   ...
   const { login, manager, loading: authLoading } = useAuth();
   ...
   if (authLoading) {
      return (
         <div className="min-h-screen bg-re-bg flex items-center justify-center">
            <RefreshCw className="animate-spin text-re-navy w-8 h-8" />
         </div>
      );
   }
   ...
   return (
      <div className="flex min-h-screen bg-re-bg font-sans overflow-hidden relative">
         {/* Background glows */}
         <div className="absolute top-0 right-0 w-96 h-96 bg-re-navy/5 blur-3xl -mr-48 -mt-48 rounded-full pointer-events-none"></div>
         ...
         <div className="w-full flex items-center justify-center p-0 md:p-8 z-10">
            <div className="group bg-white flex flex-col md:flex-row shadow-2xl md:rounded-0 w-full md:max-w-4xl overflow-hidden h-full md:max-h-[520px]">
               ...
               <div className="flex w-full h-52 md:h-auto md:w-1/2 relative overflow-hidden group/image shrink-0">
                  <div className="w-full h-full bg-re-navy flex items-center justify-center">
                      <School className="text-re-gold/20 w-32 h-32 animate-pulse" />
                  </div>
                  {/* Overlays */}
                  <div className="absolute inset-0 bg-re-navy/40 backdrop-blur-[1.5px]"></div>
                     ...
                     <div>
                        ...
                        <div className="flex items-center bg-re-bg border border-black/5 rounded-xl overflow-hidden mt-1 shadow-inner focus-within:ring-2 transition-all" style={{ '--tw-ring-color': 'rgba(30,58,95,0.35)' }}>
                           ...
                           <button
                              ...
                              className="text-re-text-muted/40 hover:text-re-navy mr-3 transition-colors"
                           >
