  ...
  try {
    ...
    const [rows] = await promisePool.query(
      `SELECT id, service_code, name, category, description, short_tagline, icon_url,
              ...
              stock_quantity, terms_conditions, status
       FROM services
  ...
  try {
    ...
    const [rows] = await promisePool.query(
      isNum
        ...
        : `SELECT * FROM services WHERE service_code = ? AND status = 'active' LIMIT 1`,
  ...
  try {
    ...
    const [ins] = await conn.query(
      `INSERT INTO services (
        ...
        redemption_method, delivery_method, stock_quantity, payment_rules, terms_conditions, status
      ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [
...
router.delete('/admin/services/:id', requireSuper, async (req, res) => {
  try {
    const id = parseInt(req.params.id, 10);
    await promisePool.query(`UPDATE services SET status = 'archived' WHERE id = ?`, [id]);
...
router.post('/public/quote', async (req, res) => {
  try {
    ...
    let service;
    if (sid) {
      const [sr] = await promisePool.query(`SELECT * FROM services WHERE id = ? AND status = 'active' LIMIT 1`, [sid]);
      service = sr[0];
    } else if (sCode) {
...
router.post('/public/pay-momo', async (req, res) => {
  try {
    ...
    const [[service]] = await promisePool.query(`SELECT * FROM services WHERE id = ? AND status = 'active' LIMIT 1`, [
      serviceId,
    ]);
    if (!service) {
      return res.status(404).json({ success: false, message: 'Service not found' });
