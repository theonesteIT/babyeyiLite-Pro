    ...
    const [[rows], [countRows]] = await Promise.all([
      db.query(
        `SELECT s.id, s.school_name, s.school_code, s.province, s.district, s.district_code, s.sector,
                ...
                s.a_level_combinations, s.is_skeleton
 ...
 * JSON body: schoolName, province, district, sector, ownership, levels[], aLevelCombinations[]?
 */
  ...
  try {
    ...
    const [schoolResult] = await conn.query(
      `INSERT INTO schools (
        ...
        a_level_combinations, is_skeleton,
  ...
  async (req, res) => {
    ...
    try {
      ...
      const fieldMap = {
        ...
        school_code: 'school_code', schoolCode: 'school_code',
        school_category: 'school_category', category: 'school_category',
        ownership_type: 'ownership_type', ownership: 'ownership_type',
        year_established: 'year_established', yearEstablished: 'year_established',
        boarding_type: 'boarding_type',
        vision: 'vision',
        province: 'province', district: 'district', sector: 'sector',
        cell: 'cell', village: 'village', full_address: 'full_address', fullAddress: 'full_address',
        map_url: 'map_url', mapUrl: 'map_url',
        phone: 'phone', email: 'email', 
        postal_address: 'postal_address', postal: 'postal_address',
        website: 'website',
        head_teacher_name: 'head_teacher_name', headName: 'head_teacher_name',
        head_teacher_phone: 'head_teacher_phone', headPhone: 'head_teacher_phone',
        head_teacher_email: 'head_teacher_email', headEmail: 'head_teacher_email',
        deputy_head_name: 'deputy_head_name', deputyName: 'deputy_head_name',
      };

      const updateData = new Map();
      updateData.set('updated_at', 'NOW()');

      // Efficiently collect unique column updates
      Object.entries(fieldMap).forEach(([bodyKey, colName]) => {
        if (req.body[bodyKey] !== undefined) {
          let val = req.body[bodyKey];
          if (colName === 'school_code') val = String(val).toUpperCase().trim();
          updateData.set(colName, val);
        }
      });

      const setClauses = [];
      const params = [];
      for (const [col, val] of updateData.entries()) {
        if (col === 'updated_at') {
      ...
      if (req.body.levels !== undefined || req.body.education_levels !== undefined) {
        let levels = req.body.levels || req.body.education_levels;
        try { if (typeof levels === 'string') levels = JSON.parse(levels); } catch (_) {}
        setClauses.push('education_levels = ?');
        params.push(JSON.stringify(levels));
      }

      const comboVal = req.body.offered_combinations || req.body.a_level_combinations;
      if (comboVal !== undefined) {
        let combinations = comboVal;
        if (typeof combinations === 'string' && combinations.includes(',')) {
          combinations = combinations.split(',').map(c => c.trim()).filter(Boolean);
        } else if (typeof combinations === 'string' && combinations) {
