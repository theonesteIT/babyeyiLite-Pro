...
// ════════════════════════════════════════════════════════════
// PATCH /api/babyeyi/school-profile  — school manager updates directory fields
// ════════════════════════════════════════════════════════════
router.patch("/school-profile", async (req, res) => {
  try {
    if (!req.user?.id) {
    ...
    const userSid = Number(
      req.user.school_id || req.session?.user?.school_id || req.session?.school_id || 0
    );
    if (userSid && userSid !== Number(schoolId)) {
      return res.status(403).json({ success: false, message: "No access to this school" });
    }
    const b = req.body || {};
    const updates = [];
    const vals = [];
    ...
    if (b.name != null) set("school_name", String(b.name).trim());
    if (b.headTeacher != null) set("head_teacher_name", String(b.headTeacher).trim());
    if (b.district != null) set("district", String(b.district).trim());
    if (b.sector != null) set("sector", String(b.sector).trim());
    if (b.email != null) set("email", String(b.email).trim());
    if (b.phone != null) set("phone", String(b.phone).trim());
    if (b.category != null) set("school_category", String(b.category).trim());

    if (!updates.length) {
      return res.json({ success: true, message: "No changes" });
    }
    vals.push(schoolId);
    await query(`UPDATE schools SET ${updates.join(", ")}, updated_at = NOW() WHERE id = ?`, vals);
    res.json({ success: true, message: "School profile updated" });
  } catch (err) {
    console.error("❌ PATCH /school-profile:", err.message);
...
// ════════════════════════════════════════════════════════════
// GET /api/babyeyi/audit — audit log rows for this school (must be before /:id)
// ════════════════════════════════════════════════════════════
router.get("/audit", async (req, res) => {
  try {
    ...
    const raw = await query(
      `SELECT a.* FROM babyeyi_audit_log a
       INNER JOIN school_babyeyi b ON b.id = a.babyeyi_id AND b.is_active = 1
       WHERE b.school_id = ?
       ORDER BY a.id DESC
       LIMIT ?`,
      [schoolId, limit]
    ...
    const rows = (raw || []).map((r) => ({
      ...
      action: r.action,
      changed_by: r.changed_by ?? r.user_id ?? r.actor_id ?? null,
      old_values: r.old_values ?? r.old_status ?? null,
      new_values: r.new_values ?? r.changes ?? null,
      created_at: r.created_at ?? null,
      ip_address: r.ip_address ?? null,
    }));
    res.json({ success: true, data: rows });
  } catch (err) {
    console.error("[babyeyi/audit]", err.message);
