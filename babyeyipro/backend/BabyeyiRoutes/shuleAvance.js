...
router.get('/teacher/my-requests', requireRole(ROLE_TEACHER), async (req, res) => {
  try {
...
router.post('/teacher/requests', requireRole(ROLE_TEACHER), async (req, res) => {
  try {
    const { schoolId, userId } = req.ctx;
    ...
    res.status(201).json({ success: true, message: 'Request submitted to accountant', data: created });
  } catch (error) {
...
router.put('/teacher/requests/:id', requireRole(ROLE_TEACHER), async (req, res) => {
  try {
    const { schoolId, userId } = req.ctx;
...
router.delete('/teacher/requests/:id', requireRole(ROLE_TEACHER), async (req, res) => {
  try {
    const { schoolId, userId } = req.ctx;
    const id = Number(req.params.id);
    if (!id) return res.status(400).json({ success: false, message: 'Invalid request id' });
...
router.get('/finance/pending-invoices', requireRole(ROLE_ACCOUNTANT), async (req, res) => {
  try {
...
router.patch('/finance/invoice-requests/:id/send-to-manager', requireRole(ROLE_ACCOUNTANT), async (req, res) => {
  try {
    const { schoolId, userId } = req.ctx;
    const id = Number(req.params.id);
...
router.patch('/finance/invoice-requests/:id/reject', requireRole(ROLE_ACCOUNTANT), async (req, res) => {
  try {
    const { schoolId, userId } = req.ctx;
    const id = Number(req.params.id);
    const note = String(req.body?.note || '').trim();
...
router.get('/manager/pending-requests', requireRole(MANAGER_ROLES), async (req, res) => {
  try {
    const { schoolId } = req.ctx;
    const [rows] = await promisePool.query(
      `SELECT r.id, r.school_id, r.teacher_user_id, r.amount_rwf, r.purpose, r.repayment_term_months,
...
router.patch('/manager/invoice-requests/:id/decision', requireRole(MANAGER_ROLES), async (req, res) => {
  try {
    const { schoolId, userId } = req.ctx;
    const id = Number(req.params.id);
    ...
    res.json({ success: true, message: `Request ${verb} successfully` });
  } catch (error) {
