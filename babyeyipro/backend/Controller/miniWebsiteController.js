...
function parseBody(body, files) {
  ...
  // ── Fees ─────────────────────────────────────────────────────────────────
  const VALID = new Set(['nursery', 'primary', 'olevel', 'alevel', 'tvet']);
  let fees = tryJson(body.fees);
  if (fees && typeof fees === 'object') {
    const clean = {};
    for (const [lv, d] of Object.entries(fees)) {
      if (!VALID.has(lv) || !d?.items?.length) continue;
      ...
      if (items.length) clean[lv] = { currency: d.currency || 'RWF', notes: d.notes || null, items };
    }
...
async function syncLeaders(conn, schoolId, leaders) {
  ...
  // Split head vs others based on role label
  const head = clean.find(l => (l.role || '').toLowerCase() === 'head teacher') || null;
  const others = clean.filter(l => (l.role || '').toLowerCase() !== 'head teacher');
  ...
  if (head) {
    // Upsert head row without touching signature/stamp columns
    await conn.query(
      `INSERT INTO school_leaders (school_id, role_type, full_name, role_title, phone, email, photo_url, sort_order)
  ...
  try {
    ...
    res.json({
      ...
      data: rows.map(r => ({
        ...
        aLevelCombos: tryJson(r.a_level_combinations, []),
        tvetTrades: tryJson(r.tvet_trades, []),
      })),
...
exports.getFilterOptions = async (req, res) => {
  try {
    const [rows] = await db.query(
      `SELECT a_level_combinations AS aLevelCombos, tvet_trades AS tvetTrades
       FROM school_mini_websites
       WHERE status = 'published'`
    );
    const comboMap = new Map();
    const tradeSet = new Set();
    for (const row of rows) {
      let combos = tryJson(row.aLevelCombos) || [];
      ...
      let trades = tryJson(row.tvetTrades) || [];
