        ...
        <div className="relative p-6 bg-[linear-gradient(135deg,#FF8C00,#FF5E00)] overflow-hidden shrink-0">
          ...
          <div className="flex items-center justify-between relative z-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-[11px] font-bold text-white leading-none">
                  New Requisition
                </h1>
          ...
          <div className="w-full mx-auto space-y-5 animate-in fade-in duration-500">
            {activeStep === 1 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 animate-in slide-in-from-right-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-re-text-muted opacity-60 ml-1">
                    Total Amount (RWF) <span className="font-bold normal-case opacity-60">(optional)</span>
                  </label>
                  <div className="relative group">
                    <div className="absolute left-3.5 top-1/2 -translate-y-1/2 text-re-orange">
                      <Banknote size={16} />
                    ...
                    <input
                      ...
                      inputMode="numeric"
                      placeholder="0"
                      className="w-full h-12 rounded-[16px] bg-re-bg pl-11 pr-4 font-bold text-[14px] tabular-nums outline-none border border-transparent shadow-inner focus:border-re-orange/30 focus:bg-white focus:ring-2 focus:ring-re-orange/15 transition-all"
                    />
                  </div>
                </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <label className="flex items-center gap-3 h-14 px-4 rounded-[20px] bg-re-bg border border-dashed border-black/10 cursor-pointer hover:bg-white hover:border-re-orange/30 transition-all group">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[11px] font-bold text-re-text truncate">{attachment ? attachment.name : 'Click to upload attachment'}</p>
                        <p className="text-[10px] font-medium text-re-text-muted/60 mt-0.5">{attachment ? 'Ready to send' : 'PDF or Image (Optional)'}</p>
                      </div>
    ...
    <div className="fixed inset-0 z-[240] bg-[#08111F]/60 backdrop-blur-sm p-4 flex items-center justify-center">
      <div className="w-full max-w-2xl rounded-[28px] bg-white shadow-2xl border border-black/5 overflow-hidden">
        <div className="px-5 py-4 border-b border-black/5 bg-re-bg/40 flex items-center justify-between">
          <div>
            <p className="text-[10px] font-bold text-re-text-muted">Requisition Details</p>
            <h3 className="text-sm font-bold text-re-text mt-1">{req.id}</h3>
          </div>
        ...
        <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
          <div><p className="text-[10px] text-slate-400 font-bold">Department</p><p className="font-bold text-re-text mt-1">{req.dept || '—'}</p></div>
          <div><p className="text-[10px] text-slate-400 font-bold">Requester</p><p className="font-bold text-re-text mt-1">{req.requester || '—'}</p></div>
          <div><p className="text-[10px] text-slate-400 font-bold">Amount</p><p className="font-bold text-re-text mt-1">{fmtMoney(req.amount)}</p></div>
          <div><p className="text-[10px] text-slate-400 font-bold">Submitted</p><p className="font-bold text-re-text mt-1">{fmtDate(req.submitted)}</p></div>
          <div className="sm:col-span-2"><p className="text-[10px] text-slate-400 font-bold">Items / Services</p><p className="font-bold text-re-text mt-1 whitespace-pre-wrap">{req.items || '—'}</p></div>
          <div className="sm:col-span-2"><p className="text-[10px] text-slate-400 font-bold">Description</p><p className="font-bold text-re-text mt-1 whitespace-pre-wrap">{req.description || req.note || '—'}</p></div>
          <div className="sm:col-span-2">
            <span className={`inline-block text-[10px] font-bold px-2 py-0.5 rounded-full border ${STATUS_BADGE[req.status] || 'bg-slate-50 text-slate-600 border-slate-200'}`}>
              {req.status}
            </span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[24px] shadow-xl border border-black/5 overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="py-16 text-center px-6">
              ...
              <p className="text-[11px] font-bold text-slate-400">No requisitions yet</p>
                ...
                <div key={r.db_id || r.id} className="px-5 py-4 hover:bg-re-bg/40 transition-colors">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <div className="text-right shrink-0">
                      <p className="text-lg font-bold text-re-text">{fmtMoney(r.amount)}</p>
                      <span
                      ...
                      <div className="mt-2 flex items-center justify-end gap-2">
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 h-7 px-2.5 rounded-lg border border-slate-200 text-slate-600 hover:text-re-orange hover:border-re-orange/30 hover:bg-orange-50/40 text-[10px] font-bold transition-colors"
                        >
                          <Eye size={12} /> View
                        </button>
                        <button
                          ...
                          className={`inline-flex items-center gap-1.5 h-7 px-2.5 rounded-lg text-[10px] font-bold transition-colors ${
                            r.status === 'pending'
