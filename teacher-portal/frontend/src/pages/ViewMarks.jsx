                ...
                <div className="relative z-20 max-w-[1600px] mx-auto px-6 md:px-12 pt-16 pb-24">
                    <div className="space-y-1">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-[10px] font-bold text-white/80">Performance Analytics</span>
                        </div>
                ...
                <div className="bg-white rounded-t-[2.5rem] shadow-2xl border border-black/5 overflow-hidden flex flex-col">
                    ...
                    <div className={`${!isClassSelected ? 'hidden lg:flex' : 'flex'} px-4 py-4 lg:px-3 lg:py-2 border-b border-black/5 flex flex-col lg:flex-row lg:flex-nowrap lg:items-center lg:justify-start gap-4 lg:gap-2 bg-re-bg/20`}>
                        ...
                        <div className={`${showMobileFilters ? 'flex' : 'hidden lg:flex'} flex-col lg:flex-row lg:flex-nowrap lg:items-center gap-3 lg:gap-2 w-full lg:flex-1 lg:min-w-0 animate-in slide-in-from-top-2 duration-300`}>
                            ...
                            <button
                                type="button"
                                className="w-full lg:w-auto h-10 lg:h-8 px-5 lg:px-4 bg-re-bg border border-black/[0.08] text-re-text font-bold text-[11px] lg:text-[10px] rounded-xl lg:rounded-lg hover:bg-white shadow-[inset_0_2px_6px_rgba(15,23,42,0.06)] transition-all flex items-center justify-center gap-2 shrink-0"
                            >
                    ...
                    {!isClassSelected && (
                        <div className="lg:hidden p-6 bg-re-bg/20 flex flex-col items-center justify-center text-center py-12 animate-in fade-in zoom-in-95 duration-500">
                            ...
                            <h2 className="text-xl font-bold text-re-text leading-tight mb-2">Select Class</h2>
                            <p className="text-[10px] text-re-text-muted font-bold leading-relaxed mb-8 max-w-[240px]">Choose a class to view academic performance marks.</p>
                                ...
                                {classes.map(cls => (
                                    ...
                                    >
                                        <div className="flex flex-col items-center gap-0.5">
                                            <span className="text-[11px] font-bold text-re-text group-hover:text-re-orange">{cls}</span>
                                            <span className="text-[10px] font-medium text-re-text-muted opacity-40 italic">Performance</span>
                                        </div>
                    ...
                    <div className={`${!isClassSelected ? 'hidden lg:block' : 'block'} w-full overflow-hidden`}>
                        ...
                        <div className="overflow-x-auto custom-scrollbar">
                            {isClassSelected && (
                                <div className="lg:hidden px-6 py-3 bg-white border-b border-black/5 flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-[10px] font-bold text-re-text">{selectedClass} Performance</span>
                                    </div>
                                    <button
                            ...
                            <table className="w-full text-left border-collapse min-w-full sm:min-w-[900px]">
                                <thead>
                                    <tr>
                                        <th className="bg-re-bg/50 border-b border-r border-black/5 px-2 sm:px-4 py-4 text-[10px] font-bold text-re-text-muted w-10 text-center">Rank</th>
                                        ...
                                        <th className="bg-re-bg/50 border-b border-r border-black/5 px-1 sm:px-4 py-4 text-[10px] font-bold text-re-text-muted w-12 text-center">Trend</th>
                                        {columns.map((c) => (
                                            <th
                                                key={c.slug}
                                                className="bg-re-bg/50 border-b border-r border-black/5 px-1 sm:px-3 py-4 text-[10px] font-bold text-re-text-muted w-14 text-center"
                                            >
                                                {c.label}
                                            </th>
                                        ))}
                                        <th className="bg-re-bg/50 border-b border-r border-black/5 px-1 sm:px-4 py-4 text-[10px] font-bold text-re-text-muted min-w-[120px] text-center hidden md:table-cell">Breakdown</th>
                                        <th className="bg-re-bg/50 border-b border-r border-black/5 px-4 py-4 text-[10px] font-bold text-re-text-muted w-24 text-center hidden sm:table-cell">Performance Bar</th>
                                        <th className="bg-re-bg/50 border-b border-r border-black/5 px-1 sm:px-4 py-4 text-[10px] font-bold text-re-text-muted w-16 text-center">Total</th>
                                        <th className="bg-re-bg/50 border-b border-black/5 px-1 sm:px-4 py-4 text-[10px] font-bold text-re-text-muted w-16 text-center">Grade</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-black/5">
                                    {loading ? (
                                        <tr>
                                            <td colSpan={6 + columns.length} className="p-12 text-center">
                                                ...
                                                <p className="text-[10px] font-bold text-re-text-muted">Loading marks…</p>
                                            </td>
                                        </tr>
                                    ) : columns.length === 0 ? (
                                        <tr>
                                            <td colSpan={6} className="p-10 text-center text-[10px] font-bold text-re-text-muted">
                                    ...
                                    [...filteredMarks].sort((a, b) => calculateTotal(b) - calculateTotal(a)).map((m, idx) => {
                                        ...
                                        return (
                                            <tr key={m.id} className="hover:bg-re-bg/30 transition-colors">
                                                <td className="border-r border-b border-black/5 px-2 sm:px-4 py-4 text-center text-[10px] font-bold text-re-text-muted/40 font-mono">
                                                ...
                                                <td className="border-r border-b border-black/5 px-2 sm:px-4 py-4 min-w-0 overflow-hidden">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            ...
                                                            <p className="text-[10px] font-medium text-re-text-muted italic opacity-40">{m.adm}</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                ...
                                                <td className="border-r border-b border-black/5 px-2 py-4 text-center hidden md:table-cell max-w-[200px]">
                                                    <span className="text-[10px] font-medium text-re-text-muted leading-snug block truncate" title={formatParts(m)}>
                                                        {formatParts(m)}
                                                    </span>
                                                </td>
                                                ...
                                                <td className="border-b border-black/5 px-1 sm:px-4 py-4 text-center">
                                                    <div className={`inline-flex h-7 w-7 flex items-center justify-center px-1.5 py-0.5 rounded-full text-[10px] font-bold ring-1 ring-inset ${grade.color}`}>
                                                        {grade.label}
                                                    </div>
                                                </td>
                                            </tr>
                        ...
                        <div className="px-6 py-5 bg-re-bg/10 border-t border-black/5 flex flex-col sm:flex-row items-center justify-between gap-4">
                            <div className="flex items-center gap-4 text-[10px] font-bold text-re-text-muted opacity-40 italic">
                                Report Generated: {new Date().toLocaleDateString()}
                            </div>
                            <div className="flex items-center gap-2 w-full sm:w-auto">
                                <button className="w-full sm:w-auto h-11 px-6 rounded-2xl bg-re-grad-orange text-white text-[11px] font-bold shadow-re-glow active:scale-95 transition-all">Generate Report Cards</button>
                            </div>
                        </div>
                    </div>
                </div>
