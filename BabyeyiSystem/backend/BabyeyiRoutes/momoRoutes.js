...
//  2. markFailed() helper never crashes
//  3. MOMO_CALLBACK_URL is ignored if placeholder/localhost/non-HTTPS
//  4. All DB writes wrapped so a DB error never causes a 500
//  5. Non-ASCII chars stripped from payerMessage/payeeNote
//     (MTN WAF rejects Unicode like · — é etc.)
// ================================================================
...
const CALLBACK_URL = (
  ...
  !RAW_CALLBACK.includes('placeholder')
) ? RAW_CALLBACK : '';
...
// ── Strip non-ASCII chars (MTN WAF rejects Unicode in messages) ───
// Characters like · — é à etc. cause HTTP 400 from MTN's F5 WAF
...
function mapMtnError(httpStatus, detail) {
  ...
  if (httpStatus === 400) {
    if (!detail || detail.trim() === '') {
      return 'Payment could not be processed. Please check your MoMo number and try again.';
    }
...
router.post('/request-to-pay', async (req, res) => {
  try {
    ...
    const externalId  = rawExtId ? String(rawExtId).slice(0, 100) : `pay-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`;

    // ── Strip non-ASCII from messages (MTN WAF rejects Unicode) ──
    const safePayerMsg  = toAsciiSafe(payerMessage, 'School fees payment');
    const safePayeeNote = toAsciiSafe(payeeNote,    'Babyeyi school payment');
