  ...
  return `<div id="babyeyi-pdf-auth-block" style="margin-top:20px;padding-top:8px;page-break-inside:avoid;break-inside:avoid;-webkit-column-break-inside:avoid">
    <div style="margin-bottom:18px">
      <div style="border-bottom:1.5px solid #1e3a5f;padding-bottom:5px;margin-bottom:12px">
        <span style="font-size:14px;font-weight:700;color:#1e3a5f;text-transform:uppercase;letter-spacing:0.05em">${esc(T.secAuth)}</span>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:20px;margin-top:12px">
        <div style="border:1px solid #e2e8f0;padding:14px;text-align:center;min-height:120px;box-sizing:border-box">
          <p style="font-size:11px;color:#64748b;font-weight:600;text-transform:uppercase;margin:0 0 8px">${esc(T.sigHeadTeacher)}</p>
          ...
          <p style="font-size:11px;color:#94a3b8;margin:4px 0 0">${sigB64 ? esc(T.sigSigned) : esc(T.sigRequired)}</p>
        </div>
        ...
        <div style="border:1px solid #e2e8f0;padding:14px;text-align:center;min-height:120px;box-sizing:border-box">
          <p style="font-size:11px;color:#64748b;font-weight:600;text-transform:uppercase;margin:0 0 8px">${esc(T.sigStamp)}</p>
          ...
          <p style="font-size:11px;color:#94a3b8;margin:0">${esc(T.sigCachet)}</p>
        </div>
      </div>
    </div>
    <div style="border-top:1px solid #1e3a5f;padding:8px 40px;display:flex;justify-content:space-between;align-items:center">
      <span style="font-size:11px;color:#64748b">${esc(rec.schoolName || "")} · ${esc(rec.district || "")}</span>
      <span style="font-size:11px;color:#1e3a5f;font-weight:700;text-transform:uppercase">${esc(T.docOfficial)}</span>
      <span style="font-size:11px;color:#64748b">${esc(footerLeft)} ${esc(rec.docId || "")} · ${esc(today)}</span>
    </div>
...
function buildBabyeyiDocHtml({
  ...
  const hdg = (title) => `<div style="padding-bottom:5px;margin-bottom:12px;margin-top:20px"><span style="font-size:14px;font-weight:700;color:#1e3a5f;text-transform:uppercase;letter-spacing:0.05em">${esc(title)}</span></div>`;

  const parentSection = parentMsg
    ? `<div data-babyeyi-pdf-section="parent" style="margin-bottom:22px">${hdg(T.parentMessageHeading)}<div style="padding-left:16px;margin-top:4px"><p style="font-size:12px;color:#1e293b;line-height:1.7;white-space:pre-line;margin:0">${esc(parentMsg)}</p></div></div>`
    : "";
  ...
  const otherSection = allOtherInfos.length > 0
    ? `<div data-babyeyi-pdf-section="other" style="margin-bottom:22px">${hdg(T.secOtherInfo)}<table style="${tblStyle}"><thead><tr><th style="${thS};width:42px;text-align:center">#</th><th style="${thS}">${esc(T.thItem || "Item")}</th><th style="${thS}">${esc(T.thDetails || "Details")}</th></tr></thead><tbody>${otherRows}</tbody></table></div>`
    : "";
  ...
  const notesSection = classNotes.length > 0
    ? `<div data-babyeyi-pdf-section="notes" style="margin-bottom:22px">${hdg(T.secClassNotes)}<table style="${tblStyle}"><thead><tr><th style="${thS};width:42px;text-align:center">#</th><th style="${thS}">${esc(T.thItem || "Item")}</th><th style="${thS}">${esc(T.thDetails || "Details")}</th></tr></thead><tbody>${noteRows}</tbody></table></div>`
    : "";
...
const PRINT_STYLES = `
  ...
  #babyeyi-pdf-auth-block { page-break-before: always; break-before: page; }
