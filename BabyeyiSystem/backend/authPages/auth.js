...
router.post('/login', loginLimiter, async (req, res) => {
  try {
    ...
    // UPDATED: We now allow login without school code if the user is uniquely linked to exactly one school.
    const [roleProbe] = await promisePool.query(
      `SELECT r.role_code AS role_code, sc.id AS school_id
       FROM users u
    ...
    const previouslyBlockedRoles = ['SCHOOL_ADMIN', 'SCHOOL_MANAGER', 'ACCOUNTANT', 'HOD', 'DOS'];
    
    if (previouslyBlockedRoles.includes(probeRole) && roleProbe.length > 0 && !scNorm) {
      const matchedSchoolIds = [...new Set(roleProbe.map(r => r.school_id).filter(Boolean))];
      
      // If ambiguous (multiple schools) or no school linked yet, require the code.
      // If exactly one school is found, we allow bypassing the school code requirement.
      if (matchedSchoolIds.length !== 1) {
        return res.status(400).json({
          success: false,
          code: 'SCHOOL_CODE_REQUIRED',
          message: matchedSchoolIds.length > 1 
            ? 'Your account is linked to multiple schools. Please enter your school code to sign in.'
            : 'Enter your school code (same code as in the directory, e.g. 001).',
        });
      }
    }
