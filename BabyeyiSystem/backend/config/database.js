...
// FIX 2: executeQuery uses .query() not .execute() to avoid
//         ER_WRONG_ARGUMENTS with LIMIT/OFFSET params.
// ================================================================
...
const proDbName = process.env.DB_NAME_PRO || process.env.PRO_DB_NAME || '';
...
const testConnection = async () => {
  try {
    ...
    if (proDbName) console.log(`    Pro DB: ${proDbName} (DB_NAME_PRO)`);
...
// ── Simple query helper ──────────────────────────────────────────
// IMPORTANT: uses .query() not .execute() — avoids ER_WRONG_ARGUMENTS
// when LIMIT/OFFSET params are passed (mysql2 prepared statement strict typing)
...
// ── Optional second pool: Pro feature DB (babyeyipro) — same host, different database ──
let proPool = null;
let proPromisePool = null;
if (proDbName) {
  proPool = mysql.createPool({
    host:     process.env.DB_HOST     || 'localhost',
    port:     parseInt(process.env.DB_PORT || '3306'),
    user:     process.env.DB_USER     || 'root',
    password: process.env.DB_PASSWORD || '',
  ...
  proPromisePool = proPool.promise();
  proPool.on('error', err => {
    console.error('❌  Pro DB pool error:', err.message);
  });
}
