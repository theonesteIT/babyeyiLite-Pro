#!/usr/bin/env python3
"""
Run once on the Linux server from the frontend directory:
  cd ~/index/BabyeyiSystem/frontend && python3 fix_server_build_once.py && npm run build

Fixes:
  - payments.jsx: MoMo regex uses new RegExp() (rolldown-safe), strips U+FFFD / odd chars near isValidMomoPhone
  - SuperAdminShuleAvanceOrgs.jsx: if missing, invalid UTF-8, or broken first line, writes known-good component

Does NOT use decode(errors='replace') on whole files (that corrupts source).
"""
...
PAYMENTS = ROOT / "src/Pages/Public Page/payments.jsx"
SHULE = ROOT / "src/Pages/SuperAdmin/SuperAdminShuleAvanceOrgs.jsx"

# Known-good ShuleAvance SuperAdmin page (matches backend create payload expectations)
SHULE_JSX = r"""import { useEffect, useState } from 'react';
...
def fix_payments() -> None:
    s = read_text_safe(PAYMENTS)
    s = s.replace("\r\n", "\n").replace("\r", "\n")
    s = s.replace("\u0a3d", "")  # stray Gurmukhi sign reported by rolldown
    s = s.replace("\ufffd", "")  # replacement char from bad repair scripts

    old = "return /^2507[0-9]{8}$/.test(sanitizeRwandaPhone(raw));"
    new = "return new RegExp('^2507[0-9]{8}$').test(sanitizeRwandaPhone(raw));"
    if old in s:
        s = s.replace(old, new)
        print("PATCH payments.jsx: MoMo regex -> new RegExp()")
    elif "new RegExp('^2507[0-9]{8}$')" in s:
        print("OK payments.jsx: MoMo line already uses new RegExp()")
    else:
        print("WARN payments.jsx: MoMo pattern not found — open file and search isValidMomoPhone", file=sys.stderr)

    PAYMENTS.write_text(s, encoding="utf-8", newline="\n")
    print("Wrote", PAYMENTS)


def shule_broken(s: str) -> bool:
    t = s.lstrip()
    if len(t) < 20:
        return True
    if not t.startswith("import "):
        return True
    if "export default function SuperAdminShuleAvanceOrgs" not in s:
        return True
    return False


def fix_shule() -> None:
    if not SHULE.is_file() or SHULE.stat().st_size == 0:
        SHULE.parent.mkdir(parents=True, exist_ok=True)
        SHULE.write_text(SHULE_JSX, encoding="utf-8", newline="\n")
        print("WRITE SuperAdminShuleAvanceOrgs.jsx (missing or empty)")
        return

    s = read_text_safe(SHULE)
    s = s.replace("\r\n", "\n").replace("\r", "\n")
    if shule_broken(s):
        SHULE.write_text(SHULE_JSX, encoding="utf-8", newline="\n")
        print("WRITE SuperAdminShuleAvanceOrgs.jsx (replaced broken content)")
        return

    SHULE.write_text(s, encoding="utf-8", newline="\n")
    print("OK SuperAdminShuleAvanceOrgs.jsx (UTF-8 + LF only)")


def main() -> None:
    if not PAYMENTS.is_file():
        print("ERROR: run this script from BabyeyiSystem/frontend (payments.jsx not found)", file=sys.stderr)
        sys.exit(1)
    fix_payments()
    fix_shule()
    print("Done. Next: npm run build")
