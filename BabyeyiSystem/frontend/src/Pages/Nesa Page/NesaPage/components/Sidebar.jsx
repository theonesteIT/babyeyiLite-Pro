...
import { User, Wifi, WifiOff, X } from 'lucide-react';
import { resolveUrl } from '../utils/helpers';
import LogoutButton from './LogoutButton';
import babyeyiIcon from '../../../../assets/babyeyi-icon.png';

function NavButton({ item, active, onSelect, onClose, badge }) {
...
function SidebarPanel({ tab, navConfig, switchTab, nesaUser, online, notifCount, onOpenProfile, onClose }) {
  return (
    <div
    ...
    >
      <div className="shrink-0 border-b border-white/[0.06] p-4 pb-3">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-[10px] font-medium tracking-wide text-amber-400">NESA Portal</p>
          </div>
        </div>
        <div
      ...
      <nav
        className="nesa-sidebar-scroll min-h-0 flex-1 space-y-0.5 overflow-y-auto overflow-x-hidden overscroll-y-contain px-3 py-3 pr-1"
        aria-label="NESA navigation"
      >
        ...
        {nesaUser && (
          <div className="flex items-start gap-2.5 rounded-xl border border-white/10 bg-white/[0.06] p-2.5">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-[12px] font-semibold text-white">{nesaUser.fullName}</p>
              <p className="mt-0.5 truncate text-[10px] text-amber-400/80">{nesaUser.role}</p>
            </div>
          </div>
        )}
        {onOpenProfile && (
