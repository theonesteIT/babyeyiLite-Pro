...
export default function StudentIdentityRegistrationModal({ open, onClose, session, toast, onSaved }) {
  ...
  const doSavePhotoAndContinue = useCallback(async () => {
    if (!selectedStudent) {
  ...
  }, [API, identityFingerprint, identityRemarks, identityRfid, onClose, onSaved, selectedStudent, toast, validateIdentityPayload]);
  ...
  return (
    ...
    >
      <div
        className="bg-white w-full max-w-2xl flex flex-col overflow-hidden shadow-2xl rounded-t-[1.35rem] sm:rounded-3xl h-[100dvh] max-h-[100dvh] sm:h-auto sm:max-h-[min(92vh,900px)]"
        ...
        <div className="px-4 sm:px-5 py-3 sm:py-4 flex items-center justify-between bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 shrink-0 pt-[max(0.75rem,env(safe-area-inset-top))] sm:pt-4">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <div className="w-9 h-9 rounded-2xl bg-amber-400 flex items-center justify-center shadow-lg shadow-amber-500/40 shrink-0">
              <IdCard className="w-4 h-4 sm:w-4 sm:h-4 text-slate-900" />
            </div>
            <div className="min-w-0">
              <h2 className="text-sm sm:text-base font-black text-white truncate">Student Identity Setup</h2>
              ...
              {!selectedStudent ? (
                ...
                <>
                  <div className="rounded-2xl border border-slate-200 bg-white p-3 sm:p-4">
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div className="min-w-0">
                        <p className="text-sm font-black text-slate-900">Assign student photo</p>
                        <p className="text-[11px] text-slate-500 mt-1 leading-relaxed">
                      ...
                      <div className="space-y-3">
                        {!cameraOpen ? (
                          ...
                          <div className="rounded-2xl border border-slate-200 bg-white p-3">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-[1fr,auto] gap-3 items-start mb-3">
                              <div className="min-w-0">
                                <div className="relative rounded-2xl overflow-hidden border border-slate-200 bg-slate-50">
                                  <video ref={videoRef} playsInline muted className="w-full max-h-[46vh] object-cover" />
                                  {cameraBusy && (
                                    <div className="absolute inset-0 bg-white/70 flex items-center justify-center">
                                      <Loader2 className="w-7 h-7 animate-spin text-[#B88A00]" />
                                    </div>
                                  )}
                                </div>
                              </div>
                            ...
                            <div className="flex gap-2">
                              <button
                                ...
                                className="flex-1 min-h-[48px] rounded-2xl border border-slate-200 bg-white text-sm font-bold text-slate-600 hover:bg-slate-50 touch-manipulation"
                              >
                                Cancel
                              </button>
                              <button
                                ...
                                disabled={cameraBusy}
                                className="flex-1 min-h-[48px] rounded-2xl bg-[#FEBF10] text-[#1A1200] text-sm font-black hover:bg-[#FDEAA0] disabled:opacity-60 transition-all touch-manipulation"
                              >
                                Capture
                              </button>
                            </div>
                  ...
                  <div className="rounded-2xl border border-[#FDEAA0]/70 bg-white p-3 sm:p-4">
                    <div className="flex items-start gap-3">
                      <div className="w-16 h-16 rounded-2xl overflow-hidden bg-slate-100 border border-slate-200 flex items-center justify-center shrink-0">
                        {resolveMediaUrl(selectedStudent.student_photo_url) ? (
                          <img
                            src={resolveMediaUrl(selectedStudent.student_photo_url)}
                            alt="Student"
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-black text-slate-900">{fullName || "Selected student"}</p>
                        <p className="text-[11px] font-mono text-slate-600 mt-1">
                          {selectedStudent.student_code || selectedStudent.student_uid || "—"}
                          {selectedStudent.sdm_code ? <span className="text-slate-400"> · SDMS {selectedStudent.sdm_code}</span> : null}
                        </p>
                        <p className="text-[11px] text-slate-500 mt-1">
                          Class / Department: <span className="font-semibold text-slate-700">{selectedStudent.class_name || "—"}</span>
                        </p>
                      </div>
                    </div>
                  </div>
