...
function navClass({ isActive }) {
  return [
    ...
    isActive
      ? "bg-gradient-to-br from-[#1A1200] to-[#3D2C00] text-[#FEBF10] shadow-md shadow-[rgba(26,18,0,0.18)]"
      : "text-[#7A5C00] hover:bg-[#FFF3CC]/90",
  ].join(" ");
}
...
export default function DosLayout() {
    ...
    return (
      ...
      >
        <div className="flex flex-col items-center gap-4">
          <div className="h-12 w-12 animate-spin rounded-full border-4 border-[#FDEAA0] border-t-[#FEBF10]" />
  ...
  const SidebarInner = ({ onNavigate } = {}) => (
    <div className="flex h-full flex-col" style={{ fontFamily: font }}>
      <div className="border-b px-5 py-5" style={{ borderColor: C.goldBorder }}>
        <div className="mb-3 flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="truncate text-[13px] font-black leading-tight" style={{ color: C.dark }}>
              {school?.name || "School"}
            </h1>
            <p className="truncate text-[10px] font-semibold" style={{ color: C.goldDark }}>
              Director of Study · Babyeyi
            </p>
          </div>
        </div>
        <div
      ...
      <div className="border-b px-4 py-4" style={{ borderColor: C.goldBorder }}>
        ...
        >
          <Shield className="mt-0.5 h-4 w-4 shrink-0" style={{ color: C.goldDark }} />
            ...
            {({ isActive }) => (
              <>
                <span
                  ...
                  style={{ background: isActive ? "rgba(254,191,16,0.18)" : "transparent" }}
                >
                  <Icon className="h-4 w-4 shrink-0" strokeWidth={2.2} />
                </span>
                <span className="flex-1">{label}</span>
                {isActive ? <span className="h-1.5 w-1.5 shrink-0 rounded-full" style={{ background: C.gold }} /> : null}
              </>
            )}
          </NavLink>
        ))}
  ...
  return (
    ...
    >
      ...
      <aside
        className="dos-scroll hidden lg:fixed lg:left-0 lg:top-0 lg:z-30 lg:flex lg:h-full lg:w-60 lg:flex-col xl:w-64"
        style={{
      ...
      <aside
        className={[
          "dos-scroll fixed left-0 top-0 z-50 flex h-full w-[min(100vw-2rem,18rem)] flex-col shadow-2xl transition-transform duration-300 ease-out lg:hidden",
      ...
      >
        <div className="flex items-center justify-between border-b px-4 py-4" style={{ borderColor: C.goldBorder }}>
          <div className="flex min-w-0 items-center gap-2">
            <div
              className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg"
              style={{ background: `linear-gradient(135deg, ${C.dark}, ${C.darkMid})` }}
            >
              <Shield className="h-3.5 w-3.5" style={{ color: C.gold }} />
            </div>
      ...
      >
        <header className="dos-scroll sticky top-0 z-20 flex h-14 shrink-0 items-center gap-3 border-b border-slate-200/80 bg-white/95 px-3 backdrop-blur-md shadow-sm lg:hidden">
          <button
            type="button"
          ...
          <div
            ...
            style={{ background: `linear-gradient(135deg, ${C.dark}, ${C.darkMid})` }}
          >
            <FileBarChart className="h-4 w-4 text-[#FEBF10]" />
          </div>
        </header>

        <main className="dos-scroll flex-1 px-3 py-4 sm:px-5 sm:py-6 lg:px-8 lg:py-8">
