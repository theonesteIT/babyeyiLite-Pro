...
import { User, Phone, Mail, LogOut, ChevronRight, Lock, AlertCircle, CheckCircle2 } from "lucide-react";
...
export default function Profile() {
  ...
  const email = auth.user?.email || auth.user?.father_email || auth.user?.mother_email || null;
  const needsCompleteRegistration = !!auth.user?.phone_only_registration_required;
  ...
  const submitCompleteRegistration = async (e) => {
    ...
    try {
      ...
      setOkMsg("Registration completed. You can now log in with your password next time.");
      await auth.refresh();
    } catch {
  ...
  return (
    <div className="space-y-6 pb-4 max-w-lg mx-auto">
      <div className="flex flex-col items-center text-center pt-2">
        ...
        <h1 className="text-xl font-extrabold text-slate-900 mt-4">{displayName}</h1>
      ...
      <div className="rounded-2xl border border-slate-100 bg-white shadow-sm overflow-hidden divide-y divide-slate-100">
        ...
        {hasFullPortal && (
          <div className="flex items-center gap-4 p-4">
            ...
            <div className="flex-1 min-w-0 text-left">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wide">Recovery email</p>
              <p className="font-semibold text-slate-900 truncate">
                {auth.user?.recovery_email || "Not set — add below to reset your phone by email"}
              </p>
            </div>
          </div>
        )}
      ...
      {needsCompleteRegistration && (
        ...
        >
          <p className="text-sm font-extrabold text-amber-900 mb-1">Complete registration</p>
          ...
          <form onSubmit={submitCompleteRegistration} className="space-y-3">
            <label className="block">
              <span className="text-xs font-bold text-amber-900 uppercase tracking-wide">
                Recovery email <span className="text-amber-700/70 font-semibold normal-case">(optional)</span>
              </span>
              <input
                type="email"
                autoComplete="email"
            ...
            <label className="block">
              <span className="text-xs font-bold text-amber-900 uppercase tracking-wide">Password</span>
            ...
            <label className="block">
              <span className="text-xs font-bold text-amber-900 uppercase tracking-wide">Confirm password</span>
              <input
                type="password"
      ...
      {hasFullPortal && !needsCompleteRegistration && (
        <section className="rounded-2xl border border-slate-100 bg-white p-4 sm:p-5 shadow-sm">
          <p className="text-sm font-extrabold text-slate-900 mb-1">Recovery email</p>
          <p className="text-sm text-slate-600 mb-4">
            Used only to send you a secure link if you change or lose your phone. You can update it anytime.
          </p>
          {recoveryErr && (
            <div className="mb-3 flex items-start gap-2 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">
              <AlertCircle className="w-4 h-4 mt-0.5 shrink-0" />
          ...
          <p className="mt-3 text-xs text-slate-400">
            Reset flow:{" "}
            <Link to="/parents/reset-phone" className="font-semibold text-orange-600 hover:text-orange-700">
              /parents/reset-phone
            </Link>
          </p>
        </section>
      ...
      <Link
        to="/"
        className="flex items-center justify-between rounded-2xl border border-slate-100 bg-white p-4 shadow-sm font-semibold text-slate-800 hover:border-orange-200 transition-colors"
      >
        Visit babyeyi.rw
      ...
      <p className="text-center text-xs text-slate-400 hidden lg:block">
        Use <strong className="text-slate-600">Log out</strong> in the sidebar to sign out.
      </p>
    </div>
  );
}
