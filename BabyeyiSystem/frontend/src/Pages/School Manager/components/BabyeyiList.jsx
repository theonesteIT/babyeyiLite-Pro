...
// #000435 navy + amber-400 · · Tailwind only · Mobile-first
// ================================================================
...
import {
  BABYEYI_AUTO_LANG_OPTIONS,
  CORE_BABYEYI_LANG_OPTIONS,
  isCoreBabyeyiLang,
  normalizeBabyeyiLang,
} from '../schoolLiteSupport/babyeyiTranslateLangs.js';
...
const CORE_LANG_OPTIONS = CORE_BABYEYI_LANG_OPTIONS;
...
function buildWordDocHeaderMetaHtml({ T, rec, levelLabel, classesArr }) {
  ...
  const docIdHtml = rec.docId
    ...
    : "";
  const classHtml = buildBabyeyiDocumentClassHeaderHtml(classesArr, T.classLabel || "Class");
...
//  Official doc modal (also embedded on public verify page)
      ...
      <div className={embedded ? "w-full" : "w-full max-w-3xl my-4"}>
        ...
        <div className={`bg-[#000435] border-2 border-amber-400/30 px-3 sm:px-4 py-3 flex items-center gap-2 flex-wrap ${embedded ? "rounded-2xl mb-3" : "rounded-t-2xl"}`}>
          ...
          <button onClick={onClose} className="flex items-center gap-1 px-3 py-1.5 bg-white/8 border border-white/15 hover:bg-white/14 text-white rounded-xl text-[11px] font-bold shrink-0">
            {publicVerify ? (T.closeBtn || "Close") : (T.backBtn || "Back")}
          </button>
          )}
        ...
        <div className="relative bg-white shadow-sm rounded-b-2xl overflow-hidden" style={{ fontFamily: "Georgia,'Times New Roman',serif" }}>
          ...
          <div id="babyeyi-pdf-body" style={{ padding: "20px 40px 28px" }}>
            ...
            {banks.length > 0 && (
              <div style={DOC.section}>
                ...
                <table style={tblStyle}>
                  ...
                  <tbody>{banks.map((bk, i) => (<tr key={i}><Td center color="#64748b">{i + 1}</Td><Td bold>{bk.bankName || "â€”"}</Td><Td mono>{bk.accountNumber || "â€”"}</Td><Td>{bk.accountName || "â€”"}</Td><Td center color="#059669" bold>{bk.isPrimary || i === 0 ? "" : ""}</Td></tr>))}</tbody>
                </table>
              </div>
            )}
            {reqs.length > 0 && (
            ...
            <div id="babyeyi-pdf-auth-block" style={{ marginTop: "14px", pageBreakInside: "avoid", breakInside: "avoid", pageBreakBefore: "avoid", breakBefore: "avoid" }}>
              ...
              <div style={{ borderTop: "1px solid #1e3a5f", padding: "6px 0", marginTop: "10px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
...
// Data mapping  API list row → card summary record
function mapRow(row) {
  let paymentsArr = [];
  try {
  ...
  const totalFee =
    row.total_fee ?? row.total_amount ?? paymentsArr.reduce((s, p) => s + Number(p.amount || 0), 0);
  let classes = [];
  try {
    if (row.classes_json) {
...
export default function BabyeyiList({ session, lang: dashboardLang, setLang: setDashboardLang }) {
  ...
  const dashboardLangSync =
    typeof setDashboardLang === "function" &&
    dashboardLang != null &&
    String(dashboardLang).trim() !== "";

  const [lang, setLang] = useState(() => {
    try {
      const raw = localStorage.getItem("babyeyi_lang");
      if (raw != null && String(raw).trim() !== "") {
        return normalizeBabyeyiLang(raw);
      }
      if (dashboardLang != null && String(dashboardLang).trim() !== "") {
        return normalizeBabyeyiLang(dashboardLang);
      }
    } catch {
      /* use fallthrough */
    }
  ...
  const prevDashboardLang = useRef();
  useEffect(() => {
    if (!dashboardLangSync) return;
    const next = normalizeBabyeyiLang(dashboardLang);
    if (prevDashboardLang.current === undefined) {
      prevDashboardLang.current = next;
      return;
    }
    if (prevDashboardLang.current === next) return;
    prevDashboardLang.current = next;
    setLang(next);
    try {
      localStorage.setItem("babyeyi_lang", next);
    } catch {}
  }, [dashboardLang, dashboardLangSync]);
  ...
  const handleLangChange = useCallback(
    (newLang) => {
      ...
      if (dashboardLangSync && isCoreBabyeyiLang(n)) {
        setDashboardLang(n);
        prevDashboardLang.current = n;
      }
    },
    [dashboardLangSync, setDashboardLang]
  );
