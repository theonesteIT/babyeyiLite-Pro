...
export default function StaffIdentityModal({ open, onClose, staffList, creatorRole, toast }) {
  ...
  const filtered = useMemo(() => {
    ...
    return base.filter((s) => {
      ...
      return name.includes(q) || em.includes(q) || String(s.staff_id || "").toLowerCase().includes(q);
    });
            ...
            <div className="space-y-4">
              <p className="text-sm text-gray-600">
                {String(creatorRole || "").toUpperCase() === "DOS"
                  ? "Search a teacher at your school by name or email."
                  : "Search a teacher or staff member by name or email."}
              </p>
