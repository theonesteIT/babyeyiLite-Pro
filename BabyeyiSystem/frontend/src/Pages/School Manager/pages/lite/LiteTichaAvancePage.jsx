...
export default function LiteTichaAvancePage() {
   const navigate = useNavigate();
   const { user } = useAuth();
   const staff = user && user !== false ? user : null;
   ...
   const load = useCallback(async (silent) => {
      ...
      try {
         const [reqRes, payrollRes, eligRes] = await Promise.all([
         ...
         const eligData = eligRes?.data?.success ? eligRes.data.data : null;
         ...
         } else if (eligData?.net_salary_rwf) {
            const net = Number(eligData.net_salary_rwf) || 0;
            setPayrollSalary({ net, gross: net, allowancesTotal: 0, basic: net });
         } else {
   ...
   }, [loadCatalog, loadTeacherDealProducts, staff?.id]);
   ...
   /** School manager policy % — used for monthly cap and instant cashout auto-approval (same ratio). */
   const cashoutAutoPolicy = useMemo(() => {
      const pct = Number(eligibility?.max_percent) || 25;
      const net = Number(payrollSalary?.net ?? eligibility?.net_salary_rwf) || 0;
      const limit =
         Number(eligibility?.auto_approval_limit_rwf) ||
         Math.floor(net * (pct / 100));
      const remainingRaw = eligibility?.auto_approval_remaining_rwf;
      const remaining =
         Number.isFinite(Number(remainingRaw)) && Number(remainingRaw) >= 0
            ? Number(remainingRaw)
            : limit;
      return { pct, net, limit, remaining };
   }, [eligibility, payrollSalary?.net]);
            ...
            if (res.data.auto_approved) {
               setStatusAlerts((prev) => {
                  ...
                  return [{
                     ...
                     message: res.data.message || `Your cashout of ${formatMoney(amt)} was instantly approved and will be deducted from your next payroll.`,
                  }, ...prev].slice(0, 8);
               });
            } else if (res.data.exceeds_monthly_cap || res.data.requires_manager_approval) {
               setStatusAlerts((prev) => {
                  const key = `manager-review-${Date.now()}`;
                  return [{
                     key,
                     variant: 'info',
                     title: 'Sent to school manager',
                     message: res.data.message || 'Your request was submitted for school manager approval.',
                  }, ...prev].slice(0, 8);
               });
            }
            closeFlow();
   ...
   if ((loading || eligibilityLoading) && !rows.length) return (
   ...
   if (eligibility && eligibility.allowed === false) {
      return (
         <div className="min-h-screen bg-re-bg flex items-center justify-center p-6">
            <div className="max-w-md w-full rounded-[32px] bg-white border border-black/5 shadow-xl p-8 text-center">
               <div className="w-16 h-16 rounded-2xl bg-red-50 text-red-600 flex items-center justify-center mx-auto mb-4">
                  <AlertCircle size={32} />
               </div>
               <h1 className="text-xl font-black text-re-text mb-2">Shule Avance unavailable</h1>
   ...
   const renderRequestButton = (isMobile) => {
      const trigger = () => {
         ...
         setSelectedBillKey('cashout');
         setApplyMode('direct');
         setShowApplyModal(true);
         setCashoutReason('Emergency Cashout');
      };
               ...
               <div className="flex items-center gap-4">
                  ...
                  <div className="text-left">
                     ...
                     <p className="text-[10px] font-bold text-re-text-muted opacity-60 uppercase">
                        {cashoutAutoPolicy.limit > 0
                           ? `≤ ${formatMoney(cashoutAutoPolicy.remaining)} auto-approved (${cashoutAutoPolicy.pct}% cap)`
                           : `Instant approval within ${cashoutAutoPolicy.pct}% of net salary`}
                     </p>
                  </div>
               </div>
   ...
   return (
      <div className="animate-in fade-in duration-700 bg-re-bg min-h-screen">
         ...
         {eligibility?.allowed && (
            <div className="max-w-[1600px] mx-auto px-4 md:px-12 pt-4">
                           ...
                           {dealRequests.length === 0 ? (
                              <div className="flex flex-col items-center justify-center py-14 gap-3">
                                 ...
                                 <div className="text-center">
                                    ...
                                    <button
                                       type="button"
                                       onClick={() => navigate('/lite/shule-avance/deals')}
                                       className="mt-3 inline-flex items-center gap-1.5 rounded-xl bg-re-orange px-4 py-2 text-[10px] font-black uppercase tracking-widest text-white shadow-md hover:opacity-95"
                                    >
                                       Browse TichaDeals <ChevronRight size={12} />
                                    </button>
                                 </div>
                              </div>
                           ) : (
                        ...
                        <div className="space-y-6 animate-in slide-in-from-right-4">
                           {wizardStep === 1 && (
                              <div className="space-y-6 py-4">
                                 <div className="space-y-2">
                                    ...
                                    {/* School % auto-approval badge */}
                                    {(() => {
                                       const { pct, net, limit, remaining } = cashoutAutoPolicy;
                                       const enteredAmt = Number(amount) || 0;
                                       if (!enteredAmt || (!net && !limit)) return null;
                                       const isAuto = enteredAmt <= remaining && enteredAmt <= (eligibility?.remaining_cap_rwf ?? Infinity);
                                       const exceedsMonthly = eligibility?.remaining_cap_rwf != null && enteredAmt > eligibility.remaining_cap_rwf;
                                       if (exceedsMonthly) {
                                          return (
                                             <div className="flex items-center gap-2 px-3 py-2 rounded-xl border text-[10px] font-bold mt-1 bg-amber-50 border-amber-200 text-amber-900">
                                                <Send size={13} className="shrink-0 text-amber-700" />
                                                Over your {pct}% monthly limit ({formatMoney(eligibility.remaining_cap_rwf)} remaining) — sent to your school manager for approval.
                                             </div>
                                          );
                                       }
                                       return (
                                          <div className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-[10px] font-bold mt-1 ${isAuto ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-amber-50 border-amber-200 text-amber-800'}`}>
                                             ...
                                             {isAuto
                                                ? `Auto-approved — within ${pct}% of your net salary (${formatMoney(limit)} limit, ${formatMoney(remaining)} left this month)`
                                                : `Requires review — over ${pct}% instant limit (${formatMoney(limit)}). Sent to your school manager.`
                                             }
                                          </div>
                                       );
                                    })()}
                           ...
                           {wizardStep === 2 && (
                              <div className="space-y-4 py-2">
                                 ...
                                 {(() => {
                                    const { pct, remaining } = cashoutAutoPolicy;
                                    const enteredAmt = Number(amount) || 0;
                                    const exceedsMonthly =
                                       eligibility?.remaining_cap_rwf != null && enteredAmt > eligibility.remaining_cap_rwf;
                                    const isAuto =
                                       !exceedsMonthly
                                       && enteredAmt > 0
                                       && enteredAmt <= remaining
                                       && (eligibility?.remaining_cap_rwf == null || enteredAmt <= eligibility.remaining_cap_rwf);
                                    return (
                                       <div className={`flex items-center gap-2 px-4 py-3 rounded-2xl border text-[10px] font-bold ${isAuto ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-amber-50 border-amber-200 text-amber-800'}`}>
                                          ...
                                          <span>
                                             {isAuto
                                                ? 'Instant approval — deducted from your next payroll.'
                                                : exceedsMonthly
                                                  ? `Over ${pct}% monthly limit — your school manager must approve or reject this request.`
                                                  : 'This request will be sent to your school manager for approval.'}
                                          </span>
                                       </div>
                                    );
                                 })()}
