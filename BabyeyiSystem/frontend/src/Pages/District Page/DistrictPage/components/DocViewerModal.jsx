...
import { fileKind } from '../utils/helpers';
...
export default function DocViewerModal({ url, title, onClose }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          throw new Error(`Document could not be loaded (${res.status})`);
        }
  ...
  const downloadHref = blobUrl || url;

  return (
    <div
      className="fixed inset-0 z-[150] flex items-end justify-center bg-[#000435]/70 p-0 backdrop-blur-sm sm:items-center sm:p-4"
      ...
      <div className="flex max-h-[100dvh] w-full flex-col overflow-hidden rounded-t-2xl border border-[#fde68a] bg-white shadow-2xl sm:max-h-[92vh] sm:max-w-3xl sm:rounded-2xl">
        ...
        <div className="flex shrink-0 flex-wrap items-center justify-between gap-2 border-b border-[#fde68a] bg-[#fffbeb] px-4 py-3 sm:px-5">
          <div className="flex min-w-0 flex-1 items-center gap-2">
            ...
            <h3 className="m-0 truncate text-sm font-bold text-[#000435]">
              {title || 'Document'}
            </h3>
          </div>
          <div className="flex shrink-0 items-center gap-2">
            ...
            <button
              type="button"
              onClick={onClose}
              className="flex h-9 w-9 items-center justify-center rounded-xl border-none bg-[#000435]/5 text-[#000435] cursor-pointer hover:bg-[#000435]/10"
