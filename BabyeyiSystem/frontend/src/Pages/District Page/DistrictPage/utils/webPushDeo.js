/**
 * Web Push for DEO District Babyeyi portal.
 */
...
export async function subscribeDeoPush() {
  if (!isWebPushEnvironmentSupported()) {
  ...
  await apiFetch('/district/babyeyi/push/subscribe', {
    method: 'POST',
...
export async function unsubscribeDeoPush() {
  const reg = await navigator.serviceWorker.getRegistration();
  if (!reg) return;
  ...
  try {
    await apiFetch('/district/babyeyi/push/unsubscribe', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
