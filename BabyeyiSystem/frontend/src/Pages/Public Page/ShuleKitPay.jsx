...
/** Legacy rows may use requirement id 0 — only null/undefined means missing. */
          ...
          <div key={s.id} className="flex items-center flex-1 min-w-0">
            <div className="flex flex-col items-center gap-1.5 flex-shrink-0">
              <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center transition-all duration-300 border-2 ${
                done ? "bg-amber-400 border-amber-400 text-white shadow-md shadow-amber-200"
                  : active ? "bg-white border-amber-400 text-amber-500 shadow-sm"
...
function SummaryCard({ label, value, sub, highlight, green }) {
  return (
    <div className={`rounded-xl p-3.5 sm:p-4 border ${
      highlight ? "bg-amber-50 border-amber-200" : green ? "bg-emerald-50 border-emerald-200" : "bg-gray-50 border-gray-200"
    }`}>
      <p className="text-[9px] font-black uppercase tracking-[.1em] text-gray-400 mb-1">{label}</p>
      <p className={`text-[16px] sm:text-[18px] font-black leading-none ${highlight ? "text-amber-500" : green ? "text-emerald-600" : "text-[#000435]"}`}>{value}</p>
    ...
    <div className="flex items-center gap-3 pt-5 mt-1 border-t border-gray-100">
      ...
      <button type="button" onClick={onNext} disabled={nextDisabled || nextLoading}
        className={`flex-1 sm:flex-none sm:ml-auto flex items-center justify-center gap-2 px-6 py-3 rounded-xl font-black text-[14px] min-h-[48px] transition-all ${
          nextDisabled || nextLoading ? "bg-gray-100 text-gray-400 cursor-not-allowed"
...
export default function ShuleKitPay() {
  ...
  useEffect(() => {
    ...
    if (!remain && targetStep !== 4) return;
    remainDeepLinkDone.current = true;
    if (remain) setAmountInput(String(remain));
    if (targetStep === 4) goStep(4);
  }, [catalog, pricingData, student, searchParams, goStep]);
  ...
  useEffect(() => {
    ...
    fetch(`${API}/public/babyeyi-pay/pricing/${selectedCombo.babyeyi_id}?school_id=${encodeURIComponent(school.id)}`)
      .then(r => r.json()).then(j => {
        ...
        setSchoolCounterPaidByFeeId({});
        setFeeSel(new Set());
        setReqSel(new Set(j.data.requirements?.map((x) => normReqId(x.babyeyi_requirement_id)).filter((x) => x != null) || []));
  ...
  const effectiveReqIds = useMemo(() => {
    ...
    return Array.from(reqSel).map((x) => normReqId(x)).filter((x) => x != null);
  }, [payScope, reqSel]);
  ...
  const allocationNote = useMemo(() => {
    ...
    if (payScope === "combined" && grand > 0 && reqTotal > 0 && feeTotal > 0) {
    ...
    return null;
  }, [pricingData, enteredAmount, reqTotal, feeTotal, payScope, grand]);
  ...
  const { totalDue: selectionDueRwF, remainingBalance: remainingOwedDisplay, alreadyPaid: alreadyPaidRwF, payCap } =
    resolvePublicPayTotals({ grand: reqTotal, balanceQuote });
  ...
  const remainingFullDocumentAfterCurrentPayment = remainingFullDocument != null
    ...
    : null;
  const overpays = enteredAmount > payCap + 1.5;
  const amountOverSel = enteredAmount > payCap + 1.5;
  const minPayAmountPartial = 100;
  const amountValid = enteredAmount + 1e-6 >= minPayAmountPartial && !amountOverSel && enteredAmount <= payCap + 1.5;
  ...
  const continueToPayment = () => {
    setPayErr("");
    if (!school || !hasBabyeyiId(selectedCombo?.babyeyi_id) || !pricingData) { setPayErr("Load term, year, and requirements first."); return; }
    if (!amountValid) {
      setPayErr(amountStepError || `Enter at least ${minPayAmountPartial.toLocaleString()} RWF.`);
      return;
    }
  ...
  useEffect(() => {
    ...
    if (!d?.fromCustomShuleKit || !d.babyeyiId || !combinations.length || !student) return;
  ...
  useEffect(() => {
    ...
    if (!d?.fromCustomShuleKit || !pricingData || !selectedCombo) return;
    if (Number(d.babyeyiId) !== Number(selectedCombo.babyeyi_id)) return;
    if (Number(d.schoolId) !== Number(school?.id)) return;
    setReqSel(new Set((d.selectedReqIds || []).map(String).filter(Boolean)));
    ...
    <div className="min-h-screen bg-gray-50" style={{ fontFamily: FONT }}>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 h-14 sm:h-16 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 ml-1">
            <div className="w-8 h-8 rounded-xl bg-amber-400 flex items-center justify-center shadow-sm shadow-amber-200">
              <Package size={15} className="text-[#000435]"/>
            </div>
          </div>
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
        <div className="mb-7 sm:mb-8 text-center">
          <div className="inline-flex items-center gap-1.5 rounded-full bg-amber-50 border border-amber-200 px-3.5 py-1.5 mb-4">
            ...
            <span className="text-[10px] font-black uppercase tracking-[.12em] text-amber-600">
              {t("publicPay.requirementsOnly", { defaultValue: "Requirements Only" })}
            </span>
          </div>
          <h1 className="font-black text-[#000435] text-[26px] sm:text-[30px] xl:text-[34px] tracking-tight leading-tight mb-2">
            {t("publicPay.payForSchoolKit", { defaultValue: "Pay for SchoolKit by" })}&nbsp;
          ...
          <p className="text-gray-400 text-[13px] font-semibold">
            {t("publicPay.requirementsHeroSub", { defaultValue: "Pay school kit and Babyeyi requirements only" })}
          </p>
        </div>
        ...
        <div className="rounded-2xl bg-white border border-gray-100 overflow-hidden shadow-lg">
          ...
          <div className="px-5 sm:px-8 py-6">
                    ...
                    <div className="flex items-start gap-3 p-4 rounded-xl border border-gray-100 bg-gray-50">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[12px] text-gray-500 font-semibold mt-1">
                          ...
                          {student.academic_year ? <span className="text-gray-400"> · Record year {student.academic_year}</span> : null}
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <div key={stepKey} className="step-in">
                <div className="mb-6">
                  ...
                  <p className="text-gray-400 text-[13px]">
                    Requirements are for <span className="text-amber-600 font-bold">{student.class_name || "your class"}</span> at {school.school_name}.
                ...
                {yearPick && termPick && matchingComboIndices.length === 0 && (
                  <div className="rounded-xl border border-amber-200 bg-amber-50 px-3.5 py-3 text-[12px] text-amber-900 font-semibold leading-relaxed">
                    The school has not published a Babyeyi document for this class, term, and year yet. Please contact the school office or try another term/year.
                  </div>
                )}
            ...
            {step === 3 && school && (
              <div key={stepKey} className="step-in">
                <div className="mb-5">
                  <h2 className="font-black text-[#000435] text-[18px] sm:text-[20px] mb-1">School kit requirements</h2>
                  <p className="text-gray-400 text-[13px]">
                    Items listed here come from your school&apos;s Babyeyi document for <span className="text-amber-600 font-bold">{student?.class_name || "your class"}</span>, this term, and academic year. Select what you need; total updates instantly.
                  </p>
                </div>
                ...
                {pricingLoading && (
                  <div className="flex flex-col items-center justify-center py-12 gap-3">
                    ...
                    <p className="text-gray-400 text-[13px] font-semibold">Loading requirements…</p>
                  </div>
                )}
                ...
                {!pricingLoading && !pricingErr && !hasBabyeyiId(selectedCombo?.babyeyi_id) && (
                  <div className="flex items-start gap-2.5 p-4 rounded-xl border border-amber-200 bg-amber-50 text-amber-900 text-[13px] font-semibold leading-relaxed">
                    ...
                    No Babyeyi document is available for this student&apos;s class and selected term. Please contact the school office.
                  </div>
                )}
                ...
                {!pricingLoading && !pricingErr && pricingData && (
                  <>
                    ...
                    {(pricingData.requirements || []).length > 0 && (
                      <div className="mb-5">
                        ...
                        <div className="space-y-2">
                          {pricingData.requirements.map((r, reqIdx) => {
                            ...
                            return (
                              <div
                                key={`${String(r.babyeyi_requirement_id)}-${r.requirement_name || reqIdx}-${reqIdx}`}
                    ...
                    {(pricingData.requirements || []).length === 0 && (
                      <div className="rounded-xl border border-amber-200 bg-amber-50 px-3.5 py-3 text-[12px] text-amber-900 font-semibold">
                    ...
                    <div className="p-4 rounded-xl border-2 border-amber-300 bg-amber-50">
                      <div className="flex items-center justify-between">
                        <p className="font-black text-[#000435] text-[15px]">Selected kit total</p>
                        <p className="font-black text-amber-500 text-[22px] font-mono">{reqTotal.toLocaleString()} <span className="text-[14px]">RWF</span></p>
                      </div>
                    </div>
                  </>
                )}
                ...
                <NavBtns
                  onBack={() => goStep(2)}
                  onNext={() => { if (pricingData && reqSel.size > 0) goStep(4); }}
                  nextLabel="Set Amount"
                  nextDisabled={!pricingData || pricingLoading || !!pricingErr || reqSel.size === 0}
                />
              </div>
            )}
            ...
            {step === 4 && pricingData && (
              <div key={stepKey} className="step-in">
                <div className="mb-5">
                  <h2 className="font-black text-[#000435] text-[18px] sm:text-[20px] mb-1.5">{t("publicPay.paymentAmount", { defaultValue: "Payment amount" })}</h2>
                  <p className="text-gray-400 text-[13px]">{t("publicPay.requirementsAmountSub", { defaultValue: "Pay any amount toward selected requirements — balance updates automatically." })}</p>
                </div>

                <PublicPayAmountCard
                  cardTitle={t("publicPay.schoolKitCard", { defaultValue: "School Kit" })}
                  student={student}
                  school={school}
                  ...
                  payCap={payCap}
                  onPayFullRemaining={() => {
                    if (remainingOwed != null && remainingOwed > 0) setAmountInput(String(remainingOwed));
                    else setAmountInput(String(Math.round(reqTotal * 100) / 100));
                  }}
                />

                {balanceErr ? <div className="mt-3 text-[12px] text-red-500 font-semibold">{balanceErr}</div> : null}

                <NavBtns
                  onBack={() => goStep(3)}
                  onNext={() => { if (amountValid) goStep(5); }}
                  nextLabel={t("publicPay.reviewStep", { defaultValue: "Review" })}
            ...
            {step === 6 && pricingData && amountValid && student && !classMismatch && (
              <div key={stepKey} className="step-in">
                ...
                <button type="button" onClick={continueToPayment} disabled={balanceLoading || classMismatch || overpays}
                  className={`w-full flex items-center justify-center gap-2.5 py-4 rounded-xl font-black text-[15px] transition-all min-h-[56px] ${
                    balanceLoading || classMismatch || overpays
        ...
        <div className="text-center mt-6 text-[12px] text-gray-400 font-semibold">
          ...
          }} className="text-amber-600 hover:text-amber-700 transition-colors font-bold">Start over</button>
          {" · "}
          <Link to="/schools" className="text-amber-600 hover:text-amber-700 transition-colors font-bold">Browse schools</Link>
        </div>
      </div>
