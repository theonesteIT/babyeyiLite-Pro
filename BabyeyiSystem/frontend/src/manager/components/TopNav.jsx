...
import { Menu, Search, Bell, ChevronDown, LogOut, Settings, User, ArrowLeft } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '../context/AuthContext';
import { h } from '../utils/href';
import { resolveUserPhotoUrl } from '../../shared/utils/userPhotoUrl';

const TopNav = ({ title, onMenuClick, showBack = false, hideMenu = false }) => {
        ...
        <header className="h-14 sm:h-[3.75rem] flex items-center gap-3 sm:gap-4 px-3 sm:px-5 md:px-7 bg-white border-b border-slate-200/90 shadow-[0_1px_0_rgba(15,23,42,0.06)] sticky top-0 z-30 shrink-0 font-sans transition-all duration-300">
            ...
            <div className="flex items-center gap-2 sm:gap-3 shrink-0 min-w-0">
                {showBack ? (
                  <button
                    type="button"
                    onClick={() => navigate(h('/finance'))}
                    className="flex h-10 sm:h-11 items-center gap-2 px-3 sm:px-4 rounded-xl bg-re-gold text-re-navy shadow-sm border border-[#d4a20a]/30 hover:bg-re-gold-light active:scale-[0.97] transition-all font-bold text-[12px] sm:text-[13px] uppercase tracking-wide shrink-0"
                    aria-label="Back to finance center"
                  >
