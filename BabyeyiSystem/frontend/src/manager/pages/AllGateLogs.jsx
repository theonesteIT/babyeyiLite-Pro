...
import { Search, CalendarRange, ShieldCheck, Clock3, GraduationCap, BookOpen } from 'lucide-react';
  ...
  return (
    <div className="space-y-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-slate-900">All Gate Logs</h2>
          <p className="text-xs font-semibold text-slate-500">Full attendance history with advanced filters.</p>
        </div>
        <div className="rounded-lg bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-700">{pagination.total} records</div>
      </div>

      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <label className="rounded-xl border border-slate-200 bg-white p-2">
          <span className="mb-1 inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-widest text-slate-500"><CalendarRange size={12} />From Date</span>
        ...
        <label className="rounded-xl border border-slate-200 bg-white p-2">
          <span className="mb-1 inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-widest text-slate-500"><CalendarRange size={12} />To Date</span>
        ...
        <label className="rounded-xl border border-slate-200 bg-white p-2">
          <span className="mb-1 inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-widest text-slate-500"><ShieldCheck size={12} />Role</span>
        ...
        <label className="rounded-xl border border-slate-200 bg-white p-2">
          <span className="mb-1 inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-widest text-slate-500"><Clock3 size={12} />Session</span>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <label className="rounded-xl border border-slate-200 bg-white p-2 lg:col-span-2">
          <span className="mb-1 inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-widest text-slate-500"><Search size={12} />Search</span>
        ...
        <label className="rounded-xl border border-slate-200 bg-white p-2">
          <span className="mb-1 inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-widest text-slate-500"><GraduationCap size={12} />Academic Year</span>
        ...
        <label className="rounded-xl border border-slate-200 bg-white p-2">
          <span className="mb-1 inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-widest text-slate-500"><BookOpen size={12} />Term</span>
            ...
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                {['Date', 'Name', 'Role', 'Card UID', 'Ref ID', 'Morning', 'Evening', 'Status', 'Term', 'Academic Year'].map((h) => (
                  <th key={h} className="px-3 py-3 text-left text-[10px] font-semibold uppercase tracking-widest text-slate-600">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {loading && (
                <tr><td colSpan={10} className="px-4 py-16 text-center">
                  <div className="flex flex-col items-center gap-3">
                    <div className="w-8 h-8 rounded-xl border-2 border-amber-400 border-t-transparent animate-spin" />
              ...
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.id} className="border-b border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="px-3 py-2"><span className={`rounded-full px-2 py-1 text-[10px] font-semibold ${statusClass}`}>{status}</span></td>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="text-xs font-semibold text-slate-500">Page {pagination.page} of {pagination.total_pages}</div>
        <div className="flex items-center gap-2">
          <button type="button" disabled={pagination.page <= 1 || loading} onClick={() => setPagination((p) => ({ ...p, page: p.page - 1 }))} className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 disabled:opacity-40">Previous</button>
          <button type="button" disabled={pagination.page >= pagination.total_pages || loading} onClick={() => setPagination((p) => ({ ...p, page: p.page + 1 }))} className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 disabled:opacity-40">Next</button>
        </div>
      </div>
    </div>
  );
