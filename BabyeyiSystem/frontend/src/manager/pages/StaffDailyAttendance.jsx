                    ...
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="text-[9px] font-semibold uppercase tracking-[0.3em] text-[#FEBF10]">Staff Roll Call</span>
                            </div>
            ...
            <div className="relative z-30 max-w-[1600px] mx-auto px-2 md:px-6 -mt-14">
                <div className="bg-white rounded-t-[2.5rem] shadow-sm border border-black/10 overflow-hidden">
                    ...
                    <div className="grid grid-cols-3 md:grid-cols-6 divide-x divide-y md:divide-y-0 divide-black/5 border-b border-black/5">
                        {[
                            ...
                            <div key={i} className="p-3 sm:p-5 flex flex-col items-center justify-center text-center group hover:bg-slate-50 transition-all">
                                ...
                                <div className="text-base sm:text-xl font-semibold text-slate-800 tracking-tighter group-hover:text-[#1E3A5F]">{s.value}</div>
                                <div className="text-[7px] font-semibold text-slate-400 uppercase tracking-[0.18em] mt-0.5">{s.label}</div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="px-4 pt-3 pb-0 border-b border-black/5 bg-slate-50/50 flex flex-col gap-3">
                        ...
                        <div className="flex items-center gap-1 bg-slate-100 rounded-xl p-1 w-fit">
                            <button
                                ...
                                className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-[9px] font-semibold uppercase tracking-widest transition-all ${activeTab === 'morning' ? 'bg-white text-[#1E3A5F] shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                            >
                                <Sun size={11} /> Morning
                            </button>
                            <button
                                onClick={() => setActiveTab('evening')}
                                className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-[9px] font-semibold uppercase tracking-widest transition-all ${activeTab === 'evening' ? 'bg-white text-[#1E3A5F] shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                            >
                        ...
                        <div className="flex flex-col lg:flex-row items-center gap-3 pb-3">
                            ...
                            {activeTab === 'morning' ? (
                                <div className="flex items-center gap-2 shrink-0">
                                    <button onClick={() => markAllIn('Present')} className="h-10 px-3 bg-emerald-50 text-emerald-700 text-[9px] font-semibold uppercase tracking-wider rounded-xl border border-emerald-100 hover:bg-emerald-100 transition-colors">All Present</button>
                                    <button onClick={() => markAllIn('Absent')}  className="h-10 px-3 bg-red-50 text-red-600 text-[9px] font-semibold uppercase tracking-wider rounded-xl border border-red-100 hover:bg-red-100 transition-colors">All Absent</button>
                                </div>
                            ) : (
                                <div className="flex items-center gap-2 shrink-0">
                                    <button onClick={() => markAllOut('Checked out')} className="h-10 px-3 bg-teal-50 text-teal-700 text-[9px] font-semibold uppercase tracking-wider rounded-xl border border-teal-100 hover:bg-teal-100 transition-colors">All Checked Out</button>
                                    <button onClick={() => markAllOut('Missing')}     className="h-10 px-3 bg-slate-100 text-slate-600 text-[9px] font-semibold uppercase tracking-wider rounded-xl border border-slate-200 hover:bg-slate-200 transition-colors">All Missing</button>
                                </div>
                            )}
                        ...
                        <table className="w-full text-left border-collapse min-w-[680px]">
                            <thead>
                                <tr className="bg-slate-50 border-b border-black/5">
                                    ...
                                    <th className="px-4 py-3 text-[8px] font-semibold text-slate-400 uppercase tracking-[0.2em] border-r border-black/5">Staff Member</th>
                                    <th className="hidden md:table-cell px-4 py-3 text-[8px] font-semibold text-slate-400 uppercase tracking-[0.2em] border-r border-black/5">Role</th>
                                    <th className="px-4 py-3 text-[8px] font-semibold text-slate-400 uppercase tracking-[0.2em] text-center">
                                        {activeTab === 'morning' ? (
                                ...
                                {loading ? (
                                    <tr>
                                        <td colSpan={4} className="py-16 text-center">
                                            ...
                                            <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest mt-2">Loading staff…</p>
                                        </td>
                                    </tr>
                                ) : filtered.length === 0 ? (
                                    <tr>
                                        <td colSpan={4} className="py-16 text-center">
                                            ...
                                            <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest">No staff found</p>
                                        </td>
                                    </tr>
                                ) : filtered.map((s, idx) => (
                                    <tr key={s.teacher_id} className="hover:bg-slate-50/80 transition-colors border-b border-black/4 group">
                                        ...
                                        <td className="hidden md:table-cell px-4 py-3 border-r border-black/5">
                                            <span className="text-[9px] font-semibold text-slate-500 uppercase tracking-wider bg-slate-100 px-2.5 py-1 rounded-lg">
                    ...
                    <div className="px-6 py-4 bg-slate-50/60 border-t border-black/5 flex items-center justify-between">
                        ...
                        <span className="text-[8px] font-semibold text-slate-400 uppercase tracking-widest">{filtered.length} staff shown</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="px-5 py-4 border-b border-black/5 bg-gradient-to-r from-slate-50 to-white">
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                <div className="flex items-center gap-3 shrink-0">
                    ...
                    <div>
                        <p className="text-[11px] font-semibold text-slate-800 uppercase tracking-tight">{currentTerm || 'Term'} Staff Attendance</p>
                ...
                <div className="flex-1 space-y-2.5 min-w-0">
                    <div>
                        <div className="flex items-center justify-between mb-1">
                            <span className="text-[8px] font-semibold text-slate-500 uppercase tracking-widest">Staff Attendance Rate</span>
...
function StaffAvatar({ name }) {
    ...
    return (
        <div className={`w-9 h-9 rounded-xl bg-gradient-to-br ${bg} flex items-center justify-center text-white text-[11px] font-semibold shrink-0 shadow-sm`}>
            {initials}
        </div>
    );
}
...
function StatusChip({ status, type }) {
    ...
    return (
        <span className={`inline-block mt-0.5 text-[8px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-md border ${cls}`}>{status}</span>
    );
}
