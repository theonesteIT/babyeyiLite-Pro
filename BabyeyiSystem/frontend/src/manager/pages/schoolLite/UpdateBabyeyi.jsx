...
const getDefaultClassesForLevel = (lvl) => {
  ...
  const first = LEVEL_CLASSES[lvl]?.[0];
  return first ? [first] : ["P1"];
};
...
export function WizardContent({ session, onClose, onSuccess, editRecord = null }) {
    ...
    (async () => {
      try {
        ...
        setForm(prev => {
          ...
          let feeTargetStudents =
            mapped.schoolKind === "private" ? "private" : "public";
          if (mapped.schoolKind === "government_aided") {
            feeTargetStudents = base.category === "Private" ? "private" : "public";
          }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const qrResults = [];
      for (const { id, cls } of createdIds) {
        try {
          const qrRes  = await fetch(`${API_BASE}/babyeyi/${id}/qrcode`, { credentials:"include" });
          const qrJson = await qrRes.json();
          if (qrJson.success && qrJson.data?.qr_code_url) {
            qrResults.push({ id, cls, qrDataUrl: qrJson.data.qr_code_url, viewUrl: qrJson.data.qr_view_url || `${window.location.origin}/babyeyi/verify/${id}` });
          }
  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center text-center px-6 py-8">
        ...
        {qrGenerating ? (
          <div className="mb-5 p-4 rounded-2xl flex flex-col items-center gap-2 border w-full"
            style={{ background: C.goldBg, borderColor: C.goldBorder }}>
        ...
        <div className="grid grid-cols-2 gap-3 bg-white rounded-2xl p-4 border border-slate-200 shadow-sm mb-5 text-left w-full">
          {[
            ...
            { label:"Status",    val: !nesaApplies ? "✅ Saved" : exceeds && form.requestIncrease ? "⏳ Pending" : "✅ Approved" },
          ].map(f => (
            <div key={f.label}>
        ...
        <div className="flex gap-3 w-full">
          <button onClick={() => {
            setForm({
              ...
              category: schoolKind === "government_aided" ? "Public" : schoolKind === "private" ? "Private" : schoolKind === "government" ? "Public" : "Public",
            });
            ...
            className="flex-1 px-5 py-2.5 rounded-2xl font-bold shadow-lg text-sm transition-all hover:opacity-90"
            style={{ background: `linear-gradient(135deg, ${C.gold}, ${C.goldDark})`, color: C.dark }}>
            Create Another
          </button>
          {onClose && (
            <button onClick={onClose}
              className="flex-1 px-5 py-2.5 rounded-2xl font-bold text-sm border-2 hover:bg-slate-50 transition-all"
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 1: {
        const provinceNames = Object.keys(RW_LOCATIONS);
        const districts = form.province && RW_LOCATIONS[form.province] ? Object.keys(RW_LOCATIONS[form.province]) : [];
        const sectors   = form.province && form.district && RW_LOCATIONS[form.province]?.[form.district]
          ? Object.keys(RW_LOCATIONS[form.province][form.district]) : [];
        const cells     = form.province && form.district && form.sector && RW_LOCATIONS[form.province]?.[form.district]?.[form.sector]
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select value={form.province}
                      onChange={e => { up("province", e.target.value); up("district",""); up("sector",""); up("cell",""); }}
                  ...
                  <div>
                    ...
                    <select value={form.district}
                      onChange={e => { up("district", e.target.value); up("sector",""); up("cell",""); }}
                  ...
                  <div>
                    ...
                    <select value={form.sector}
                      onChange={e => { up("sector", e.target.value); up("cell",""); }}
                  ...
                  <div>
                    ...
                    <select value={form.cell} onChange={e => up("cell", e.target.value)}
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (!next) setShowIncreaseModal(false);
                      else {
        ...
        return (
          <div className="space-y-4">
            ...
            <div>
              ...
              {extraBanks.length === 0 ? (
                <div className="border-2 border-dashed rounded-2xl p-5 text-center" style={{ borderColor: C.goldBorder, background: C.goldBg }}>
                  <p className="text-xs font-bold" style={{ color: C.goldDark }}>No additional bank accounts</p>
                  <p className="text-[10px] mt-0.5" style={{ color: C.goldDeep }}>Click "Add Bank" to add another payment account</p>
                </div>
              ) : (
                <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              ...
              <div key={idx} className="bg-white border-2 rounded-2xl p-4 space-y-3" style={{ borderColor: C.goldBorder }}>
                ...
                {previews[item.dbKey] && (
                  <div className="rounded-xl p-3 flex items-center justify-center min-h-[80px]"
                ...
                <FileZone
                  ...
                  file={form[item.formKey]}
                  fromDB={dbAssets[item.dbKey]}
                  ...
                  onFile={async f => {
                    ...
                    if (f) {
                      ...
                      if (url) {
                        const full = toAssetUrl(url);
                        setPreviews(p => ({ ...p, [item.dbKey]: full }));
                        setDbAssets(d => ({ ...d, [item.dbKey]: true }));
                      }
                    } else {
                      if (!dbAssets[item.dbKey]) setPreviews(p => ({ ...p, [item.dbKey]: null }));
                    }
                  }}
      ...
      case 7: {
        ...
        const removeLeader = (idx) => { if (leaders.length === 1) return; up("leaders", leaders.filter((_, i) => i !== idx)); };
        ...
        return (
          <div className="space-y-4">
                  ...
                  <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div className="sm:col-span-2">
                      <label className="block text-[10px] font-bold uppercase mb-1 flex items-center gap-1"
                        ...
                        <I n="user" size={10} color={C.darkMid} /> Full Name <span style={{ color: C.red }}>*</span>
                      </label>
                      <input value={leader.name} onChange={e => updateLeader(idx, "name", e.target.value)}
                        placeholder="e.g. Jean Pierre UWIMANA" className={inp} style={{ borderColor: C.goldBorder }} />
                    </div>
                    ...
                    <div className="sm:col-span-2">
                      <label className="block text-[10px] font-bold uppercase mb-1 flex items-center gap-1"
                        style={{ color: C.darkMid }}>
                        <I n="layers" size={10} color={C.darkMid} /> Role / Position <span style={{ color: C.red }}>*</span>
                      </label>
                      <select
                      ...
                      {/* Free-type fallback for custom / "Other" roles */}
                      {!LEADER_ROLE_PRESETS.includes(leader.role) && (
                        <input value={leader.role} onChange={e => updateLeader(idx, "role", e.target.value)}
                          placeholder="Or type a custom role…" className={`${inp} mt-2`} style={{ borderColor: C.goldBorder }} />
                      )}
                    </div>
                    ...
                    <div>
                      <label className="block text-[10px] font-bold uppercase mb-1 flex items-center gap-1"
                        style={{ color: C.darkMid }}>
                        <I n="phone" size={10} color={C.darkMid} /> Phone
                      </label>
                      <div className="relative">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] font-bold" style={{ color: C.goldDark }}>+250</span>
                        <input value={leader.phone} onChange={e => updateLeader(idx, "phone", e.target.value)}
                          placeholder="7XXXXXXXX" className={`${inp} pl-12 font-mono`} style={{ borderColor: C.goldBorder }} />
                      </div>
                    </div>
                    ...
                    <div>
                      <label className="block text-[10px] font-bold uppercase mb-1 flex items-center gap-1"
                        style={{ color: C.darkMid }}>
                        <I n="mail" size={10} color={C.darkMid} /> Email
                      </label>
                      <input type="email" value={leader.email} onChange={e => updateLeader(idx, "email", e.target.value)}
                        placeholder="e.g. director@school.rw" className={inp} style={{ borderColor: C.goldBorder }} />
                    </div>
                  </div>

                  {/* Contact preview pills */}
            ...
            <button type="button" onClick={addLeader}
              ...
              <div className="w-7 h-7 rounded-xl flex items-center justify-center" style={{ background: C.gold }}>
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 overflow-hidden w-full" style={{ fontFamily: "'Montserrat', sans-serif" }}>
      ...
      <div className="border-t px-4 sm:px-6 py-3 flex items-center gap-2 shrink-0 bg-white"
        ...
        {!isLast ? (
          ...
          <button onClick={handleSave} disabled={saving || qrGenerating}
            className="flex items-center gap-2 px-4 sm:px-6 py-2.5 rounded-xl font-bold text-xs sm:text-sm active:scale-95 transition-all disabled:opacity-60"
      ...
      {/* Increase request modal (matches Babyeyi.jsx flow) */}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => {
                    ...
                    up("requestOtherReason", val);
                    const parts = [...(form.requestReasons||[])];
...
// ════════════════════════════════════════════════════════════
// MAIN APP — shows list view with modal trigger
// ════════════════════════════════════════════════════════════
    ...
    <div className="min-h-screen" style={{ background: C.goldBg, fontFamily: "'Montserrat', sans-serif" }}>
      ...
      <div className="fixed bottom-6 right-6 z-40">
        <button
          ...
          <Svg d={ic.plus} size={18} color={C.dark} />
          Create Babyeyi
        </button>
      </div>
