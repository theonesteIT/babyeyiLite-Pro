        ...
        <div className="animate-in fade-in duration-700 bg-re-bg min-h-screen">
            ...
            <div className="relative w-full min-h-[280px] overflow-hidden bg-[#c87800]">
                ...
                <div className="relative z-20 max-w-[1600px] mx-auto px-6 md:px-12 pt-16 pb-24 flex items-center gap-8">
                    ...
                    <div className="hidden md:flex shrink-0 w-24 h-24 rounded-[32px] border border-white/10 bg-white/5 items-center justify-center backdrop-blur-xl shadow-sm relative overflow-hidden group">
                        ...
                        <GraduationCap size={40} style={{ color: "#FEBF10" }} className="" />
                    </div>

                    <div className="space-y-1">
                        <div className="flex items-center gap-2 mb-2">
                            <span className="w-6 h-1 rounded-full animate-pulse" style={{ background: "#FEBF10" }}></span>
                            <p className="text-[10px] font-semibold uppercase tracking-[0.3em]" style={{ color: "#FEBF10" }}>Institutional Intelligence</p>
                        </div>
            ...
            <div className="max-w-[1600px] mx-auto px-6 md:px-12 -mt-4 sm:-mt-5 md:-mt-6 pt-2 relative z-20 pb-20">
                        ...
                        <div className="lg:col-span-3 grid grid-cols-2 md:grid-cols-4 divide-x divide-y md:divide-y-0 divide-black/5">
                            <div className="p-4 sm:p-8 flex flex-col items-center justify-center text-center group hover:bg-re-bg/20 transition-all cursor-default relative overflow-hidden">
                                ...
                                <div className="flex items-baseline gap-1 relative z-10">
                                    <span className="text-sm sm:text-2xl font-semibold text-re-text tracking-tighter group-hover:text-[#1E3A5F] transition-colors">{stats.instGpa}</span>
                                    <span className="text-[10px] font-semibold uppercase tracking-widest text-emerald-500">{stats.gpaTrend}</span>
                                </div>
                                <p className="text-[6px] sm:text-[8px] font-semibold text-re-text-muted uppercase tracking-[0.2em] mt-0.5 sm:mt-1 opacity-60">Institutional GPA</p>
                            </div>
                                ...
                                {activeDropdown === 'export' && (
                                    <>
                                        ...
                                        <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-black/10 shadow-md rounded-2xl overflow-hidden py-1 z-[50] animate-in slide-in-from-top-2 duration-200">
                                            <button className="w-full text-left px-4 py-3 text-[10px] font-semibold uppercase tracking-widest text-[#1E3A5F] hover:bg-re-navy/5 transition-colors flex items-center gap-2.5">
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-re-bg/20 border-b border-black/5">
                                    <th className="px-4 sm:px-8 py-3 sm:py-4 text-[7px] sm:text-[8px] font-semibold text-re-text-muted uppercase tracking-[0.2em] opacity-40 border-r border-black/5 last:border-r-0">Class Intel</th>
                                    <th className="hidden md:table-cell px-8 py-4 text-[8px] font-semibold text-re-text-muted uppercase tracking-[0.2em] opacity-40 border-r border-black/5">Performance Core</th>
                                    <th className="hidden md:table-cell px-8 py-4 text-[8px] font-semibold text-re-text-muted uppercase tracking-[0.2em] opacity-40 border-r border-black/5">Metrics Overview</th>
                                ...
                                {loading ? (
                                    <tr>
                                        <td colSpan="5" className="p-12 text-center">
                                            ...
                                            <p className="text-[10px] font-semibold text-re-text-muted uppercase tracking-widest">Loading Analytics...</p>
                                        </td>
                                    </tr>
                                ) : filteredAnalytics.length === 0 ? (
                                    <tr>
                                        <td colSpan="5" className="p-12 text-center text-[10px] font-semibold text-re-text-muted uppercase tracking-widest italic opacity-40">
                                        ...
                                        <td className="hidden md:table-cell px-8 py-5">
                                            <div className="flex flex-col gap-0.5">
                                                <div className="flex items-center gap-2">
                                                    <p className="text-[10px] font-semibold text-re-text uppercase tracking-tight">GPA: {cls.avgGpa}</p>
                                        ...
                                        <td className="hidden md:table-cell px-8 py-5">
                                            <div className="space-y-1.5 max-w-[120px]">
                                                <div className="flex items-center justify-between">
                                                    <p className="text-[9px] font-semibold text-re-text">{cls.passRate}% Pass Rate</p>
                                                </div>
                    ...
                    <div className="flex px-4 sm:px-8 py-5 bg-re-bg/20 border-t border-black/5 flex-row items-center justify-between gap-4">
                        <div className="flex items-center gap-3 sm:gap-4">
                            <div className="hidden xs:flex items-center gap-2">
                                ...
                                <p className="text-[6px] sm:text-[8px] font-semibold text-re-text-muted uppercase tracking-[0.2em] opacity-40 italic whitespace-nowrap">Analytics Verified</p>
                            </div>
                            <div className="hidden xs:block w-px h-3 bg-black/10"></div>
                            <p className="text-[6px] sm:text-[8px] font-semibold text-re-text-muted uppercase tracking-[0.2em] opacity-40 italic whitespace-nowrap">Analytics for {selectedYear} • {selectedTerm}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col md:flex-row items-center justify-between mt-8 px-4 gap-4">
                    <p className="text-[7px] text-re-text-muted font-semibold uppercase tracking-[0.3em] opacity-30 italic">Powered by Babyeyi Intelligence Core</p>
                    <div className="flex items-center gap-4 opacity-20">
                        <span className="text-[8px] font-semibold text-re-text uppercase tracking-widest">Analytics Module</span>
                    </div>
                </div>
            </div>
        </div>
