...
export async function subscribeTeacherPortalPush(api) {
  ...
  if (!res.data?.success || !res.data?.publicKey) {
    throw new Error(res.data?.message || 'Web Push is not enabled on the server.');
  }
  ...
  try {
    ...
    if (msg.includes('MIME type') || msg.includes('text/html')) {
      throw new Error(
        'Service worker file is missing. Ensure public/sw.js exists and restart the dev server.'
      );
    }
    throw err;
  }
