/**
 * Web Push for Shule Avance (Babyeyi backend VAPID + `/public/sw.js`).
 */
...
export async function getShuleAvancePushState() {
 ...
 */
export async function subscribeShuleAvancePush(api) {
  if (!isWebPushEnvironmentSupported()) {
 ...
 * @param {import('axios').AxiosInstance} api
 */
export async function unsubscribeShuleAvancePush(api) {
  const reg = await navigator.serviceWorker.getRegistration();
  if (!reg) return;
  const sub = await reg.pushManager.getSubscription();
  if (!sub) return;
