/**
 * Web Push for Babyeyi school portals (VAPID on backend + `/sw.js`).
 */
...
export async function getPortalPushState() {
 ...
 */
export async function subscribePortalPush(api) {
  if (!isWebPushEnvironmentSupported()) {
    throw new Error('This browser does not support Web Push.');
  }
  const res = await api.get('/portal/push/vapid-key');
  ...
  await api.post('/portal/push/subscribe', sub.toJSON());
  return sub;
}
 ...
 * @param {import('axios').AxiosInstance} api
 */
export async function unsubscribePortalPush(api) {
  const reg = await navigator.serviceWorker.getRegistration();
  if (!reg) return;
  const sub = await reg.pushManager.getSubscription();
  if (!sub) return;
  ...
  try {
    await api.post('/portal/push/unsubscribe', { endpoint: json.endpoint });
  } finally {
