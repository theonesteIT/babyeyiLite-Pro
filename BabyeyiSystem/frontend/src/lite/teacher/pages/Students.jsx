...
import {
    ...
    Phone, Clock, Home, Tag, Edit3, Printer, Eye, CheckCircle, RefreshCw, ChevronDown, ChevronUp
} from 'lucide-react';
            ...
            <div className="fixed inset-y-0 right-0 z-[110] w-full md:w-[420px] bg-white shadow-[-20px_0_60px_-15px_rgba(0,0,0,0.1)] flex flex-col animate-in slide-in-from-right duration-500 ease-out">
                ...
                <div className="flex items-center justify-between px-8 py-6 border-b border-black/5 bg-white shrink-0">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-re-bg border border-black/5 flex items-center justify-center text-re-text font-bold text-lg shadow-inner relative overflow-hidden">
                            <span className="relative z-10">{student.name.charAt(0)}</span>
                            <div className="absolute inset-0 bg-re-grad-orange opacity-5"></div>
                        </div>
                        <div>
                            <h3 className="font-bold text-re-text text-base">{student.name}</h3>
                            <p className="text-[10px] text-re-text-muted font-bold flex items-center gap-1 mt-0.5 opacity-40">
                ...
                <div className="flex-1 overflow-y-auto px-8 py-8 space-y-8 custom-scrollbar">
                    ...
                    <div className={`flex items-center gap-3 px-4 py-3 rounded-2xl border ${student.status === 'On leave' ? 'bg-blue-50 border-blue-100/50' : 'bg-re-orange/5 border-re-orange/10'}`}>
                        <div className={`p-1.5 rounded-lg ${student.status === 'On leave' ? 'bg-blue-500' : 'bg-re-orange'} text-white`}>
                            <Award size={14} />
                        </div>
                        <div>
                            <p className="text-[10px] font-bold text-re-text">{student.status}</p>
                            <p className="text-[10px] text-re-text/40 font-bold mt-0.5">Registry: {student.grade}{student.stream ? ` · ${student.stream}` : ''}</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-re-bg rounded-3xl p-5 border border-black/5 shadow-inner relative overflow-hidden group">
                            ...
                            <p className="text-[10px] text-re-text-muted font-bold mb-1 relative z-10 opacity-60">Avg marks</p>
                            <div className="flex items-baseline gap-1 relative z-10">
                                <span className="text-2xl font-bold text-re-text">
                                    {student.gpa != null ? (Number(student.gpa) * 25).toFixed(0) : '—'}
                                </span>
                                {student.gpa != null && (
                                    <span className="text-[10px] font-bold text-re-orange">%</span>
                                )}
                            </div>
                        </div>
                        <div className="bg-re-bg rounded-3xl p-5 border border-black/5 shadow-inner relative overflow-hidden group text-right">
                            ...
                            <p className="text-[10px] text-re-text-muted font-bold mb-1 relative z-10 opacity-60">Roll-call presence (120d)</p>
                            <div className="flex items-baseline gap-1 justify-end relative z-10">
                                <span className="text-2xl font-bold text-re-text">{student.attendance ?? '—'}</span>
                                <span className="text-[10px] font-bold text-re-orange">%</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center gap-2 mb-2">
                            <span className="text-[10px] font-bold text-re-text-muted opacity-40">Profile Intelligence</span>
                        ...
                        {[
                            ...
                            <div key={i} className="flex items-center justify-between group">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] font-bold text-re-text-muted">{item.label}</span>
                                </div>
                                <div className="flex-1 mx-3 border-b border-dashed border-black/10 group-hover:border-re-orange/30 transition-colors" />
                                <span className="text-[10px] font-bold text-re-text">{item.value}</span>
                            </div>
                        ))}
                    </div>
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-4">
                            <span className="text-[10px] font-bold text-re-text-muted opacity-40">Scholastic Activity Log</span>
                            <div className="flex-1 h-px bg-black/5" />
                        </div>
                                ...
                                <div key={i} className="flex items-start gap-3 p-4 rounded-2xl bg-re-bg/50 border border-black/[0.02] group hover:bg-white hover:border-black/5 transition-all">
                                    ...
                                    <div className="min-w-0">
                                        <div className="flex items-center gap-2 mb-0.5">
                                            <span className="text-[10px] font-bold text-re-text">{log.type}</span>
                                            <span className="w-1 h-1 bg-black/10 rounded-full"></span>
                                            <span className="text-[10px] font-medium text-re-text-muted opacity-40">{log.date}</span>
                                        </div>
                                        <p className="text-[10px] font-bold text-re-text-muted leading-relaxed group-hover:text-re-text transition-colors">{log.msg}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <div className="px-8 py-6 border-t border-black/5 bg-re-bg/20 flex flex-col gap-3">
                    <button
                        ...
                        className="h-12 w-full flex items-center justify-center gap-2 bg-re-grad-orange text-white rounded-2xl font-bold text-[11px] shadow-re-glow hover:scale-[1.02] active:scale-95 transition-all"
                    >
                        <Activity size={15} /> <span>+/-</span> Conduct Marks
                    </button>
                    <div className="grid grid-cols-2 gap-3">
                        <button className="h-12 flex items-center justify-center gap-2 bg-white border border-black/5 text-re-text font-bold text-[11px] rounded-2xl hover:bg-re-bg transition-all">
    ...
    return (
        <div className="animate-in fade-in duration-700 bg-white md:bg-re-bg min-h-screen pt-0">
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white w-full max-w-sm rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                        <div className="px-6 py-5 border-b border-black/5 flex items-center justify-between">
                            <div>
                                <h3 className="text-sm font-bold text-re-text">Select Class</h3>
                        ...
                        <div className="p-4 grid grid-cols-2 gap-2">
                            {classes.map(cls => (
                                <button
                                    ...
                                    className={`h-12 flex items-center gap-2 px-4 rounded-xl border text-[10px] font-bold transition-all ${selectedClass === cls
            ...
            <div className="hidden md:block relative w-full min-h-[280px] overflow-hidden">
                ...
                <img src="/teacher.png" alt="Hero" className="absolute inset-0 w-full h-full object-cover scale-105" />
                ...
                <div className="relative z-20 max-w-[1600px] mx-auto px-6 md:px-12 pt-16 pb-24">
                    <div className="space-y-1">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <p className="text-[10px] font-bold text-white/70">Institutional Repository</p>
                        </div>
                        <h1 className="text-2xl sm:text-4xl md:text-5xl font-bold text-white leading-none mb-2 mt-2">Students <span className="text-re-orange">List</span></h1>
                        <p className="text-[10px] md:text-sm font-medium text-white/55 max-w-lg leading-relaxed italic">Professional Academic & Behavioral Analytics View</p>
                    </div>
                </div>
            </div>

            {/* ── Consolidated High-Fidelity Card (Dashboard Stats Style) ── */}
            <div className="max-w-[1600px] mx-auto px-0 md:px-12 mt-0 md:-mt-24 relative z-20 pb-10 md:pb-20">
                <div className="bg-white md:rounded-t-[32px] md:shadow-2xl md:border md:border-black/5 overflow-hidden flex flex-col min-h-[calc(100vh-56px)] md:min-h-0">
                    ...
                    <div className="hidden md:grid grid-cols-1 lg:grid-cols-4 border-b border-black/5">
                        ...
                        <div className="lg:col-span-3 grid grid-cols-2 md:grid-cols-4 divide-x divide-y md:divide-y-0 divide-black/5">
                            {[
                                ...
                                <div key={i} className="p-4 sm:p-8 flex flex-col items-center justify-center text-center group hover:bg-re-bg/20 transition-all cursor-default">
                                    ...
                                    <span className="text-sm sm:text-2xl font-bold text-re-text group-hover:text-re-orange transition-colors">
                                        {stat.value}
                                    </span>
                                    <p className="text-[10px] font-bold text-re-text-muted mt-0.5 sm:mt-1 opacity-60">
                                        {stat.label}
                                    </p>
                                </div>
                            ))}
                        ...
                        <div className="hidden lg:flex flex-col border-l border-black/5 bg-re-bg/30 p-6 justify-center gap-3">
                            <button className="w-full h-11 flex items-center justify-center gap-2 bg-re-grad-orange text-white rounded-xl font-bold text-[11px] shadow-re-glow hover:scale-[1.02] active:scale-95 transition-all">
                                <Download size={14} />
                                Export Registry
                            </button>
                            <button className="w-full h-11 flex items-center justify-center gap-2 bg-white border border-black/5 text-re-text font-bold text-[11px] rounded-xl hover:bg-re-bg hover:shadow-re-soft transition-all">
                    ...
                    <div className={`${!isClassSelected ? 'hidden md:flex' : 'flex'} px-4 py-3 lg:px-3 lg:py-2 border-b border-black/5 flex-col lg:flex-row lg:flex-nowrap lg:items-center lg:justify-start gap-4 lg:gap-2 bg-re-bg/20`}>
                        
                        {/* Desktop Controls (Hidden on mobile) */}
                        <div className="hidden md:flex flex-col lg:flex-row lg:flex-nowrap lg:items-center gap-3 lg:gap-2 w-full lg:flex-1 lg:min-w-0 animate-in slide-in-from-top-2 duration-300">
                            <div className="relative w-full lg:flex-1 lg:min-w-[7rem] lg:max-w-[12rem] group">
                                <Search size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-re-text-muted/50 group-focus-within:text-re-orange transition-colors hidden md:block z-[1] pointer-events-none" />
                                <input
                                    type="text"
                            ...
                            <div className="flex flex-col sm:flex-row gap-2 lg:gap-1.5 w-full lg:w-auto lg:items-center lg:shrink-0">
                                <button
                                    ...
                                    className="h-10 lg:h-8 px-3 bg-white border border-black/[0.07] rounded-xl lg:rounded-lg flex items-center justify-center gap-2 text-re-text font-medium text-[10px] hover:bg-re-bg transition-all shadow-sm whitespace-nowrap"
                                >
                                    <Filter size={13} className="text-re-orange" />
                                    {showClassFilter ? 'Hide Classes' : 'Show Classes'}
                                </button>
                                <button
                                    onClick={() => openConductModal(null)}
                                    className="h-10 lg:h-8 px-3 bg-re-grad-orange text-white rounded-xl lg:rounded-lg font-medium text-[10px] shadow-re-glow hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-1.5 whitespace-nowrap"
                                >
                                    <Activity size={12} />
                                    Conduct Marks
                                </button>
                    ...
                    {showClassFilter && (
                        <div className={`${!isClassSelected ? 'hidden md:flex' : 'flex'} px-4 lg:px-3 py-3 bg-re-bg/20 border-b border-black/5 items-center overflow-x-auto gap-2 custom-scrollbar animate-in slide-in-from-top-2 fade-in duration-300`}>

                            {/* All Classes Option */}
                            <button
                                onClick={() => setSelectedClass('View All')}
                                className={`flex items-center justify-center gap-1.5 shrink-0 h-8 lg:h-7 px-3 lg:px-4 rounded-xl lg:rounded-lg border font-black text-[8px] lg:text-[7px]   transition-all whitespace-nowrap ${selectedClass === 'View All'
                            ...
                            {classes.map((cls, idx) => (
                                <button
                                    ...
                                    className={`flex items-center justify-center gap-1.5 shrink-0 h-8 lg:h-7 px-3 lg:px-4 rounded-xl lg:rounded-lg border font-black text-[8px] lg:text-[7px]   transition-all whitespace-nowrap ${idx > 1 ? 'hidden md:flex' : ''} ${selectedClass === cls
                                        ? 'bg-re-orange border-re-orange text-white shadow-re-glow hover:scale-105'
                                        : 'bg-white border-black/5 text-re-text-muted hover:bg-re-bg hover:text-re-text'
                                        }`}
                                >
                            ...
                            <button
                                ...
                                className="md:hidden flex items-center justify-center gap-1.5 shrink-0 h-7 px-3 rounded-lg border border-black/5 bg-white text-re-orange font-black text-[7px]   transition-all shadow-sm active:scale-95"
                            >
                                <Plus size={10} /> More
                            </button>
                        </div>
                    ...
                    {!isClassSelected && (
                        <div className="md:hidden bg-white animate-in fade-in zoom-in-95 duration-500">
                            <div className="flex flex-col overflow-hidden">
                                <button
                                    ...
                                    className="flex items-center gap-4 px-6 py-2 bg-re-grad-orange text-white active:scale-[0.98] transition-all group"
                                >
                                ...
                                {classes.map((cls, i) => (
                                    <button
                                        ...
                                        className={`flex items-center ${i % 2 ==0 && 'bg-re-bg'} gap-4 px-6 py-2 hover:bg-re-bg active:bg-re-bg transition-all group ${i !== 0 ? 'border-t border-black/5' : ''}`}
                                    >
                                        <div className="w-8 h-8 rounded-full bg-re-grad-orange text-white flex items-center justify-center shadow-re-glow shrink-0">
                                            <Users size={16} />
                                        </div>
                                        <div className="flex-1 text-left">
                    ...
                    <div className={`${!isClassSelected ? 'hidden md:block' : 'block'} overflow-x-auto bg-white`}>
                            ...
                            <>
                                <div className="md:hidden px-6 py-3 bg-white border-b border-black/5 flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-xs text-re-text  ">{selectedClass}  students list</span>
                                    </div>
                                    <button
                                        onClick={() => setIsClassSelected(false)}
                                        className="text-xs text-re-orange   hover:underline"
                                    >
                                        Change Class
                                    </button>
                                </div>
                                ...
                                {filteredStudents.length > 0 && (
                                    <div className="md:hidden px-6 py-3 bg-re-bg/40 border-b border-black/5 flex items-center gap-3 w-full">
                                        ...
                                        <button 
                                            onClick={() => openConductModal(null)}
                                            className="flex items-center gap-1.5 text-[11px] font-bold text-re-orange whitespace-nowrap active:scale-95 transition-all"
                                        >
                                            <Activity size={14} /> +/- Conduct marks
                                        </button>
                                    </div>
                                )}
                            </>
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-re-bg/20 border-b border-black/5">
                                        <th className="px-4 sm:px-8 py-3 sm:py-4 text-[10px] font-medium text-re-text-muted opacity-40 border-r border-black/5 last:border-r-0">Student Info</th>
                                        <th className="hidden md:table-cell px-8 py-4 text-[10px] font-medium text-re-text-muted opacity-40 border-r border-black/5">Academics</th>
                                        <th className="hidden md:table-cell px-8 py-4 text-[10px] font-medium text-re-text-muted opacity-40 border-r border-black/5">Presence</th>
                                        <th className="hidden md:table-cell px-8 py-4 text-[10px] font-medium text-re-text-muted opacity-40 border-r border-black/5">Status</th>
                                        <th className="px-4 sm:px-8 py-3 sm:py-4 text-[10px] font-bold text-re-text-muted opacity-40 text-right">Actions</th>
                                    </tr>
                                </thead>
                                ...
                                {loading ? (
                                    <tr>
                                        <td colSpan="5" className="p-12 text-center">
                                            ...
                                            <p className="text-[10px] font-bold text-re-text-muted  ">Accessing Central Registry...</p>
                                        </td>
                                    </tr>
                                ) : (
                                    <>
                                                ...
                                                >
                                                    <td className="px-4 sm:px-8 py-3 sm:py-5 border-r border-black/5 last:border-r-0">
                                                        <div className="flex items-center gap-3 sm:gap-4">
                                                            ...
                                                            <div>
                                                                <p className="text-xs sm:text-sm font-bold text-re-text leading-none mb-1 group-hover:text-re-orange transition-colors">{s.name}</p>
                                                                <p className="text-[10px] font-medium text-re-text-muted opacity-30 leading-none font-mono italic">{s.id}</p>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className="hidden md:table-cell px-8 py-5">
                                                        <div className="flex flex-col gap-0.5">
                                                            <p className="text-[10px] font-medium text-re-text">{s.grade}</p>
                                                            <div className="flex items-center gap-2">
                                                                <p className="text-[10px] font-medium text-re-text-muted opacity-30">
                                                                    Marks {s.gpa != null ? s.gpa : '—'}
                                                                </p>
                                                                <div className="w-1 h-3 bg-black/5 rounded-full shrink-0"></div>
                                                                <p className="text-[10px] font-medium text-re-orange italic">{s.stream}</p>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className="hidden md:table-cell px-8 py-5">
                                                        <div className="space-y-1.5 max-w-[100px]">
                                                            <div className="flex items-center justify-between">
                                                                <p className="text-[10px] font-medium text-re-text">{s.attendance}%</p>
                                                            </div>
                                                    ...
                                                    <td className="hidden md:table-cell px-8 py-5">
                                                        <div className={`inline-flex px-3 py-1.5 rounded-lg text-[10px] font-medium ring-1 ring-inset ${s.status === 'On leave' ? 'bg-blue-50 text-blue-600 ring-blue-500/20' :
                                                            ...
                                                            <div className="relative">
                                                                ...
                                                                {openDropdownId === s.id && (
                                                                    <>
                                                                        ...
                                                                        >
                                                                            <button
                                                                                className="w-full text-left px-4 py-3 text-[10px] font-medium text-re-text hover:bg-re-bg transition-colors flex items-center gap-2.5"
                                                                                onClick={() => { setSelectedStudent(s); setOpenDropdownId(null); }}
                                                                            >
                                                                                <Eye size={13} className="text-re-text-muted" /> View Profile
                                                                            </button>
                                                                            <button
                                                                                className="w-full text-left px-4 py-3 text-[10px] font-medium text-re-orange hover:bg-re-orange/5 transition-colors flex items-center gap-2.5 border-t border-black/5"
                                                                                onClick={() => { setOpenDropdownId(null); openConductModal(s); }}
                                                                            >
                                                                                <Activity size={13} /> <span>+/-</span> Conduct Marks
                                                                            </button>
                                                                            <button
                                                                                className="w-full text-left px-4 py-3 text-[10px] font-medium text-re-text hover:bg-re-bg transition-colors flex items-center gap-2.5"
                                                                            ...
                                                                            <button
                                                                                className="w-full text-left px-4 py-3 text-[10px] font-medium text-re-text hover:bg-re-bg transition-colors flex items-center gap-2.5 border-t border-black/5"
                                                                                onClick={() => setOpenDropdownId(null)}
                                                                            >
                        ...
                        {isClassSelected && mockStudents.length === 0 && !loading && (
                            <div className="flex flex-col items-center justify-center py-24 px-6 text-center animate-in fade-in zoom-in-95 duration-700">
                                ...
                                <h3 className="text-base font-bold text-re-text mb-2">No Students Enrolled</h3>
                                <p className="text-[11px] font-medium text-re-text-muted leading-relaxed max-w-sm">
                                    This class registry is currently empty. Please verify the enrollment data or contact your administration for registry synchronization.
                                </p>
                                <button
                                    onClick={() => setSelectedClass(null)}
                                    className="mt-8 px-8 h-11 rounded-xl bg-re-bg border border-black/5 text-[11px] font-bold text-re-orange hover:bg-re-orange/5 transition-all"
                                >
                                    Change Class
                                </button>
                            </div>
                    ...
                    {isClassSelected && filteredStudents.length > 0 && (
                        <div className="hidden md:flex px-4 sm:px-8 py-5 bg-re-bg/20 border-t border-black/5 flex-row items-center justify-between gap-4">
                            ...
                            <div className="flex items-center gap-1 sm:gap-1.5">
                                <button className="h-7 sm:h-8 px-2 sm:px-4 rounded-lg bg-white border border-black/5 text-[10px] font-bold text-re-text-muted opacity-40 hover:opacity-100 transition-all font-mono italic">Prev_set</button>
                                <div className="h-7 sm:h-8 px-3 sm:px-4 rounded-lg flex items-center justify-center bg-white border border-black/5 text-[10px] font-bold text-re-text">Page 01</div>
                                <button className="h-7 sm:h-8 px-3 sm:px-4 rounded-lg bg-re-grad-orange text-white text-[10px] font-bold shadow-re-glow">Next_set</button>
                            </div>
                        </div>
                    )}
                </div>
